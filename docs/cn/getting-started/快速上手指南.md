...
<tbody><tr id="row1372280416410"><td class="cellrowborder" valign="top" width="20.580000000000002%" headers="mcps1.1.3.1.1 "><p id="p3734547016410"><a name="p3734547016410"></a><a name="p3734547016410"></a><a name="image2670064316410"></a><a name="image2670064316410"></a><span><img src="figures/zh-cn_image_0000002446762205.png" alt="" /></span></p>
</td>
<td class="cellrowborder" valign="top" width="79.42%" headers="mcps1.1.3.1.2 "><p id="p1757432116410"><a name="p1757432116410"></a><a name="p1757432116410"></a>表示如不避免则将会导致死亡或严重伤害的具有高等级风险的危害。</p>
</td>
</tr>
<tr id="row466863216410"><td class="cellrowborder" valign="top" width="20.580000000000002%" headers="mcps1.1.3.1.1 "><p id="p1432579516410"><a name="p1432579516410"></a><a name="p1432579516410"></a><a name="image4895582316410"></a><a name="image4895582316410"></a><span><img src="figures/zh-cn_image_0000002446762209.png" alt="" /></span></p>
</td>
<td class="cellrowborder" valign="top" width="79.42%" headers="mcps1.1.3.1.2 "><p id="p959197916410"><a name="p959197916410"></a><a name="p959197916410"></a>表示如不避免则可能导致死亡或严重伤害的具有中等级风险的危害。</p>
</td>
</tr>
<tr id="row123863216410"><td class="cellrowborder" valign="top" width="20.580000000000002%" headers="mcps1.1.3.1.1 "><p id="p1232579516410"><a name="p1232579516410"></a><a name="p1232579516410"></a><a name="image1235582316410"></a><a name="image1235582316410"></a><span><img src="figures/zh-cn_image_0000002413043268.png" alt="" /></span></p>
</td>
<td class="cellrowborder" valign="top" width="79.42%" headers="mcps1.1.3.1.2 "><p id="p123197916410"><a name="p123197916410"></a><a name="p123197916410"></a>表示如不避免则可能导致轻微或中度伤害的具有低等级风险的危害。</p>
</td>
</tr>
<tr id="row5786682116410"><td class="cellrowborder" valign="top" width="20.580000000000002%" headers="mcps1.1.3.1.1 "><p id="p2204984716410"><a name="p2204984716410"></a><a name="p2204984716410"></a><a name="image4504446716410"></a><a name="image4504446716410"></a><span><img src="figures/zh-cn_image_0000002446642333.png" alt="" /></span></p>
</td>
<td class="cellrowborder" valign="top" width="79.42%" headers="mcps1.1.3.1.2 "><p id="p4388861916410"><a name="p4388861916410"></a><a name="p4388861916410"></a>用于传递设备或环境安全警示信息。如不避免则可能会导致设备损坏、数据丢失、设备性能降低或其它不可预知的结果。</p>
<p id="p1238861916410"><a name="p1238861916410"></a><a name="p1238861916410"></a>“须知”不涉及人身伤害。</p>
</td>
</tr>
<tr id="row2856923116410"><td class="cellrowborder" valign="top" width="20.580000000000002%" headers="mcps1.1.3.1.1 "><p id="p5555360116410"><a name="p5555360116410"></a><a name="p5555360116410"></a><a name="image799324016410"></a><a name="image799324016410"></a><span><img src="figures/zh-cn_image_0000002413043264.png" alt="" /></span></p>
</td>
...
图像分析引擎1与图像分析引擎2存在差异，详细差异请参见[《图像分析引擎2与图像分析引擎1使用差异说明》](../modules/ai/图像分析引擎2与图像分析引擎1使用差异说明.md)，请根据说明选择合适的引擎。本指南针对图像分析引擎2。
...
>-   AMCT模型压缩工具：
>    -   推荐使用AMCT的情况
>        提高模型精度和减少量化误差，请参见[《AMCT使用指南（Caffe）》](../modules/ai/AMCT使用指南（Caffe）.md)“参数调优说明”章节和[《AMCT使用指南（PyTorch）》](../modules/ai/AMCT使用指南（PyTorch）.md)“量化调优”章节。
>    -   必须使用AMCT的情况
>        量化重训，请参见[《AMCT使用指南（Caffe）》](../modules/ai/AMCT使用指南（Caffe）.md)、[《AMCT使用指南（PyTorch）》](../modules/ai/AMCT使用指南（PyTorch）.md)“量化感知训练”章节。
>-   ATC工具模型转换：请参见[《MindCmd使用指南》](../tools/MindCmd 使用指南.md)“模型转换”章节或[《ATC工具使用指南》](../modules/ai/ATC工具使用指南.md)，将训练好的模型转换成平台识别的离线模型，应用程序指定模型资源后即可进行推理；具体支持的Caffe算子规格请参见[《ATC工具使用指南》](../modules/ai/ATC工具使用指南.md)“算子规格说明”章节。
...
注：环境安装可具体参考[《驱动和开发环境安装指南》](Hi3403V100环境搭建指南.md)文档。
...
4.  执行如下命令进行安装（以下命令支持--install-path=<path\>等参数，具体参数说明请参考文档[《驱动和开发环境安装指南》](Hi3403V100环境搭建指南.md)）。
...
需要搭建交叉编译环境，具体方法请参考文档[《驱动和开发环境安装指南》](Hi3403V100环境搭建指南.md)“交叉编译环境准备”章节。
...
以PC sample中2\_object\_detection/yolo为例，在该Sample中，涉及的关键功能点如下所示。API 接口的详细介绍请参见[《应用开发指南》](应用开发指南.md)。
...
本章节介绍 ONNX 模型转换的基本操作, 使用 ATC工具将 ONNX 模型编译成 om 模型。请先参考[开发环境准备](Hi3403V100环境搭建指南.md)章节完成开发环境搭建。本节示例模型为开源模型 yolov5s。
...
ATC工具详细的用法请参考[《ATC工具使用指南》](../modules/ai/ATC工具使用指南.md)。
...
1.  准备离线模型om
    ...
    确认model目录下有yolov5\_original.om离线模型。模型编译，请参见[模型编译](应用开发指南.md)。
...
-   板端环境安装请参见[《Hi3403V100╱Hi3403V100 SDK 安装以及升级使用说明》](SS928V100╱SS927V100 SDK 安装以及升级使用说明.md)。
-   SVP ACL接口使用说明请参见[《应用开发指南》](应用开发指南.md)中“SVP ACL API参考”章节。
...
MindCmd是一个Python命令行工具，依赖Python3.7.5和CANN。如果还没有安装CANN软件包，请先参考[软件包安装](Hi3403V100环境搭建指南.md)章节完成安装。
...
2.  <a name="li134221448812"></a>配置CANN软件包安装路径
    ...
    <img src="figures/查看CANN配置路径.png" alt="" />
...
3.  查看版本信息
    ...
    <img src="figures/zh-cn_image_0000002446642329.png" alt="" />
...
<img src="figures/仿真运行.png" alt="" />

## 开发板运行<a name="ZH-CN_TOPIC_0000002446762169"></a>

请参考《SSxxxVxxx SDK 安装以及升级使用说明》完成板端基础环境安装。参考[《驱动和开发环境安装指南》](Hi3403V100环境搭建指南.md)“OpenSSH服务搭建”章节完成板端ssh环境安装，服务器NFS环境安装。
...
本节示例为[MindCmd工具准备](../tools/MindCmd 使用指南.md)完成sample网络yolov5s\_cpu.onnx的一键精度比对。所需模型文件、推理数据均位于SVP\__NNN_\_PC软件包中的2\_object\_detection/yolo样例目录，请参考资源准备[资源准备](Hi3403V100环境搭建指南.md)资源准备章节完成资源准备。
...
<img src="figures/精度比对展示.png" alt="" />

如果想了解各项参数的详细说明，请参考[《MindCmd使用指南》](../tools/MindCmd 使用指南.md)“10 精度比对”章节。
...
-   若原始模型与AMCT量化模型的推理结果存在精度问题时，请参考[《AMCT使用指南》](../modules/ai/AMCT使用指南（Caffe）.md)进行精度调优，或使用QAT解决
-   若指令编译阶段引入的精度问题，请对比cnn\_net\_tree.dot和原始模型（\*.dot查看请参考[《ATC工具使用指南》](../modules/ai/ATC工具使用指南.md)“2.3输出文件说明”章节），查看是否修改了网络结构，比较被修改网络结构层的后面层的相似度。
...
详细使用方式请参见[《应用开发指南》](应用开发指南.md)手册“8.6 Profiling性能数据采集”章节。
...
2.  参考1，新建文件_$HOME_/ssh.cfg。

    > **说明：** 
    >-   请参考[《MindCmd使用指南》](../tools/MindCmd 使用指南.md)“NFS环境搭建”章节配置NFS环境。
    >-   MindCmd工作路径与板端挂载路径的配置请参考[《MindCmd使用指南》](../tools/MindCmd 使用指南.md)“关于工作路径、挂载路径、数据卷路径和NFS共享路径”章节。
...
3.  执行Oneclick
    ...
    **图 1**  性能数据表格<a name="fig1239501163019"></a>  
    <img src="figures/性能数据表格.png" alt="" />

    **图 2**  性能数据表格存放位置<a name="fig1691436783"></a>  
    <img src="figures/性能数据表格存放位置.png" alt="" />

    **图 3**  模型整网性能数据<a name="fig1073092175015"></a>  
    <img src="figures/模型整网性能数据.png" alt="" />
    ...
    **图 4**  AI Core数据表（op\_summary\_\*.csv）<a name="fig6208184011418"></a>  
    <img src="figures/AI-Core数据表（op_summary_-csv）.png" alt="" />
...
如果想了解Profiling各项数据的详细说明，请参考[《MindCmd使用指南》](../tools/MindCmd 使用指南.md)“性能分析数据说明”章节。
...
-   使用AMCT对模型进行进一步量化压缩
    ...
    如果希望进一步提升性能，还可以参考[《AMCT使用指南》](../modules/ai/AMCT使用指南（Caffe）.md)使用AMCT工具进行混合精度量化调优，然后重新对精度进行分析。

-   设计高性能算子模型

    通过修改网络连接方式和设置算子属性，可对网络模型的性能做进一步优化。详见[《ATC工具使用指南》](../modules/ai/ATC工具使用指南.md)“性能优化推荐设置”章节。
...
-   自定义算子更详细的用法请参考[《ATC自定义算子开发指南》](../modules/ai/ATC自定义算子开发指南.md)手册。
...
-   Graph更详细的用法请参考[《ATC Graph开发指南》](../modules/ai/ATC Graph开发指南.md)手册。
