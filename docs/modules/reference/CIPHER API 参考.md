...
1.  CIPHER设备初始化。调用接口[ss\_mpi\_cipher\_init](ss_mpi_cipher_init.md)。
2.  创建CIPHER句柄。调用接口[ss\_mpi\_cipher\_create](ss_mpi_cipher_create.md)。
3.  创建KEYSLOT句柄。调用接口[ss\_mpi\_keyslot\_create](ss_mpi_keyslot_create.md)。
4.  绑定CIPHER和KEYSLOT句柄。调用接口[ss\_mpi\_cipher\_attach](ss_mpi_cipher_attach.md)。
5.  配置CIPHER控制信息。包含加解密算法、工作模式等信息。调用接口[ss\_mpi\_cipher\_set\_cfg](ss_mpi_cipher_set_cfg.md)。
...
8.  解绑定CIPHER和KEYSLOT句柄。调用接口[ss\_mpi\_cipher\_detach](ss_mpi_cipher_detach.md)。
9.  销毁KEYSLOT句柄。调用接口[ss\_mpi\_keyslot\_destroy](ss_mpi_keyslot_destroy.md)。
10. 销毁CIPHER句柄。调用接口[ss\_mpi\_cipher\_destroy](ss_mpi_cipher_destroy.md)。
11. CIPHER设备去初始化。调用接口[ss\_mpi\_cipher\_deinit](ss_mpi_cipher_deinit.md)。
...
-   该接口支持AES对称加解密算法。
-   各算法支持ECB/CBC/CFB/OFB/CTR/CCM/GCM等工作模式。CCM/GCM工作模式加解密请参考“[CCM/GCM加解密（对称加解密算法）](CCM-GCM加解密（对称加解密算法）.md)”章节。
...
-   CIPHER内部采用DMA方式传输数据，所以调用[ss\_mpi\_cipher\_encrypt](ss_mpi_cipher_encrypt.md)或[ss\_mpi\_cipher\_decrypt](ss_mpi_cipher_decrypt.md)接口进行数据的加密或解密时，传入的地址参数为数据的物理地址。当调用[ss\_mpi\_cipher\_encrypt\_virt](ss_mpi_cipher_encrypt_virt.md)或[ss\_mpi\_cipher\_decrypt\_virt](ss_mpi_cipher_decrypt_virt.md)接口进行数据的加密或解密时，传入的地址参数为数据的虚拟地址。
...
-   对于多个包的操作，每个包都使用[ss\_mpi\_cipher\_set\_cfg](ss_mpi_cipher_set_cfg.md)配置的向量进行运算，IV作用域是可配置的，前一个包的向量运算结果可以作为下一个包的IV，或者每个包IV都是独立运算的（前一次函数调用的结果不会影响后一次函数调用的运算结果）。
-   进行多包加解密时，需要使用物理地址作为地址参数。
-   其它注意事项同“[单包数据加解密（对称加解密算法）](单包数据加解密（对称加解密算法）.md)”章节。
...
8.  获取TAG值。调用接口[ss\_mpi\_cipher\_get\_tag](ss_mpi_cipher_get_tag.md)。
9.  解绑定CIPHER和KEYSLOT句柄。调用接口[ss\_mpi\_cipher\_detach](ss_mpi_cipher_detach.md)。
10. 销毁KEYSLOT句柄。调用接口[ss\_mpi\_keyslot\_destroy](ss_mpi_keyslot_destroy.md)。
11. 销毁CIPHER句柄。调用接口[ss\_mpi\_cipher\_destroy](ss_mpi_cipher_destroy.md)。
12. CIPHER设备去初始化。调用接口[ss\_mpi\_cipher\_deinit](ss_mpi_cipher_deinit.md)。
...
1.  CIPHER设备初始化。调用接口[ss\_mpi\_cipher\_init](ss_mpi_cipher_init.md)完成。
2.  创建一路HASH，获取HASH句柄，选择HASH算法。调用接口[ss\_mpi\_cipher\_hash\_init](ss_mpi_cipher_hash_init.md)完成。
3.  <a name="li579917172559"></a>输入数据，逐个数据块依次计算HASH值。调用接口[ss\_mpi\_cipher\_hash\_update](ss_mpi_cipher_hash_update.md)完成。
4.  如果摘要未计算完成，再次执行[3](#li579917172559)。
5.  完成摘要计算，结束输入，获取计算结果。调用接口[ss\_mpi\_cipher\_hash\_final](ss_mpi_cipher_hash_final.md)完成。
6.  关闭CIPHER设备。调用接口[ss\_mpi\_cipher\_deinit](ss_mpi_cipher_deinit.md)完成。

#### 注意事项<a name="ZH-CN_TOPIC_0000002441572881"></a>

支持软件多通道，可同时进行多个HASH运算， 即执行[2](工作流程-8.md#li16799101725513)启动一个HASH运算，在本次HASH计算未完成（即未执行[5](工作流程-8.md#li379991710553)）之前，可申请一个新通道启动另一个HASH运算，直到申请不到通道为止。
...
1.  调用[ss\_mpi\_cipher\_init](ss_mpi_cipher_init.md)初始化CIPHER模块。
2.  调用[ss\_mpi\_cipher\_hash\_init](ss_mpi_cipher_hash_init.md)选择使用的HASH算法，并配置HMAC计算的密钥，初始化HASH模块。
3.  调用[ss\_mpi\_cipher\_hash\_update](ss_mpi_cipher_hash_update.md)输入数据，可以一个BLOCK接一个BLOCK输入。
4.  调用[ss\_mpi\_cipher\_hash\_final](ss_mpi_cipher_hash_final.md)结束输入，并输出HMAC值。
5.  调用[ss\_mpi\_cipher\_deinit](ss_mpi_cipher_deinit.md)  去初始化CIPHER设备。

#### 注意事项<a name="ZH-CN_TOPIC_0000002441653081"></a>

支持软件多通道，可同时进行多个HMAC运算， 即执行[2](工作流程-11.md#li132961530185714)启动一个HMAC运算，在本次HMAC计算未完成（即未执行[4](工作流程-11.md#li19296113055717)）之前，可申请一个新通道启动另一个HMAC运算，直到申请不到通道为止。
...
1.  CIPHER设备初始化。调用接口[ss\_mpi\_cipher\_init](ss_mpi_cipher_init.md)完成。
2.  获取32bits的随机数。调用接口[ss\_mpi\_cipher\_get\_random\_num](ss_mpi_cipher_get_random_num.md)完成。
3.  关闭CIPHER设备。调用接口[ss\_mpi\_cipher\_deinit](ss_mpi_cipher_deinit.md)完成。
...
-   [ss\_mpi\_cipher\_init](ss_mpi_cipher_init.md)：初始化CIPHER模块。
-   [ss\_mpi\_cipher\_deinit](ss_mpi_cipher_deinit.md)：去初始化CIPHER模块。
-   [ss\_mpi\_cipher\_create](ss_mpi_cipher_create.md)：对称加解密时创建一路的CIPHER句柄。
-   [ss\_mpi\_cipher\_destroy](ss_mpi_cipher_destroy.md)：对称加解密时销毁已存在的CIPHER句柄。
-   [ss\_mpi\_cipher\_set\_cfg](ss_mpi_cipher_set_cfg.md)：对称加解密时配置CIPHER通道对应的控制信息。
-   [ss\_mpi\_cipher\_get\_cfg](ss_mpi_cipher_get_cfg.md)：对称加解密时获取CIPHER通道对应的配置信息。
-   [ss\_mpi\_cipher\_encrypt](ss_mpi_cipher_encrypt.md)：对称加解密时单包数据加密功能。
-   [ss\_mpi\_cipher\_decrypt](ss_mpi_cipher_decrypt.md)：对称加解密时单包数据解密功能。
-   [ss\_mpi\_cipher\_encrypt\_virt](ss_mpi_cipher_encrypt_virt.md)：对称加解密时对数据进行加密。
-   [ss\_mpi\_cipher\_decrypt\_virt](ss_mpi_cipher_decrypt_virt.md)：对称加解密时对数据进行解密。
-   [ss\_mpi\_cipher\_encrypt\_multi\_pack](ss_mpi_cipher_encrypt_multi_pack.md)：对称加解密时多包数据加密功能。
-   [ss\_mpi\_cipher\_decrypt\_multi\_pack](ss_mpi_cipher_decrypt_multi_pack.md)：对称加解密时多包数据解密功能。
-   [ss\_mpi\_cipher\_get\_tag](ss_mpi_cipher_get_tag.md)：对称加解密时CCM/GCM模式获取TAG值。
-   [ss\_mpi\_cipher\_hash\_init](ss_mpi_cipher_hash_init.md)：HASH、HMAC计算初始化功能。
-   [ss\_mpi\_cipher\_hash\_update](ss_mpi_cipher_hash_update.md)：HASH、HMAC计算数据输入功能。
-   [ss\_mpi\_cipher\_hash\_final](ss_mpi_cipher_hash_final.md)：HASH、HMAC计算最终结果输出功能。
-   [ss\_mpi\_cipher\_get\_random\_num](ss_mpi_cipher_get_random_num.md)：获取随机数功能。
-   [ss\_mpi\_cipher\_rsa\_public\_encrypt](ss_mpi_cipher_rsa_public_encrypt.md)：使用RSA公钥加密一段明文。
-   [ss\_mpi\_cipher\_rsa\_private\_decrypt](ss_mpi_cipher_rsa_private_decrypt.md)：使用RSA私钥解密一段密文。
-   [ss\_mpi\_cipher\_rsa\_private\_encrypt](ss_mpi_cipher_rsa_private_encrypt.md)：使用RSA私钥加密一段明文。
-   [ss\_mpi\_cipher\_rsa\_public\_decrypt](ss_mpi_cipher_rsa_public_decrypt.md)：使用RSA公钥解密一段密文。
-   [ss\_mpi\_cipher\_rsa\_sign](ss_mpi_cipher_rsa_sign.md)：使用RSA私钥签名一段文本。
-   [ss\_mpi\_cipher\_rsa\_verify](ss_mpi_cipher_rsa_verify.md)：使用RSA公钥验证一段文本。
-   [ss\_mpi\_cipher\_sm2\_encrypt](ss_mpi_cipher_sm2_encrypt.md)：使用SM2公钥加密一段明文。
-   [ss\_mpi\_cipher\_sm2\_decrypt](ss_mpi_cipher_sm2_decrypt.md)：使用SM2私钥解密一段密文。
-   [ss\_mpi\_cipher\_sm2\_sign](ss_mpi_cipher_sm2_sign.md)：使用SM2签名一段文本。
-   [ss\_mpi\_cipher\_sm2\_verify](ss_mpi_cipher_sm2_verify.md)：使用SM2验证一段文本。
-   [ss\_mpi\_keyslot\_create](ss_mpi_keyslot_create.md)：对称加解密时创建一路keyslot句柄。
-   [ss\_mpi\_keyslot\_destroy](ss_mpi_keyslot_destroy.md)：对称加解密时销毁一路keyslot句柄。
-   [ss\_mpi\_cipher\_attach](ss_mpi_cipher_attach.md)：对称加解密时cipher句柄与keyslot句柄绑定。
-   [ss\_mpi\_cipher\_detach](ss_mpi_cipher_detach.md)：对称加解密时cipher句柄与keyslot句柄解绑定。
...
<td class="cellrowborder" valign="top" width="50%" headers="mcps1.1.3.1.2 "><p id="p555mcpsimp"><a name="p555mcpsimp"></a><a name="p555mcpsimp"></a>参见<a href="错误码.md">错误码</a>。</p>
</td>
</tr>
</tbody>
</table>
...
对称加解密时配置CIPHER通道对应的信息。详细配置请参见结构体[ot\_cipher\_ctrl](ot_cipher_ctrl.md)。
...
-   CIPHER句柄必须已创建。
-   可多次调用，需配合  [ss\_mpi\_cipher\_set\_cfg](ss_mpi_cipher_set_cfg.md)  使用，使用方法可参考  [注意事项](注意事项.md)。
...
-   CIPHER句柄必须已创建。
-   可多次调用，需配合  [ss\_mpi\_cipher\_set\_cfg](ss_mpi_cipher_set_cfg.md)使用，使用方法可参考 “[注意事项](注意事项.md)”。
-   对于多个包的操作，每个包都使用[ss\_mpi\_cipher\_set\_cfg](ss_mpi_cipher_set_cfg.md)配置的向量进行运算，前一个包的向量运算结果不会作用于下一个包的运算，每个包都是独立运算的。前一次函数调用的结果也不会影响后一次函数调用的运算结果。
-   加密的源地址、目的地址可以相同。
...
-   CIPHER句柄必须已创建。
-   tag不能为空。
-   输入tag\_len小于[ot\_cipher\_ctrl\_aes\_ccm\_gcm](ot_cipher_ctrl_aes_ccm_gcm.md)结构体成员tag\_len指定的值时，接口异常。
-   只有在CCM、GCM模式下此接口才有效。
...
-   [ot\_cipher\_work\_mode](ot_cipher_work_mode.md)：定义CIPHER工作模式。
-   [ot\_cipher\_alg](ot_cipher_alg.md)：定义CIPHER加密算法。
-   [ot\_cipher\_key\_len](ot_cipher_key_len.md)：定义CIPHER密钥长度。
-   [ot\_cipher\_bit\_width](ot_cipher_bit_width.md)：定义CIPHER加密位宽。
-   [ot\_cipher\_ctrl\_chg\_flag](ot_cipher_ctrl_chg_flag.md)：定义CIPHER 向量变更标志。
-   [ot\_cipher\_type](ot_cipher_type.md)：定义CIPHER加解密类型选择。
-   [ot\_cipher\_attr](ot_cipher_attr.md)：定义CIPHER加解密属性结构体。
-   [ot\_cipher\_ctrl\_aes](ot_cipher_ctrl_aes.md)：AES加密控制信息结构体。
-   [ot\_cipher\_ctrl\_aes\_ccm\_gcm](ot_cipher_ctrl_aes_ccm_gcm.md)：AES-CCM、AES-GCM 加密控制信息结构体。
-   [ot\_cipher\_ctrl\_sm4](ot_cipher_ctrl_sm4.md)：定义SM4加密控制信息结构体。
-   [ot\_cipher\_ctrl](ot_cipher_ctrl.md)：定义加密控制信息结构体。
-   [ot\_cipher\_data](ot_cipher_data.md)：定义CIPHER加解密数据。
-   [ot\_cipher\_hash\_type](ot_cipher_hash_type.md)：定义CIPHER哈希算法类型。
-   [ot\_cipher\_hash\_attr](ot_cipher_hash_attr.md)：定义CIPHER哈希算法初始化输入结构体。
-   [ot\_cipher\_common\_data](ot_cipher_common_data.md)：定义通用输入输出数据格式。
-   [ot\_cipher\_rsa\_scheme](ot_cipher_rsa_scheme.md)：定义RSA算法填充。
-   [ot\_cipher\_rsa\_public\_key](ot_cipher_rsa_public_key.md)：定义RSA公钥结构体。
-   [ot\_cipher\_rsa\_private\_key](ot_cipher_rsa_private_key.md)：定义RSA私钥结构体。
-   [ot\_cipher\_sign\_type](ot_cipher_sign_type.md)：定义非对称算法签名验签输入数据类型。
-   [ot\_cipher\_sign\_in\_data](ot_cipher_sign_in_data.md)：定义非对称算法签名验签输入数据结构体。
-   [ot\_cipher\_sm2\_public\_key](ot_cipher_sm2_public_key.md)：定义SM2公钥结构体。
-   [ot\_cipher\_sm2\_private\_key](ot_cipher_sm2_private_key.md)：定义SM2私钥结构体。
-   [ot\_cipher\_sm2\_sign](ot_cipher_sm2_sign.md)：定义SM2签名结构体。
-   [ot\_cipher\_sm2\_verify](ot_cipher_sm2_verify.md)：定义SM2验签结构体。
-   [ot\_cipher\_sm2\_sign\_data](ot_cipher_sm2_sign_data.md)：定义SM2签名验签数据结构体。
-   [ot\_keyslot\_type](ot_keyslot_type.md)：keyslot类型枚举值。
-   [ot\_keyslot\_secure\_mode](ot_keyslot_secure_mode.md)：keyslot安全模式枚举值。
-   [ot\_keyslot\_attr](ot_keyslot_attr.md)：keyslot属性结构体。
-   [OT\_CIPHER\_MAX\_IV\_SIZE\_IN\_WORD](OT_CIPHER_MAX_IV_SIZE_IN_WORD.md)：CIPHER 的IV最大长度。
-   [OT\_CIPHER\_SM2\_LEN\_IN\_WORD](OT_CIPHER_SM2_LEN_IN_WORD.md)：SM2密钥、签名验签数据长度。
...
-   当算法 alg 为 OT\_CIPHER\_ALG\_AES 且工作模式 work\_mode 为 OT\_CIPHER\_WORK\_MODE\_ECB、OT\_CIPHER\_WORK\_MODE\_CBC、OT\_CIPHER\_WORK\_MODE\_CFB、OT\_CIPHER\_WORK\_MODE\_OFB、OT\_CIPHER\_WORK\_MODE\_CTR中的一种时，联合体指向的结构体为  [ot\_cipher\_ctrl\_aes](ot_cipher_ctrl_aes.md)。
-   当算法 alg 为 OT\_CIPHER\_ALG\_AES 且工作模式 work\_mode 为 OT\_CIPHER\_WORK\_MODE\_CCM、OT\_CIPHER\_WORK\_MODE\_GCM中的一种时，联合体指向的结构体为  [ot\_cipher\_ctrl\_aes\_ccm\_gcm](ot_cipher_ctrl_aes_ccm_gcm.md)。
-   当算法 alg 为 OT\_CIPHER\_ALG\_SM4 且工作模式 work\_mode 为 OT\_CIPHER\_WORK\_MODE\_ECB、OT\_CIPHER\_WORK\_MODE\_CBC、OT\_CIPHER\_WORK\_MODE\_CFB、OT\_CIPHER\_WORK\_MODE\_OFB、OT\_CIPHER\_WORK\_MODE\_CTR中的一种时，联合体指向的结构体为  [ot\_cipher\_ctrl\_sm4](ot_cipher_ctrl_sm4.md)。
