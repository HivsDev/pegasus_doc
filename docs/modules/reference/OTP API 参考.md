...
-   [ss\_mpi\_otp\_init]：初始化OTP模块。
-   [ss\_mpi\_otp\_deinit]：去初始化OTP模块。
-   [ss\_mpi\_otp\_set\_user\_data]：设置OTP用户空间数据。
-   [ss\_mpi\_otp\_get\_user\_data]：读取OTP用户空间数据。
-   [ss\_mpi\_otp\_set\_user\_data\_lock]：设置OTP用户数据锁
-   [ss\_mpi\_otp\_get\_user\_data\_lock]：获取OTP用户数据锁
-   [ss\_mpi\_otp\_burn\_product\_pv]：烧写PV的数据和锁标志到芯片内部OTP。
-   [ss\_mpi\_otp\_read\_product\_pv]：获取芯片内部OTP的PV数据或锁标志。
-   [ss\_mpi\_otp\_get\_key\_verify\_status]：获取芯片内部OTP中存储KEY的校验状态。
...
-   [ot\_otp\_burn\_pv\_item]：OTP PV数据类型。
-   [ot\_otp\_lock\_status]：定义OTP数据锁状态。
-   [OT\_OTP\_PV\_NAME\_MAX\_LEN]：字段名称最大字节长度（单位：byte）。
-   [OT\_OTP\_PV\_VALUE\_MAX\_LEN]：ot\_otp\_burn\_pv\_item成员value最大字节长度（单位：byte）。
