...
1.  利用现有数据进行Auto AWB标定，标定步骤参考 "[自动AWB标定步骤（AWB）](自动AWB标定步骤（AWB）.md)"。因为仅采集到6000K的室外数据，因此，指定A、10K\(也可以是D75\)、6000K室外数据为KI光源进行标定，得到的标定结果如图 Auto AWB 标定所示。
...
<td class="cellrowborder" rowspan="4" valign="top" width="47%" headers="mcps1.2.4.1.3 "><p id="p4516134117920"><a name="p4516134117920"></a><a name="p4516134117920"></a>请参考“<a href="借助标定工具-判断灰点色差条件设置是否合理.md">借助标定工具，判断灰点色差条件设置是否合理</a>”进行标定。</p>
