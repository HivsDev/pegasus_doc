...
-   提高清晰度，请参见[如何解决整体锐度不足]和[如何解决低照度清晰度差]。
...
针对该问题，首先检查Global CAC标定的静态系数是否合理，如果不合理进行重新标定；如果不想标定Global CAC的静态系数，默认需要将Global CAC的使能关闭。[图2]所示为Global CAC静态系数不合理，开关Global CAC对比图。
