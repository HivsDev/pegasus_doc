...
<tbody><tr id="row1372280416410"><td class="cellrowborder" valign="top" width="20.580000000000002%" headers="mcps1.1.3.1.1 "><p id="p3734547016410"><a name="p3734547016410"></a><a name="p3734547016410"></a><a name="image2670064316410"></a><a name="image2670064316410"></a><span><img src="figures/zh-cn_image_0000002408582854.png" alt="" /></span></p>
</td>
<td class="cellrowborder" valign="top" width="79.42%" headers="mcps1.1.3.1.2 "><p id="p1757432116410"><a name="p1757432116410"></a><a name="p1757432116410"></a>表示如不避免则将会导致死亡或严重伤害的具有高等级风险的危害。</p>
</td>
</tr>
<tr id="row466863216410"><td class="cellrowborder" valign="top" width="20.580000000000002%" headers="mcps1.1.3.1.1 "><p id="p1432579516410"><a name="p1432579516410"></a><a name="p1432579516410"></a><a name="image4895582316410"></a><a name="image4895582316410"></a><span><img src="figures/zh-cn_image_0000002441982213.png" alt="" /></span></p>
</td>
<td class="cellrowborder" valign="top" width="79.42%" headers="mcps1.1.3.1.2 "><p id="p959197916410"><a name="p959197916410"></a><a name="p959197916410"></a>表示如不避免则可能导致死亡或严重伤害的具有中等级风险的危害。</p>
</td>
</tr>
<tr id="row123863216410"><td class="cellrowborder" valign="top" width="20.580000000000002%" headers="mcps1.1.3.1.1 "><p id="p1232579516410"><a name="p1232579516410"></a><a name="p1232579516410"></a><a name="image1235582316410"></a><a name="image1235582316410"></a><span><img src="figures/zh-cn_image_0000002408422942.png" alt="" /></span></p>
</td>
<td class="cellrowborder" valign="top" width="79.42%" headers="mcps1.1.3.1.2 "><p id="p123197916410"><a name="p123197916410"></a><a name="p123197916410"></a>表示如不避免则可能导致轻微或中度伤害的具有低等级风险的危害。</p>
</td>
</tr>
<tr id="row5786682116410"><td class="cellrowborder" valign="top" width="20.580000000000002%" headers="mcps1.1.3.1.1 "><p id="p2204984716410"><a name="p2204984716410"></a><a name="p2204984716410"></a><a name="image4504446716410"></a><a name="image4504446716410"></a><span><img src="figures/zh-cn_image_0000002408582858.png" alt="" /></span></p>
</td>
<td class="cellrowborder" valign="top" width="79.42%" headers="mcps1.1.3.1.2 "><p id="p4388861916410"><a name="p4388861916410"></a><a name="p4388861916410"></a>用于传递设备或环境安全警示信息。如不避免则可能会导致设备损坏、数据丢失、设备性能降低或其它不可预知的结果。</p>
<p id="p1238861916410"><a name="p1238861916410"></a><a name="p1238861916410"></a>“须知”不涉及人身伤害。</p>
</td>
</tr>
<tr id="row2856923116410"><td class="cellrowborder" valign="top" width="20.580000000000002%" headers="mcps1.1.3.1.1 "><p id="p5555360116410"><a name="p5555360116410"></a><a name="p5555360116410"></a><a name="image799324016410"></a><a name="image799324016410"></a><span><img src="figures/zh-cn_image_0000002442022069.png" alt="" /></span></p>
</td>
...
<img src="figures/ATC工具功能架构.png" alt="" />
...
-   使用ATC工具之前，请先在开发环境安装ATC，获取相关路径下的ATC工具，详细说明请参见[获取ATC工具](获取ATC工具.md)中的环境准备。
-   准备要进行转换的模型，并上传到开发环境，详细说明请参见[转换样例](转换样例.md)。
-   使用ATC工具进行模型转换，在配置相关参数时，根据实际情况选择是否进行[量化选项](量化选项.md)。图像预处理是图像分析引擎提供的硬件图像预处理模块，包括色域转换，图像归一化（减均值/乘系数）功能。
...
2.  执行如下命令生成离线模型。（如下命令中使用的目录以及文件均为样例，请以实际为准）
    ...
    关于参数的详细解释以及使用方法请参见[参数说明](参数说明.md)。
...
3.  若提示如下信息，则说明模型转换成功。
    ...
    > **说明：** 
    >如果用户使用Faster RCNN、YOLOv3、YOLOv2、SSD等Caffe框架网络模型进行模型转换，由于此类网络中包含了一些原始Caffe框架中没有定义的算子结构，如ROIPooling、Normalize、PSROI Pooling和Upsample等。为了使图像分析引擎能支持这些网络，需要对原始的Caffe框架网络模型进行扩展，降低开发者开发自定义算子/开发后处理代码的工作量，详细扩展方法请参见[检测网硬化加速prototxt示例](检测网硬化加速prototxt示例.md)。
...
-   \*.om: 转换后的模型文件，文件名通过[--output](--output.md)配置。
...
-   layer.json：输出原始模型的算子信息，见命令行参数[--json](--json.md)。
-   shape.json：输出原始模型每个算子的shape信息，见命令行参数[--json](--json.md)。
-   om.json：输出转换后的模型信息，见命令行参数[--json](--json.md)。
    -   模型信息包括但不限于：
        ...
        -   batch\_num：支持的动态batch数，见命令行参数[--batch\_num](--batch_num.md)。
...
-   如果要将Faster RCNN等网络模型转成适配图像分析引擎的离线模型，则务必参见[定制网络修改（Caffe）](定制网络修改（Caffe）.md)先修改prototxt模型文件。
...
-   只支持[算子规格说明](算子规格说明.md)中的算子，并需满足算子限制条件。不支持的算子需用户在板端使用CPU实现，把网络切分编译多个om，然后调用板端ACL API串通网络。
...
关联参数：若--mode取值为1或3，则需要与[ --om](--om.md)、[--json](--json.md)参数配合使用。如果将原始模型文件转换成带shape信息的json文件，则还需要与[ --dump\_mode](--dump_mode.md)参数配合使用。
...
关联参数：当原始模型为Caffe框架时，需要和[--weight](--weight.md)参数配合使用。
...
关联参数：当原始模型为Caffe框架时，需要和[--model](--model.md)参数配合使用。
...
关联参数：若[--mode](--mode.md)=1，则该参数必填，并且需要与[ --json](--json.md)参数配合使用。
...
-   参数值约束：当[ --mode](--mode.md)为1时，该参数可选，可以指定Caffe、Onnx、原始模型转成json，不指定时默认为离线模型转json，如果指定时需要保证--om模型和--framework类型对应一致，例如：
...
-   原始模型文件转换为json
    ...
    该参数需要与[--mode](--mode.md)=1、[--om](--om.md)参数、[ --framework](--framework.md)配合使用，caffe还需配合[--weight](--weight.md)使用。
...
[--gfpq\_param\_file](--gfpq_param_file.md)：优先使用配置的量化参数。未配置量化参数时, --matmul\_per\_channel\_enable才生效。

[--weight\_quant\_per\_channel](--weight_quant_per_channel.md)：当该配置为0时，--matmul\_per\_channel\_enable不生效
...
-   文件内容的格式：
    -   Faster RCNN和RFCN锚点坐标文件每行内容为“xmin, ymin, xmax, ymax”如下。
        ...
        <img src="figures/zh-cn_image_0000002442022093.png" alt="" />
...
[--layer\_fusion\_enable](--layer_fusion_enable.md)：深度融合的层不支持dump，即\*.om转json文件里，属性is\_dump\_available为0的层不支持dump。
...
[--input\_shape](--input_shape.md)：当某个输入采用动态分辨率时，需要配置该选项，且将宽、高配置成-1。
...
关联参数：当开启该功能时，[--batch\_num](--batch_num.md)将强制为1
...
关联参数：该参数需要与[ --output](--output.md)参数配合使用。
...
关联参数：该参数需要与[--json](--json.md)、[--mode](--mode.md)=1、[--framework](--framework.md)、[--om](--om.md)参数（需要为原始模型文件，如果为Caffe框架模型文件，还需要增加[--weight](--weight.md)参数）配合使用。
...
以下参数仅支持以cfg文件方式配置，不支持命令行方式配置，用于兼容使用NNIE的cfg配置文件转模型。cfg文件格式见[参数配置方式](参数配置方式.md)。
...
针对Faster RCNN、RFCN、SSD、YOLOV1、YOLOV2、YOLOV3等检测网络，支持对Proposal、SsdDetectionOutput、YoloDetectionOutput等层进行硬化，达到加速目的。本节描述如何把Proposal、SsdDetectionOutput、YoloDetectionOutput改为RPN硬化的[DetectionOutput](DetectionOutput.md)层。
...
<img src="figures/Faster-RCNN-Proposal层硬化结构图.png" alt="" />
...
<img src="figures/RFCN-Proposal层硬化结构图.png" alt="" />
...
<img src="figures/SSD-DetectionOutput层硬化结构图.png" alt="" />
...
<img src="figures/YOLOV1-DetectionOutput层硬化结构图.png" alt="" />
...
<img src="figures/YOLOV2-DetectionOutput层硬化结构图.png" alt="" />
...
<img src="figures/YOLOV3-V5-V7-Decbbox-DetectionOutput层硬化结构图.png" alt="" />
...
<img src="figures/YOLOV8-DecBBox-Sort-Nms-Filter层硬化结构图.jpg" alt="" />
...
<img src="figures/两种输入格式的prototxt写法.png" alt="" />
...
<img src="figures/两种输入格式的量化数据排布.png" alt="" />
...
<img src="figures/首层Roi-Psroi-Pooling网络结构图.png" alt="" />
...
<img src="figures/Recurrent网络片段结构图.jpg" alt="" />
...
仍以[图1](板端的输入格式-38.md#fig327845143214)为例，LSTM层第一个输出shape为\[18, 1, 128\]，板端在该层得到的输出符合shape为\[batch\_num, 18, 1, 128\]对应的数据排布, 即依次排列每张图片对应的计算数据。在标准Caffe下，若Prototxt首层N轴为batch\_num，LSTM层第一个输出符合shape为\[18, batch\_num, 128\]对应的数据排布，即不同图片对应的数据间存在交叉排列。
