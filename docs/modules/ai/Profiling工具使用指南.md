...
<td class="cellrowborder" valign="top" width="79.27%" headers="mcps1.2.3.1.2 "><p id="p169201344616"><a name="p169201344616"></a><a name="p169201344616"></a>使能Profiling需要先进行环境搭建以进行Profiling数据采集和解析。详情请参见<a href="环境准备.md">环境准备</a>。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="79.27%" headers="mcps1.2.3.1.2 "><p id="p18139637163211"><a name="p18139637163211"></a><a name="p18139637163211"></a>通过脚本工具msprof.pyc进行Profiling数据的解析并导出相应数据。详情请参见<a href="解析Profiling数据.md">解析Profiling数据</a>。</p>
</td>
</tr>
</tbody>
</table>
...
在您使用Profiling功能前，需要根据[场景介绍](场景介绍.md)完成相关环境搭建。具体如下。
...
1.  参考以下步骤完成acl.json文件配置，并完成应用工程编译和运行：
    ...
    -   打开工程文件，查看调用的**svp\_acl\_init\(\)**函数，获取acl.json文件路径，具体可以参考[2](通过调用acl-json文件方式采集Profiling数据.md#li66486291273)
    -   修改svp\_acl\_init方法指定的acl.json文件内容，添加Profiling相关配置，格式如下所示。

        具体参数配置可以参考[3](通过调用acl-json文件方式采集Profiling数据.md#li1333417325516)
...
3.  执行如下命令，进行上板操作：
    ...
    msprof.pyc工具详细介绍请参见[msprof.pyc脚本工具介绍](msprof-pyc脚本工具介绍.md)。

    命令行参数详细介绍请参见[采集Profiling数据](采集Profiling数据.md)。
...
在解析任意目录的Profiling数据前，需参见[采集Profiling数据](采集Profiling数据.md)采集相应的数据。
...
在导出timeline数据前，需参见[解析Profiling数据](解析Profiling数据-0.md)。参见如下步骤导出timeline数据。
...
4.  执行完上述命令后，会在collection-dir目录下生成timeline目录，不同的数据生成对应的json文件，具体内容参见[表2](#zh-cn_topic_0290106133_table972265435020)。
    ...
    **表 2**  timeline文件介绍
    ...
    <td class="cellrowborder" valign="top" width="40.150000000000006%" headers="mcps1.2.4.1.3 "><p id="p379861812562"><a name="p379861812562"></a><a name="p379861812562"></a>Task Scheduler任务调度信息。文件详情请参见<a href="Task-Scheduler任务调度信息数据说明.md">Task Scheduler任务调度信息数据说明</a>。</p>
    </td>
    </tr>
    ...
    <td class="cellrowborder" valign="top" width="40.150000000000006%" headers="mcps1.2.4.1.3 "><p id="p1472233693119"><a name="p1472233693119"></a><a name="p1472233693119"></a>ACL接口耗时数据，生成该文件需要采集的Profiling数据中包含AclModule.开头的文件。文件详情请参见<a href="ACL接口耗时数据说明.md">ACL接口耗时数据说明</a>。</p>
    </td>
    </tr>
    </tbody>
    </table>
...
请参见[导出timeline数据](导出timeline数据.md)获取Task Scheduler任务调度信息数据文件
...
请参见[导出timeline数据](导出timeline数据.md)获取ACL接口耗时数据文件acl\_\{deviceid\}.\{model\_file\_name\}.\{model\_id\}.\{batch\_num\}.json，其中\{device\_id\}表示设备ID，\{model\_file\_name\}表示模型名称，\{model\_id\}表示模型ID，\{batch\_num\}表示batch的数量。
...
在导出summary数据前，需要参见[解析Profiling数据](解析Profiling数据-0.md)解析Profiling数据。参见如下步骤导出summary数据。
...
请参见[导出summary数据](导出summary数据.md)获取ACL接口调用次数及耗时数据acl\_statistic\_\{device\_id\}\_\{model\_id\}\_\{iter\_id\}.csv，其中\{device\_id\}表示设备ID，\{model\_id\}表示模型ID，\{iter\_id\}表示某轮迭代的ID号。
...
请参见[导出summary数据](导出summary数据.md)获取_AA_  Core算子调用次数及耗时数据op\_statistic\_\{device\_id\}\_\{model\_id\}\_\{current\_pic\_count\}\_\{iter\_id\}.csv，其中\{device\_id\}表示设备ID，\{model\_id\}表示模型ID，\{current\_pic\_count\}表示当前图片数，\{iter\_id\}表示某轮迭代的ID号。
...
1.  采集Profiling数据。
    ...
    参考[采集Profiling数据](采集Profiling数据.md)，得到JOB开头的即为保存的原始Profiling数据。
...
2.  解析Profiling数据。
    ...
    3.  执行如下命令，解析JOBXXX目录下的Profiling数据。
        ...
        此处以通过import命令行方式解析Profiling数据为例，解析Profiling数据详细介绍请参见[解析Profiling数据](解析Profiling数据.md)。
...
3.  在msprof.pyc脚本所在目录继续执行如下命令，导出timeline数据。
    ...
    执行完上述命令后，会在collection-dir目录下的JOBXXX目录下生成timeline目录，不同的数据生成对应的json文件，如[图1](#fig237417213614)所示，具体内容参见[表2](导出timeline数据.md#zh-cn_topic_0290106133_table972265435020)。
...
4.  在msprof.pyc脚本所在目录继续执行如下命令，导出summary数据。
    ...
    执行完上述命令后，会在collection-dir目录下生成summary目录，不同的数据（推理，系统）生成对应的csv文件，如[图2](#fig12893536173613)所示，具体内容参见[表2](导出summary数据.md#zh-cn_topic_0290119915_table2434544115813)。
