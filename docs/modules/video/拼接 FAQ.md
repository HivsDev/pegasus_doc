...
这8种格子的张贴规律如[图1](#fig10683514734)所示，请注意不同类型的彩色棋盘格位置不需要完全与图样相同，其基本思想是：不同类型的格子均匀张贴，使同一种类型格子之间的间距尽量大，以减小特征点匹配错位的情况。[图1](#fig10683514734)\(a\)中相同类型的格子在横竖方向上都间隔4个格子，斜方向间隔2个格子。若原本的黑白棋盘格分布不均匀（如[图1](如何理解理想的产线标定环境.md#fig6703175413384)中的北极/南极附近区域）则尽量满足规律即可。
...
4.  采用彩色棋盘格标定方案。
    ...
    彩色棋盘格将结构差异要求放宽到1个格子。具体请参考“[如何制作彩色棋盘格产线标定环境](如何制作彩色棋盘格产线标定环境.md)”章节，同时调用产线标定接口时配置彩色棋盘格模式。
...
1.  优化模型标定精度。
    ...
    模型标定精度评估可参考“[如何利用PQ Tools中tmp文件评估模型标定效果](如何利用PQ-Tools中tmp文件评估模型标定效果.md)”章节进行评估，或者基于模型标定的机器，直接查看不同距离下的拼接效果，若模型标定本身的拼接效果较差，作为产线标定的种子使用时，则会影响产线标定最终的标定效果。当模型标定效果不满意时，则参考“[模型标定](模型标定.md)”指导进行改正优化。或者挑选多个机器进行模型标定，从中选择最佳的结果作为产线标定的种子。
...
请参考"[QA\_measures.txt文件内容与使用说明](QA_measures-txt文件内容与使用说明.md)"小节至"[棋盘格标志的jpg图片](棋盘格标志的jpg图片.md)"小节描述。
...
上式中：W为像素宽度，单位为pixel；w为sensor宽度，单位为mm；f为镜头焦距，单位为mm；b为相邻镜头间距，单位为mm；Z<sub>1</sub>/Z<sub>2</sub>为不同物体的成像距离，单位为mm。其中W、w，f都为定值，视角差与b、<img src="figures/zh-cn_formulaimage_0000002464864685.png" alt="" />成正比，所以为了减小视角差造成的错位影响，应该尽量减少相邻镜头的间距。而Z<sub>1</sub>、Z<sub>2</sub>与场景相关，是无法直接进行调控的，[表1](#table1228mcpsimp)是根据一定条件得到的不同物距的视差大小，可以看出当物距越近时，视差影响越大，所以全景相机更适用于室外大而远的场景，室内场景下因为视差影响更明显，更容易出现错位重影问题。
