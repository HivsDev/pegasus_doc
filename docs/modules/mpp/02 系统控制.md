...
>-   Hi3403V100/SS626V100支持低延时，支持的模块分别见[表1](#_Ref71645661)和[表2](#_Toc74057716)，未在列表中的模块不支持低延时。

**表 1**  Hi3403V100 支持低延时的模块列表
...
<img src="figures/典型的公共视频缓存池数据流图.png" alt="" title="典型的公共视频缓存池数据流图" />
...
>Hi3403V100分配MCF场景所需buffer时，如果mono路不是亮度单分量图像，则需使用ot\_vpss\_get\_mcf\_color\_buf\_size计算buffer大小。
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.2.3.1.2 "><p id="p4410mcpsimp"><a name="p4410mcpsimp"></a><a name="p4410mcpsimp"></a>HNR使用的帧数据缓存池，仅Hi3403V100支持</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.2.3.1.2 "><p id="p1059655054019"><a name="p1059655054019"></a><a name="p1059655054019"></a>raw数据根据压缩率获取缓存池配置，仅Hi3403V100支持</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.2.3.1.2 "><p id="p4435mcpsimp"><a name="p4435mcpsimp"></a><a name="p4435mcpsimp"></a>VENC参考帧大小，Hi3403V100/SS626V100不支持</p>
</td>
</tr>
...
<p id="p1492375163114"><a name="p1492375163114"></a><a name="p1492375163114"></a>Hi3403V100/SS626V100不支持</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.2.3.1.2 "><p id="p4475mcpsimp"><a name="p4475mcpsimp"></a><a name="p4475mcpsimp"></a>AVS输入的YUV数据缓存池，仅Hi3403V100支持</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.2.3.1.2 "><p id="p4480mcpsimp"><a name="p4480mcpsimp"></a><a name="p4480mcpsimp"></a>MCF场景彩色通路VPSS通道VB大小，仅Hi3403V100支持</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.2.3.1.2 "><p id="p4485mcpsimp"><a name="p4485mcpsimp"></a><a name="p4485mcpsimp"></a>MCF场景黑白通路VPSS通道VB大小，仅Hi3403V100支持</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.2.3.1.2 "><p id="p4490mcpsimp"><a name="p4490mcpsimp"></a><a name="p4490mcpsimp"></a>VI输出的YUV数据缓存池大小，仅Hi3403V100支持</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61%" headers="mcps1.2.3.1.2 "><p id="p4495mcpsimp"><a name="p4495mcpsimp"></a><a name="p4495mcpsimp"></a>VPSS输出的YUV数据缓存池大小，仅Hi3403V100支持</p>
</td>
</tr>
</tbody>
</table>
...
MPP提供系统绑定接口（[ss\_mpi\_sys\_bind](ss_mpi_sys_bind.md)），即通过数据接收者绑定数据源来建立两者之间的关联关系（只允许数据接收者绑定数据源）。绑定后，数据源生成的数据将自动发送给接收者。目前MPP支持的绑定关系如[表1](#_Ref289523395)所示。
...
> **须知：** 
>强制销毁VB功能是为了应用程序异常崩溃之后方便业务重启而设计的：用户态应用程序异常崩溃之后，应用程序已经无法按正常的业务退出流程操作，此时重启业务前须先进行[ss\_mpi\_sys\_exit](ss_mpi_sys_exit.md)和[ss\_mpi\_vb\_exit](ss_mpi_vb_exit.md)把上次应用程序异常崩溃之后无法销毁的VB资源先销毁，然后再进行正常的业务重启流程。
...
> **说明：** 
>对于Hi3403V100系列，当PIPE0为在线时，其他PIPE的离线模式只能用来获取PIPE FRAME，其他功能都不能使用，即不能使用VI\_PROC的功能。

### VI视频模式<a name="ZH-CN_TOPIC_0000002441699173"></a>

VI的视频模式分为两种：Normal模式和Advanced模式，默认为Normal模式。在单线性通路上，使用Advanced模式效果好，其它场景下使用Normal模式即可。该模式对VI所有的pipe生效。详见[ss\_mpi\_sys\_set\_vi\_video\_mode](ss_mpi_sys_set_vi_video_mode.md)接口描述。
...
-   [ss\_mpi\_sys\_set\_cfg](ss_mpi_sys_set_cfg.md)：配置系统控制参数。
-   [ss\_mpi\_sys\_get\_cfg](ss_mpi_sys_get_cfg.md)：获取系统控制参数。
-   [ss\_mpi\_sys\_init](ss_mpi_sys_init.md)：初始化MPP系统。
-   [ss\_mpi\_sys\_exit](ss_mpi_sys_exit.md)：去初始化MPP系统。
-   [ss\_mpi\_sys\_bind](ss_mpi_sys_bind.md)：数据源到数据接收者绑定。
-   [ss\_mpi\_sys\_unbind](ss_mpi_sys_unbind.md)：数据源到数据接收者解绑定。
-   [ss\_mpi\_sys\_get\_bind\_by\_dst](ss_mpi_sys_get_bind_by_dst.md)：根据目标获取绑定的源。
-   [ss\_mpi\_sys\_get\_bind\_by\_src](ss_mpi_sys_get_bind_by_src.md)：根据源获取绑定的目标。
-   [ss\_mpi\_sys\_get\_version](ss_mpi_sys_get_version.md)：获取MPP的版本号。
-   [ss\_mpi\_sys\_get\_chip\_id](ss_mpi_sys_get_chip_id.md)：获取当前芯片的ID。
-   [ss\_mpi\_sys\_get\_unique\_id](ss_mpi_sys_get_unique_id.md)：获取当前芯片的unique ID。
-   [ss\_mpi\_sys\_get\_custom\_code](ss_mpi_sys_get_custom_code.md)：获取当前芯片的custom code。
-   [ss\_mpi\_sys\_get\_cur\_pts](ss_mpi_sys_get_cur_pts.md)：获取当前时间戳。
-   [ss\_mpi\_sys\_init\_pts\_base](ss_mpi_sys_init_pts_base.md)：初始化MPP时间戳。
-   [ss\_mpi\_sys\_sync\_pts](ss_mpi_sys_sync_pts.md)：同步MPP时间戳。
-   [ss\_mpi\_sys\_mmap](ss_mpi_sys_mmap.md)：memory存储映射接口。
-   [ss\_mpi\_sys\_mmap\_cached](ss_mpi_sys_mmap_cached.md)：存储带Cache映射接口。
-   [ss\_mpi\_sys\_munmap](ss_mpi_sys_munmap.md)：存储反映射接口。
-   [ss\_mpi\_sys\_flush\_cache](ss_mpi_sys_flush_cache.md)：刷新cache里的内容到内存并且使cache里的内容无效。
-   [ss\_mpi\_sys\_mmz\_alloc](ss_mpi_sys_mmz_alloc.md)：在用户态分配MMZ内存。
-   [ss\_mpi\_sys\_mmz\_alloc\_cached](ss_mpi_sys_mmz_alloc_cached.md)：在用户态分配MMZ内存，该内存支持cache缓存。
-   [ss\_mpi\_sys\_mmz\_free](ss_mpi_sys_mmz_free.md)：在用户态释放MMZ内存。
-   [ss\_mpi\_sys\_set\_mem\_cfg](ss_mpi_sys_set_mem_cfg.md)：设置模块设备通道使用内存的DDR名。
-   [ss\_mpi\_sys\_get\_mem\_cfg](ss_mpi_sys_get_mem_cfg.md)：获取模块设备通道使用的MMZ区域名称。
-   [ss\_mpi\_sys\_close\_fd](ss_mpi_sys_close_fd.md)：关闭所有SYS打开的日志、系统Fd。
-   [ss\_mpi\_sys\_get\_virt\_mem\_info](ss_mpi_sys_get_virt_mem_info.md)：根据虚拟地址获取对应的内存信息。
-   [ss\_mpi\_sys\_set\_scale\_coef\_level](ss_mpi_sys_set_scale_coef_level.md)：设置VPSS模块和VGS模块的缩放系数等级。
-   [ss\_mpi\_sys\_get\_scale\_coef\_level](ss_mpi_sys_get_scale_coef_level.md)：获取VPSS和VGS缩放系数等级。
-   [ss\_mpi\_sys\_set\_time\_zone](ss_mpi_sys_set_time_zone.md)：设置时区信息。
-   [ss\_mpi\_sys\_get\_time\_zone](ss_mpi_sys_get_time_zone.md)：获取时区信息。
-   [ss\_mpi\_sys\_set\_gps\_info](ss_mpi_sys_set_gps_info.md)：设置GPS信息。
-   [ss\_mpi\_sys\_get\_gps\_info](ss_mpi_sys_get_gps_info.md)：获取GPS信息。
-   [ss\_mpi\_sys\_set\_schedule\_mode](ss_mpi_sys_set_schedule_mode.md)：设置系统调度模式。
-   [ss\_mpi\_sys\_get\_schedule\_mode](ss_mpi_sys_get_schedule_mode.md)：获取系统调度模式。
-   [ss\_mpi\_sys\_set\_vi\_vpss\_mode](ss_mpi_sys_set_vi_vpss_mode.md)：设置VI，VPSS的工作模式。
-   [ss\_mpi\_sys\_get\_vi\_vpss\_mode](ss_mpi_sys_get_vi_vpss_mode.md)：获取VI，VPSS的工作模式。
-   [ss\_mpi\_sys\_set\_vi\_video\_mode](ss_mpi_sys_set_vi_video_mode.md)：设置VI视频模式。
-   [ss\_mpi\_sys\_get\_vi\_video\_mode](ss_mpi_sys_get_vi_video_mode.md)：获取VI视频模式。
-   [ss\_mpi\_sys\_set\_raw\_frame\_compress\_param](ss_mpi_sys_set_raw_frame_compress_param.md)：设置RAW帧压缩参数。
-   [ss\_mpi\_sys\_get\_raw\_frame\_compress\_param](ss_mpi_sys_get_raw_frame_compress_param.md)：获取RAW帧压缩参数。
-   [ss\_mpi\_sys\_set\_tuning\_connect](ss_mpi_sys_set_tuning_connect.md)：设置Tuning工具连接。
-   [ss\_mpi\_sys\_get\_tuning\_connect](ss_mpi_sys_get_tuning_connect.md)：获取Tuning工具连接。
-   [ss\_mpi\_sys\_mem\_share](ss_mpi_sys_mem_share.md)：将handle对应的mmz buffer共享给特定的进程ID。
-   [ss\_mpi\_sys\_mem\_unshare](ss_mpi_sys_mem_unshare.md)：解除handle对应的mmz buffer对进程ID的共享。
-   [ss\_mpi\_sys\_mem\_share\_all](ss_mpi_sys_mem_share_all.md)：将handle对应的mmz buffer以不限进程ID的方式共享给所有进程。
-   [ss\_mpi\_sys\_mem\_unshare\_all](ss_mpi_sys_mem_unshare_all.md)：解除handle对应的mmz buffer对所有进程的共享。
-   [ss\_mpi\_sys\_get\_mem\_info\_by\_handle](ss_mpi_sys_get_mem_info_by_handle.md)：通过handle获取mmz buffer的内存描述信息。
-   [ss\_mpi\_sys\_get\_mem\_info\_by\_phys](ss_mpi_sys_get_mem_info_by_phys.md)：通过物理地址获取mmz buffer的内存描述信息。
-   [ss\_mpi\_sys\_get\_mem\_info\_by\_virt](ss_mpi_sys_get_mem_info_by_virt.md)：通过用户态虚拟地址获取mmz buffer的内存描述信息。
-   [ss\_mpi\_vb\_set\_cfg](ss_mpi_vb_set_cfg.md)：设置MPP视频缓存池属性。
-   [ss\_mpi\_vb\_get\_cfg](ss_mpi_vb_get_cfg.md)：获取MPP视频缓存池属性。
-   [ss\_mpi\_vb\_init](ss_mpi_vb_init.md)：初始化MPP视频缓存池。
-   [ss\_mpi\_vb\_exit](ss_mpi_vb_exit.md)：去初始化MPP视频缓存池。
-   [ss\_mpi\_vb\_create\_pool](ss_mpi_vb_create_pool.md)：创建一个用户视频缓存池。
-   [ss\_mpi\_vb\_create\_ext\_pool](ss_mpi_vb_create_ext_pool.md)：创建一个虚拟视频缓存池。
-   [ss\_mpi\_vb\_destroy\_pool](ss_mpi_vb_destroy_pool.md)：销毁一个视频缓存池。
-   [ss\_mpi\_vb\_get\_blk](ss_mpi_vb_get_blk.md)：获取一个缓存块。
-   [ss\_mpi\_vb\_release\_blk](ss_mpi_vb_release_blk.md)：释放一个已经获取的缓存块。
-   [ss\_mpi\_vb\_insert\_buf\_to\_ext\_pool](ss_mpi_vb_insert_buf_to_ext_pool.md)：添加一个缓存块到虚拟视频缓存池。
-   [ss\_mpi\_vb\_delete\_buf\_from\_ext\_pool](ss_mpi_vb_delete_buf_from_ext_pool.md)：从虚拟视频缓存池中删除一个缓存块。
-   [ss\_mpi\_vb\_phys\_addr\_to\_handle](ss_mpi_vb_phys_addr_to_handle.md)：用户态通过缓存块的物理地址获取其句柄。
-   [ss\_mpi\_vb\_handle\_to\_phys\_addr](ss_mpi_vb_handle_to_phys_addr.md)：获取一个缓存块的物理地址。
-   [ss\_mpi\_vb\_handle\_to\_pool\_id](ss_mpi_vb_handle_to_pool_id.md)：获取一个缓存块所在缓存池的ID。
-   [ss\_mpi\_vb\_get\_pool\_info](ss_mpi_vb_get_pool_info.md)：获取一个视频缓存池的信息。
-   [ss\_mpi\_vb\_init\_mod\_common\_pool](ss_mpi_vb_init_mod_common_pool.md)：初始化模块公共视频缓冲池。
-   [ss\_mpi\_vb\_exit\_mod\_common\_pool](ss_mpi_vb_exit_mod_common_pool.md)：注销模块公共视频缓冲池。
-   [ss\_mpi\_vb\_set\_mod\_pool\_cfg](ss_mpi_vb_set_mod_pool_cfg.md)：设置模块公共视频缓冲池属性。
-   [ss\_mpi\_vb\_get\_mod\_pool\_cfg](ss_mpi_vb_get_mod_pool_cfg.md)：获取模块公共视频缓冲池属性。
-   [ss\_mpi\_vb\_inquire\_user\_cnt](ss_mpi_vb_inquire_user_cnt.md)：查询缓存块使用计数信息。
-   [ss\_mpi\_vb\_get\_supplement\_addr](ss_mpi_vb_get_supplement_addr.md)：获取VB block内存的辅助信息。
-   [ss\_mpi\_vb\_set\_supplement\_cfg](ss_mpi_vb_set_supplement_cfg.md)：设置VB内存的附加信息。
-   [ss\_mpi\_vb\_get\_supplement\_cfg](ss_mpi_vb_get_supplement_cfg.md)：获取VB内存的附加信息。
-   [ss\_mpi\_vb\_get\_common\_pool\_id](ss_mpi_vb_get_common_pool_id.md)：获取公共VB池的pool ID。
-   [ss\_mpi\_vb\_get\_mod\_common\_pool\_id](ss_mpi_vb_get_mod_common_pool_id.md)：获取模块公共VB池的pool ID。
-   [ss\_mpi\_vb\_pool\_share](ss_mpi_vb_pool_share.md)：将pool ID对应的VB池共享给特定的进程ID。
-   [ss\_mpi\_vb\_pool\_unshare](ss_mpi_vb_pool_unshare.md)：解除pool ID对应的VB池对进程ID的共享。
-   [ss\_mpi\_vb\_pool\_share\_all](ss_mpi_vb_pool_share_all.md)：将pool ID对应的VB池以不限进程ID的方式共享给所有进程。
-   [ss\_mpi\_vb\_pool\_unshare\_all](ss_mpi_vb_pool_unshare_all.md)：解除pool ID对应的VB池对所有进程的共享。
-   [ss\_mpi\_log\_set\_level\_cfg](ss_mpi_log_set_level_cfg.md)：设置日志等级。
-   [ss\_mpi\_log\_get\_level\_cfg](ss_mpi_log_get_level_cfg.md)：获取日志等级。
-   [ss\_mpi\_log\_set\_wait\_flag](ss_mpi_log_set_wait_flag.md)：设置读取日志时等待标志。
-   [ss\_mpi\_log\_read](ss_mpi_log_read.md)：读取日志。
-   [ss\_mpi\_log\_close](ss_mpi_log_close.md)：关闭日志文件。
...
[ot\_mpp\_sys\_cfg](#ot_mpp_sys_cfg)
...
必须先调用[ss\_mpi\_sys\_set\_cfg](ss_mpi_sys_set_cfg.md)成功后才能获取配置。
...
[ss\_mpi\_sys\_set\_cfg](#ss_mpi_sys_set_cfg)
...
-   由于MPP系统的正常运行依赖于缓存池，因此需要先调用[ss\_mpi\_vb\_init](ss_mpi_vb_init.md)初始化缓存池，再初始化MPP系统，否则会导致业务运行异常。
-   如果多次初始化，仍会返回成功，但实际上系统不会对MPP的运行状态有任何影响。
-   只要有一个进程进行初始化即可，不需要所有的进程都做系统初始化的操作。
-   由于音频模块依赖用户态属性，故音频不支持多进程操作。用户需要保证音频的相关操作和[ss\_mpi\_vb\_init](ss_mpi_vb_init.md)在同一个进程中。
...
[ss\_mpi\_vb\_exit](#ss_mpi_vb_exit)
...
[ss\_mpi\_vb\_init](#ss_mpi_vb_init)
...
-   Hi3403V100 VI作为数据源，是以设备（pipe）、通道（chn）为发送者，向其他模块发送数据；作为数据接收者时，以设备（pipe）、通道（chn）为接收者。
...
[ss\_mpi\_sys\_unbind](#ss_mpi_sys_unbind)
...
[ss\_mpi\_sys\_bind](#ss_mpi_sys_bind)
...
初始化时间戳基准会将当前系统的时间戳强制置成pts\_base，与系统原有时间戳没有任何约束。因此，建议在媒体业务没有启动时（例如操作系统刚启动），调用这个接口。如果媒体业务已经启动，建议调用[ss\_mpi\_sys\_sync\_pts](ss_mpi_sys_sync_pts.md)进行时间戳微调。
...
-   对应的解映射接口为[ss\_mpi\_sys\_munmap](ss_mpi_sys_munmap.md)。
...
[ss\_mpi\_sys\_munmap](#ss_mpi_sys_munmap)
...
[ss\_mpi\_sys\_mmap](#ss_mpi_sys_mmap)
...
-   此接口应与[ss\_mpi\_sys\_mmap\_cached](ss_mpi_sys_mmap_cached.md)或者[ss\_mpi\_sys\_mmz\_alloc\_cached](ss_mpi_sys_mmz_alloc_cached.md)接口配套使用。
-   用户需要确保传入参数的合法性。

【相关主题】

[ss\_mpi\_sys\_mmap\_cached](#ss_mpi_sys_mmap_cached)
...
-   本接口与[ss\_mpi\_sys\_mmz\_alloc](ss_mpi_sys_mmz_alloc.md)接口的区别：通过本接口分配的内存支持cache缓存，对于频繁使用的内存，最好使用本接口分配内存，这样可以提高cpu读写的效率，提升系统性能，如用户在使用ive算子时，就存在大量数据频繁读写，此时使用此接口来分配内存，就能很好的提高cpu的效率。
-   当cpu访问此接口分配的内存时，会将内存中的数据放在cache中，而硬件设备\(如ive\)只能访问物理内存，不能访问cache的内容，对于这种cpu和硬件会共同操作的内存，需调用[ss\_mpi\_sys\_flush\_cache](ss_mpi_sys_flush_cache.md)做好数据同步。
...
-   [ss\_mpi\_sys\_mmz\_alloc](#ss_mpi_sys_mmz_alloc)
-   [ss\_mpi\_sys\_flush\_cache](#ss_mpi_sys_flush_cache)
...
[ss\_mpi\_sys\_get\_mem\_cfg](#ss_mpi_sys_get_mem_cfg)
...
[ss\_mpi\_sys\_set\_mem\_cfg](#ss_mpi_sys_set_mem_cfg)
...
-   各个缩放比例下的缩放系数等级默认值请参考[ot\_scale\_coef\_level](ot_scale_coef_level.md)结构体的注意事项。
...
-   Hi3403V100不支持此接口。
-   必须在[ss\_mpi\_sys\_init](ss_mpi_sys_init.md)后，所有的VDEC通道和所有的VPSS组创建前设置。
...
[ss\_mpi\_sys\_get\_schedule\_mode](#ss_mpi_sys_get_schedule_mode)
...
[ss\_mpi\_sys\_set\_schedule\_mode](#ss_mpi_sys_set_schedule_mode)
...
-   必须在[ss\_mpi\_sys\_init](ss_mpi_sys_init.md)后，所有的VI PIPE和所有的VPSS组创建前设置。
...
-   SS528V100/SS524V100不支持此接口。
-   必须在[ss\_mpi\_sys\_init](ss_mpi_sys_init.md)接口调用前才能调用。
-   调用[ss\_mpi\_sys\_exit](ss_mpi_sys_exit.md)后设置的值仍然有效。
...
[ss\_mpi\_sys\_mem\_unshare](#ss_mpi_sys_mem_unshare)
...
-   如果mmz buffer已经是全进程共享的状态，接口返回失败。
-   重复解除对进程ID的共享，接口返回失败。
-   与[ss\_mpi\_sys\_mem\_share](ss_mpi_sys_mem_share.md)接口配对使用。
-   如果设置MMZ模块参数mem\_process\_isolation=0，此接口返回成功了也不会生效。
...
[ss\_mpi\_sys\_mem\_share](#ss_mpi_sys_mem_share)
...
[ss\_mpi\_sys\_mem\_unshare\_all](#ss_mpi_sys_mem_unshare_all)
...
-   重复解除对所有进程的共享，接口返回成功。
-   与[ss\_mpi\_sys\_mem\_share\_all](ss_mpi_sys_mem_share_all.md)接口配对使用。
...
[ss\_mpi\_sys\_mem\_share\_all](#ss_mpi_sys_mem_share_all)
...
-   video buf根据不同的应用场景需要不同的配置。配置规则参见[视频缓存池](视频缓存池.md)  。
-   公共缓存池中每个缓存块的大小应根据当前图像像素格式以及图像是否压缩而有所不同。具体分配大小请参考[ot\_vb\_cfg](ot_vb_cfg.md)结构体中的描述。
...
[ss\_mpi\_vb\_get\_cfg](#ss_mpi_vb_get_cfg)
...
必须先调用[ss\_mpi\_vb\_set\_cfg](ss_mpi_vb_set_cfg.md)设置MPP视频缓存池属性，再获取属性。
...
[ss\_mpi\_vb\_set\_cfg](#ss_mpi_vb_set_cfg)
...
-   必须先调用[ss\_mpi\_vb\_set\_cfg](ss_mpi_vb_set_cfg.md)配置缓存池属性，再初始化缓存池，否则会失败。
-   可反复初始化，不返回失败。
...
-   必须先调用[ss\_mpi\_sys\_exit](ss_mpi_sys_exit.md)去初始化MPP系统，再去初始化缓存池，否则返回失败。
-   可以反复去初始化，不返回失败。
-   去初始化不会清除先前对缓存池的配置。
...
[ss\_mpi\_vb\_destroy\_pool](#ss_mpi_vb_destroy_pool)
...
-   用户可以调用[ss\_mpi\_vb\_destroy\_pool](ss_mpi_vb_destroy_pool.md)销毁缓存池，销毁时用户必须保证缓存池里没有实际的缓存块。
...
-   [ss\_mpi\_vb\_insert\_buf\_to\_ext\_pool](#ss_mpi_vb_insert_buf_to_ext_pool)
-   [ss\_mpi\_vb\_delete\_buf\_from\_ext\_pool](#ss_mpi_vb_delete_buf_from_ext_pool)
-   [ss\_mpi\_vb\_destroy\_pool](#ss_mpi_vb_destroy_pool)
...
-   退出VB池之前请确保VB池里的任何VB都没有被占用，否则无法退出。
-   \[0,  [OT\_VB\_MAX\_POOLS](OT_VB_MAX_POOLS.md)\)范围内的缓存池ID号，包括公共缓存池、模块公共缓存池、模块私有缓存池等的ID号。请确保Pool为[ss\_mpi\_vb\_create\_pool](ss_mpi_vb_create_pool.md)所创建的缓存池的ID号，否则会返回失败。
...
[ss\_mpi\_vb\_create\_pool](#ss_mpi_vb_create_pool)
...
[ss\_mpi\_vb\_release\_blk](#ss_mpi_vb_release_blk)
...
[ss\_mpi\_vb\_get\_blk](#ss_mpi_vb_get_blk)
...
-   缓存块添加到虚拟视频缓存池后，不能再调用[ss\_mpi\_vb\_get\_blk](ss_mpi_vb_get_blk.md)获取缓存块。
-   用户传递的物理地址需在有效的mmz地址范围内，且需要256字节对齐，接口会检查是否会存在地址冲突。
-   虚拟视频缓存池里缓存块的物理地址没有做虚拟地址映射，如果用户需要使用虚拟地址，请自己调用map函数映射。
-   用户可以调用[ss\_mpi\_vb\_delete\_buf\_from\_ext\_pool](ss_mpi_vb_delete_buf_from_ext_pool.md)从缓存池里释放缓存块。释放后缓存块就不在缓存池里，用户如果需要再使用，需要再次调用此接口把缓存块增加到缓存池。
-   用户可以调用[ss\_mpi\_vb\_inquire\_user\_cnt](ss_mpi_vb_inquire_user_cnt.md)查询此缓存块是否被占用。
...
-   [ss\_mpi\_vb\_create\_ext\_pool](#ss_mpi_vb_create_ext_pool)
-   [ss\_mpi\_vb\_delete\_buf\_from\_ext\_pool](#ss_mpi_vb_delete_buf_from_ext_pool)
...
-   当前公共视频缓冲池仅适用于VDEC模块。
-   必须先调用[ss\_mpi\_vb\_init](ss_mpi_vb_init.md)进行公共视频缓冲池初始化。
-   必须先调用[ss\_mpi\_vb\_set\_mod\_pool\_cfg](ss_mpi_vb_set_mod_pool_cfg.md)配置缓存池属性，再初始化缓存池，否则会失败。
-   可反复初始化，不返回失败。
...
[ss\_mpi\_vb\_exit\_mod\_common\_pool](#ss_mpi_vb_exit_mod_common_pool)
...
-   必须在[ss\_mpi\_vb\_exit](ss_mpi_vb_exit.md)之前调用该接口，否则返回失败。
-   可以反复退出，不返回失败。
...
[ss\_mpi\_vb\_init\_mod\_common\_pool](#ss_mpi_vb_init_mod_common_pool)
...
[ss\_mpi\_vb\_get\_mod\_pool\_cfg](#ss_mpi_vb_get_mod_pool_cfg)
...
必须先调用[ss\_mpi\_vb\_set\_mod\_pool\_cfg](ss_mpi_vb_set_mod_pool_cfg.md)设置模块公共视频缓存池属性，再获取属性。
...
[ss\_mpi\_vb\_set\_mod\_pool\_cfg](#ss_mpi_vb_set_mod_pool_cfg)
...
-   当前支持5种VB内存附加信息，分别为：
    ...
    -   视频扩展补充信息。对应结构体为ot\_video\_supplement\_misc，对应的MASK为OT\_VB\_SUPPLEMENT\_MISC\_MASK，Hi3403V100配置无效。
...
[ss\_mpi\_vb\_get\_supplement\_cfg](#ss_mpi_vb_get_supplement_cfg)
...
[ss\_mpi\_vb\_set\_supplement\_cfg](#ss_mpi_vb_set_supplement_cfg)
...
-   [ss\_mpi\_vb\_pool\_share](#ss_mpi_vb_pool_share)
-   [ss\_mpi\_vb\_pool\_unshare](#ss_mpi_vb_pool_unshare)
-   [ss\_mpi\_vb\_pool\_share\_all](#ss_mpi_vb_pool_share_all)
-   [ss\_mpi\_vb\_pool\_unshare\_all](#ss_mpi_vb_pool_unshare_all)
...
-   与[ss\_mpi\_vb\_pool\_share](ss_mpi_vb_pool_share.md)接口配对使用。
-   不支持虚拟VB池。
-   如果设置MMZ模块参数mem\_process\_isolation=0，此接口返回成功了也不会生效。
...
-   重复解除对所有进程的共享，接口返回成功。
-   与[ss\_mpi\_vb\_pool\_share\_all](ss_mpi_vb_pool_share_all.md)接口配对使用。
-   调用此接口解除对所有进程共享的状态之后，仍然保留原有对进程ID的共享状态。
-   不支持虚拟VB池。
-   如果设置MMZ模块参数mem\_process\_isolation=0，此接口返回成功了也不会生效。
...
[ss\_mpi\_log\_get\_level\_cfg](#ss_mpi_log_get_level_cfg)
...
[ss\_mpi\_log\_set\_level\_cfg](#ss_mpi_log_set_level_cfg)
...
-   [OT\_MAX\_BIND\_DST\_NUM](OT_MAX_BIND_DST_NUM.md)：定义绑定目标的最大个数。
-   [OT\_MAX\_VERSION\_NAME\_LEN](OT_MAX_VERSION_NAME_LEN.md)：定义版本名称的最大长度。
-   [OT\_MAX\_MOD\_NAME\_LEN](OT_MAX_MOD_NAME_LEN.md)：定义模块名称的最大长度。
-   [OT\_UNIQUE\_ID\_NUM](OT_UNIQUE_ID_NUM.md)：定义unique ID长度
-   [ot\_mpp\_sys\_cfg](ot_mpp_sys_cfg.md)：定义MPP系统控制属性结构体。
-   [ot\_mpp\_chn](ot_mpp_chn.md)：定义模块设备通道结构体。
-   [ot\_mpp\_bind\_dst](ot_mpp_bind_dst.md)：定义MPP系统绑定目的信息结构体。
-   [ot\_mpp\_version](ot_mpp_version.md)：定义MPP版本描述结构体。
-   [ot\_sys\_virt\_mem\_info](ot_sys_virt_mem_info.md)：定义虚拟内存信息结构体。
-   [ot\_sys\_mem\_info](ot_sys_mem_info.md)：定义mmz buffer的内存描述信息结构体。
-   [ot\_unique\_id](ot_unique_id.md)：定义unique ID结构体。
-   [ot\_scale\_range\_type](ot_scale_range_type.md)：定义VPSS和VGS的缩放比例范围的枚举类型。
-   [ot\_coef\_level](ot_coef_level.md)：定义VPSS和VGS缩放系数等级的枚举类型。
-   [ot\_scale\_range](ot_scale_range.md)：定义VPSS和VGS的水平和垂直缩放比例范围的结构体。
-   [ot\_scale\_coef\_level](ot_scale_coef_level.md)：定义VPSS和VGS水平亮度、水平色度、垂直亮度、垂直色度的缩放系数等级的结构体。
-   [ot\_vb\_pool\_info](ot_vb_pool_info.md)：定义视频缓存池信息结构体。
-   [ot\_vb\_cfg](ot_vb_cfg.md)：定义视频缓存池属性结构体。
-   [ot\_vb\_pool\_cfg](ot_vb_pool_cfg.md)：定义视频缓存池属性结构体。
-   [ot\_vb\_remap\_mode](ot_vb_remap_mode.md)：定义VB内核态虚拟地址映射模式。
-   [ot\_vb\_common\_pools\_id](ot_vb_common_pools_id.md)：定义公共VB池ID的结构体。
-   [ot\_log\_level\_cfg](ot_log_level_cfg.md)：定义日志等级信息结构体。
-   [ot\_vi\_vpss\_mode\_type](ot_vi_vpss_mode_type.md)：定义VI PIPE和VPSS组的工作模式。
-   [ot\_vi\_vpss\_mode](ot_vi_vpss_mode.md)：定义VI各个PIPE和VPSS各个组的工作模式。
-   [ot\_vi\_video\_mode](ot_vi_video_mode.md)：定义VI，VIDEO模式。
-   [ot\_ext\_chn\_src\_type](ot_ext_chn_src_type.md)：定义扩展通道的图像来源。
-   [ot\_raw\_frame\_compress\_param](ot_raw_frame_compress_param.md)：定义VI模块RAW数据的压缩比。
...
-   [ss\_mpi\_sys\_bind](#ss_mpi_sys_bind)
-   [ss\_mpi\_sys\_unbind](#ss_mpi_sys_unbind)
-   [ss\_mpi\_sys\_get\_bind\_by\_dst](#ss_mpi_sys_get_bind_by_dst)
...
[ss\_mpi\_sys\_get\_bind\_by\_src](#ss_mpi_sys_get_bind_by_src)
...
[ss\_mpi\_sys\_get\_version](#ss_mpi_sys_get_version)
...
[ss\_mpi\_sys\_get\_virt\_mem\_info](#ss_mpi_sys_get_virt_mem_info)
...
-   [ss\_mpi\_sys\_get\_mem\_info\_by\_handle](#ss_mpi_sys_get_mem_info_by_handle)
-   [ss\_mpi\_sys\_get\_mem\_info\_by\_phys](#ss_mpi_sys_get_mem_info_by_phys)
-   [ss\_mpi\_sys\_get\_mem\_info\_by\_virt](#ss_mpi_sys_get_mem_info_by_virt)
...
[ss\_mpi\_sys\_get\_unique\_id](#ss_mpi_sys_get_unique_id)
...
-   [ss\_mpi\_sys\_set\_scale\_coef\_level](#ss_mpi_sys_set_scale_coef_level)
-   [ss\_mpi\_sys\_get\_scale\_coef\_level](#ss_mpi_sys_get_scale_coef_level)
...
-   [ss\_mpi\_vb\_get\_common\_pool\_id](#ss_mpi_vb_get_common_pool_id)
-   [ss\_mpi\_vb\_get\_mod\_common\_pool\_id](#ss_mpi_vb_get_mod_common_pool_id)
...
-   Hi3403V100只支持OT\_VI\_OFFLINE\_VPSS\_OFFLINE，OT\_VI\_OFFLINE\_VPSS\_ONLINE，OT\_VI\_ONLINE\_VPSS\_OFFLINE，OT\_VI\_ONLINE\_VPSS\_ONLINE四种模式。
-   当设置OT\_VI\_OFFLINE\_VPSS\_ONLINE、OT\_VI\_ONLINE\_VPSS\_ONLINE时，对应VI PIPE pipe\_id和VPSS组 grp\_id必须保持一致。
-   SS528V100/SS524V100/SS626V100不支持此结构体。

【相关数据类型及接口】

-   [ss\_mpi\_sys\_set\_vi\_vpss\_mode](#ss_mpi_sys_set_vi_vpss_mode)
-   [ss\_mpi\_sys\_get\_vi\_vpss\_mode](#ss_mpi_sys_get_vi_vpss_mode)
...
-   [ss\_mpi\_sys\_set\_vi\_video\_mode](#ss_mpi_sys_set_vi_video_mode)
-   [ss\_mpi\_sys\_get\_vi\_video\_mode](#ss_mpi_sys_get_vi_video_mode)
...
-   [ss\_mpi\_sys\_set\_raw\_frame\_compress\_param](#ss_mpi_sys_set_raw_frame_compress_param)
-   [ss\_mpi\_sys\_get\_raw\_frame\_compress\_param](#ss_mpi_sys_get_raw_frame_compress_param)

### 视频公共类型<a name="ZH-CN_TOPIC_0000002441659165"></a>

视频公共数据类型定义如下：

-   [OT\_GPS\_EXPRESS\_NUM](OT_GPS_EXPRESS_NUM.md)：定义GPS经纬度数据个数。
-   [OT\_GPS\_GRP\_NUM](OT_GPS_GRP_NUM.md)：定义GPS经纬度维度。
-   [OT\_MAX\_COLOR\_COMPONENT](OT_MAX_COLOR_COMPONENT-0.md)：最大颜色分量个数。
-   [OT\_MAX\_PRIVATE\_DATA\_NUM](OT_MAX_PRIVATE_DATA_NUM-1.md)：最大私有数据数目。
-   [OT\_MAX\_USER\_DATA\_NUM](OT_MAX_USER_DATA_NUM.md)：最大用户数据数目。
-   [OT\_ISP\_WB\_GAIN\_NUM](OT_ISP_WB_GAIN_NUM.md)：WB颜色通道数。
-   [OT\_ISP\_CAP\_CCM\_NUM](OT_ISP_CAP_CCM_NUM.md)：颜色校正矩阵大小。
-   [ot\_aspect\_ratio\_type](ot_aspect_ratio_type.md)：定义幅型比类型。
-   [ot\_aspect\_ratio](ot_aspect_ratio.md)：定义幅型比信息。
-   [ot\_pixel\_format](ot_pixel_format.md)：定义像素格式类型。
-   [ot\_video\_field](ot_video_field.md)：定义视频图像帧场类型。
-   [ot\_color\_gamut](ot_color_gamut.md)：定义色域范围枚举。
-   [ot\_dynamic\_range](ot_dynamic_range.md)：定义动态范围枚举。
-   [ot\_data\_bit\_width](ot_data_bit_width.md)：定义数据位宽枚举。
-   [ot\_video\_supplement\_misc](ot_video_supplement_misc.md)：定义视频扩展补充信息结构体。
-   [ot\_video\_supplement](ot_video_supplement.md)：定义视频图像帧补充信息。
-   [ot\_video\_frame](ot_video_frame.md)：定义视频原始图像帧结构体。
-   [ot\_video\_frame\_info](ot_video_frame_info.md)：定义视频图像帧信息结构体。
-   [ot\_vb\_calc\_cfg](ot_vb_calc_cfg.md)：定义视频图像帧各部分数据大小配置信息结构体。
-   [ot\_bmp](ot_bmp.md)：定义位图图像信息结构。
-   [ot\_video\_format](ot_video_format.md)：定义视频格式结构体。
-   [ot\_compress\_mode](ot_compress_mode.md)：定义视频压缩数据格式结构体。
-   [ot\_vb\_supplement\_cfg](ot_vb_supplement_cfg.md)：定义VB辅助信息结构体。
-   [ot\_gps\_info](ot_gps_info.md)：定义GPS信息结构体。
-   [ot\_frame\_interrupt\_type](ot_frame_interrupt_type.md)：定义帧中断类型的枚举。
-   [ot\_frame\_interrupt\_attr](ot_frame_interrupt_attr.md)：定义帧中断属性结构体。
-   [ot\_data\_rate](ot_data_rate.md)：定义设备的速率的枚举。
-   [ot\_wdr\_mode](ot_wdr_mode.md)：定义WDR工作模式的枚举。
-   [ot\_isp\_config\_info](ot_isp_config_info.md)：定义ISP配置信息结构体。
-   [ot\_low\_delay\_info](ot_low_delay_info.md)：定义低延时信息结构体。
-   [ot\_schedule\_mode](ot_schedule_mode.md)：定义调度模式的枚举。
-   [ot\_scale\_coef\_type](ot_scale_coef_type.md)：定义缩放系数类型的枚举。
...
-   [ot\_aspect\_ratio\_type](#ot_aspect_ratio_type)
-   ot\_vo\_chn\_param
...
-   SS528V100/SS524V100/Hi3403V100仅支持SDR8。
...
当前支持5种附加信息，具体请参考[ss\_mpi\_vb\_set\_supplement\_cfg](ss_mpi_vb_set_supplement_cfg.md)接口描述。
...
-   [ss\_mpi\_sys\_set\_gps\_info](#ss_mpi_sys_set_gps_info)
-   [ss\_mpi\_sys\_get\_gps\_info](#ss_mpi_sys_get_gps_info)
...
<p id="p3614mcpsimp"><a name="p3614mcpsimp"></a><a name="p3614mcpsimp"></a>Hi3403V100最小值为0，其他最小值为40；最大值小于图像高度。</p>
</td>
</tr>
</tbody>
</table>
...
当配置为正常缩放系数类型（OT\_SCALE\_COEF\_TYPE\_NORMAL）时，采用[ss\_mpi\_sys\_set\_scale\_coef\_level](ss_mpi_sys_set_scale_coef_level.md)接口配置后生成的缩放系数对图像进行缩放；而配置为双线性缩放系数类型（OT\_SCALE\_COEF\_TYPE\_BILINEAR）时，采用双线性插值。
...
### Hi3403V100各模块输出支持的数据格式和压缩模式<a name="ZH-CN_TOPIC_0000002441698973"></a>
...
### Hi3403V100各模块输入支持的数据格式和压缩模式<a name="ZH-CN_TOPIC_0000002408099914"></a>
