...
区域管理可以实现区域的创建，并叠加到视频中或对视频进行遮挡。例如，实际应用中，用户通过创建一个区域，通过[ss\_mpi\_rgn\_attach\_to\_chn](ss_mpi_rgn_attach_to_chn.md)，将该区域叠加到某个通道（如VENC通道）中。在通道进行调度时，则会将OSD叠加在视频中。一个区域支持通过设置通道显示属性接口指定到多个通道中（如：多个VENC通道，多个VI通道，甚至多个VENC和VI通道），且支持在每个通道的显示属性（如位置、层次、透明度等）都不同。
...
-   位图填充\(针对OVERLAY和OVERLAYEX有效\)
    ...
    -   其一、用户通过[ss\_mpi\_rgn\_set\_bmp](ss_mpi_rgn_set_bmp.md)接口将位图数据拷贝至内部显示画布；
    -   其二、用户通过[ss\_mpi\_rgn\_get\_canvas\_info](ss_mpi_rgn_get_canvas_info.md)获取内部备份显示画布的地址，直接对该地址数据进行更新，然后调用[ss\_mpi\_rgn\_update\_canvas](ss_mpi_rgn_update_canvas.md)接口将备份显示画布更新为待显示画布，达到实现更新位图数据的目的。

-   区域公共属性

    用户创建一个区域时，需要设置该属性信息，它包含公共的资源信息。例如，OVERLAY包含像素格式，大小和背景色。

-   通道显示属性\([ot\_rgn\_type\_chn\_attr](ot_rgn_type_chn_attr.md)\)
...
-   颜色值通过[ss\_mpi\_rgn\_set\_attr](ss_mpi_rgn_set_attr.md)接口设置。
...
-   通过[ss\_mpi\_rgn\_get\_attr](ss_mpi_rgn_get_attr.md)、[ss\_mpi\_rgn\_set\_attr](ss_mpi_rgn_set_attr.md)获取和设置区域属性。
-   通过[ss\_mpi\_rgn\_set\_bmp](ss_mpi_rgn_set_bmp.md)\(仅针对OVERLAY、OVERLAYEX\)设置区域的位图信息。
-   通过[ss\_mpi\_rgn\_get\_display\_attr](ss_mpi_rgn_get_display_attr.md)和[ss\_mpi\_rgn\_set\_display\_attr](ss_mpi_rgn_set_display_attr.md)获取和设置区域在某通道（如VENC通道）的通道显示属性。
...
1.  创建OVERLAY。
2.  设置[ot\_rgn\_overlay\_attr](ot_rgn_overlay_attr.md)属性，设置clut\[[OT\_RGN\_CLUT\_NUM](OT_RGN_CLUT_NUM.md)\]中的值代表了查色表中的颜色值。
3.  调用[ss\_mpi\_rgn\_attach\_to\_chn](ss_mpi_rgn_attach_to_chn.md)接口绑定到VENC通道上。
4.  调用[ss\_mpi\_rgn\_set\_bmp](ss_mpi_rgn_set_bmp.md)可以加载二值化后的位图。
5.  调用[ss\_mpi\_rgn\_set\_display\_attr](ss_mpi_rgn_set_display_attr.md)可以设置颜色值。
6.  不用时解绑定，销毁区域。
...
3.  调用[ss\_mpi\_rgn\_batch\_begin](ss_mpi_rgn_batch_begin.md)设置需要进行批处理的区域。
4.  调用接口[ss\_mpi\_rgn\_set\_bmp](ss_mpi_rgn_set_bmp.md)\(仅针对OVERLAY、OVERLAYEX\)设置各个区域位图信息或者调用[ss\_mpi\_rgn\_get\_canvas\_info](ss_mpi_rgn_get_canvas_info.md)设置各个区域位图信息。
5.  调用[ss\_mpi\_rgn\_set\_display\_attr](ss_mpi_rgn_set_display_attr.md)设置各个区域显示属性。
6.  完成各个区域的设置后，调用[ss\_mpi\_rgn\_batch\_end](ss_mpi_rgn_batch_end.md)对各个区域的信息同步更新。

    其它具体细节参考sample。

    -   支持批处理的MPI接口有：

        [ss\_mpi\_rgn\_set\_bmp](ss_mpi_rgn_set_bmp.md)，[ss\_mpi\_rgn\_set\_display\_attr](ss_mpi_rgn_set_display_attr.md)，[ss\_mpi\_rgn\_get\_canvas\_info](ss_mpi_rgn_get_canvas_info.md)，[ss\_mpi\_rgn\_batch\_begin](ss_mpi_rgn_batch_begin.md)，[ss\_mpi\_rgn\_batch\_end](ss_mpi_rgn_batch_end.md)。

    -   不支持批处理的MPI接口有：

        [ss\_mpi\_rgn\_set\_attr](ss_mpi_rgn_set_attr.md)，[ss\_mpi\_rgn\_destroy](ss_mpi_rgn_destroy.md)，[ss\_mpi\_rgn\_detach\_from\_chn](ss_mpi_rgn_detach_from_chn.md)，[ss\_mpi\_rgn\_update\_canvas](ss_mpi_rgn_update_canvas.md)。

    -   接口功能与批处理无关：

        [ss\_mpi\_rgn\_create](ss_mpi_rgn_create.md)，[ss\_mpi\_rgn\_get\_attr](ss_mpi_rgn_get_attr.md)，[ss\_mpi\_rgn\_attach\_to\_chn](ss_mpi_rgn_attach_to_chn.md)，[ss\_mpi\_rgn\_get\_display\_attr](ss_mpi_rgn_get_display_attr.md)。
...
-   [ss\_mpi\_rgn\_create](ss_mpi_rgn_create.md)：创建区域。
-   [ss\_mpi\_rgn\_destroy](ss_mpi_rgn_destroy.md)：销毁区域。
-   [ss\_mpi\_rgn\_get\_attr](ss_mpi_rgn_get_attr.md)：获取区域属性
-   [ss\_mpi\_rgn\_set\_attr](ss_mpi_rgn_set_attr.md)：设置区域属性。
-   [ss\_mpi\_rgn\_set\_bmp](ss_mpi_rgn_set_bmp.md)：设置区域位图。
-   [ss\_mpi\_rgn\_attach\_to\_chn](ss_mpi_rgn_attach_to_chn.md)：将区域叠加到通道上。
-   [ss\_mpi\_rgn\_detach\_from\_chn](ss_mpi_rgn_detach_from_chn.md)：将区域从通道中撤出。
-   [ss\_mpi\_rgn\_set\_display\_attr](ss_mpi_rgn_set_display_attr.md)：设置区域的通道显示属性。
-   [ss\_mpi\_rgn\_get\_display\_attr](ss_mpi_rgn_get_display_attr.md)：获取区域的通道显示属性。
-   [ss\_mpi\_rgn\_get\_canvas\_info](ss_mpi_rgn_get_canvas_info.md)：获取区域画布信息。
-   [ss\_mpi\_rgn\_update\_canvas](ss_mpi_rgn_update_canvas.md)：更新区域画布信息。
-   [ss\_mpi\_rgn\_batch\_begin](ss_mpi_rgn_batch_begin.md)：设置区域批处理。
-   [ss\_mpi\_rgn\_batch\_end](ss_mpi_rgn_batch_end.md)：区域批处理信息同步。
-   [ss\_mpi\_rgn\_close\_fd](ss_mpi_rgn_close_fd.md)：关闭区域管理模块文件描述符。
...
<p id="p4347mcpsimp"><a name="p4347mcpsimp"></a><a name="p4347mcpsimp"></a>取值范围：[0, <a href="OT_RGN_HANDLE_MAX.md">OT_RGN_HANDLE_MAX</a>)。</p>
</td>
...
-   该句柄由用户指定，意义等同于ID号。
-   不支持重复创建。
-   区域属性必须合法，具体约束参见[ot\_rgn\_type\_attr](ot_rgn_type_attr.md)。
-   区域属性指针不能为空。
-   创建COVER、COVEREX、LINE、MOSAIC、MOSAICEX、CORNER\_RECTEX时，只需指定区域类型即可。其它的属性，如区域位置，层次等信息在调用[ss\_mpi\_rgn\_attach\_to\_chn](ss_mpi_rgn_attach_to_chn.md)接口时指定。
...
-   [ss\_mpi\_rgn\_destroy](#ss_mpi_rgn_destroy)
-   [ss\_mpi\_rgn\_get\_attr](#ss_mpi_rgn_get_attr)
-   [ss\_mpi\_rgn\_set\_attr](#ss_mpi_rgn_set_attr)
...
<p id="p1205mcpsimp"><a name="p1205mcpsimp"></a><a name="p1205mcpsimp"></a>取值范围：[0, <a href="OT_RGN_HANDLE_MAX.md">OT_RGN_HANDLE_MAX</a>)。</p>
</td>
...
-   调用该接口的过程中，不允许同时调用[ss\_mpi\_rgn\_set\_attr](ss_mpi_rgn_set_attr.md)/[ss\_mpi\_rgn\_set\_bmp](ss_mpi_rgn_set_bmp.md)。
-   无论该接口调用成功或者失败，用户都需要重新调用[ss\_mpi\_rgn\_get\_canvas\_info](ss_mpi_rgn_get_canvas_info.md)才能对画布进行操作。

【举例】

参见[ss\_mpi\_rgn\_create](ss_mpi_rgn_create.md)举例。
...
<p id="p1281mcpsimp"><a name="p1281mcpsimp"></a><a name="p1281mcpsimp"></a>取值范围：[0, <a href="OT_RGN_HANDLE_MAX.md">OT_RGN_HANDLE_MAX</a>)。</p>
</td>
...
<p id="p1358mcpsimp"><a name="p1358mcpsimp"></a><a name="p1358mcpsimp"></a>取值范围：[0, <a href="OT_RGN_HANDLE_MAX.md">OT_RGN_HANDLE_MAX</a>)。</p>
</td>
...
-   区域必须已创建。
-   当区域通过[ss\_mpi\_rgn\_attach\_to\_chn](ss_mpi_rgn_attach_to_chn.md)接口绑定到通道上时，本接口不可以用于修改静态属性，但是可以修改动态属性；当区域没有attach到任何通道上时，本接口即可用于修改静态属性，也可用于修改动态属性。
-   区域属性指针不能为空。
-   调用了[ss\_mpi\_rgn\_get\_canvas\_info](ss_mpi_rgn_get_canvas_info.md)之后，调用本接口无效，除非调用[ss\_mpi\_rgn\_update\_canvas](ss_mpi_rgn_update_canvas.md)更新画布生效。
-   调用该接口的过程中，不允许同时调用[ss\_mpi\_rgn\_destroy](ss_mpi_rgn_destroy.md)。
...
<p id="p1443mcpsimp"><a name="p1443mcpsimp"></a><a name="p1443mcpsimp"></a>取值范围：[0, <a href="OT_RGN_HANDLE_MAX.md">OT_RGN_HANDLE_MAX</a>)。</p>
</td>
...
<p id="p1547mcpsimp"><a name="p1547mcpsimp"></a><a name="p1547mcpsimp"></a>取值范围：[0, <a href="OT_RGN_HANDLE_MAX.md">OT_RGN_HANDLE_MAX</a>)。</p>
</td>
...
-   区域通道显示属性指针不能为空。
-   支持多次叠加，但此接口不用于改变属性。
-   如果是OVERLAY区域，通道显示属性中的qp信息需要注意，参见结构体[ot\_rgn\_overlay\_qp\_info](ot_rgn_overlay_qp_info.md)描述。
-   将区域叠加到组上时，必须叠加到该组的通道0上，否则叠加不成功。
-   将OVERLAY区域叠加到VENC通道上时，必须配置[ot\_rgn\_overlay\_chn\_attr](ot_rgn_overlay_chn_attr.md)的[ot\_rgn\_attach\_dst](ot_rgn_attach_dst.md)属性，否则叠加不成功。
...
<p id="p1637mcpsimp"><a name="p1637mcpsimp"></a><a name="p1637mcpsimp"></a>取值范围：[0, <a href="OT_RGN_HANDLE_MAX.md">OT_RGN_HANDLE_MAX</a>)。</p>
</td>
...
<p id="p1713mcpsimp"><a name="p1713mcpsimp"></a><a name="p1713mcpsimp"></a>取值范围：[0, <a href="OT_RGN_HANDLE_MAX.md">OT_RGN_HANDLE_MAX</a>)。</p>
</td>
...
-   区域通道显示属性指针不能为空。
-   区域必须先叠加到通道上。
-   静态属性不能修改，动态属性可以被修改，详细情况参见结构体[ot\_rgn\_type\_chn\_attr](ot_rgn_type_chn_attr.md)。
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.2 "><p id="p1827mcpsimp"><a name="p1827mcpsimp"></a><a name="p1827mcpsimp"></a>区域句柄号。取值范围：[0, <a href="OT_RGN_HANDLE_MAX.md">OT_RGN_HANDLE_MAX</a>)</p>
</td>
...
请参见[ss\_mpi\_rgn\_set\_display\_attr](ss_mpi_rgn_set_display_attr.md)的【举例】。
...
<p id="p1910mcpsimp"><a name="p1910mcpsimp"></a><a name="p1910mcpsimp"></a>取值范围：[0, <a href="OT_RGN_HANDLE_MAX.md">OT_RGN_HANDLE_MAX</a>)。</p>
</td>
...
-   区域必须已创建。
-   本接口与[ss\_mpi\_rgn\_set\_bmp](ss_mpi_rgn_set_bmp.md)功能类似，本接口相对于[ss\_mpi\_rgn\_set\_bmp](ss_mpi_rgn_set_bmp.md)接口，用户可以直接更新显示内部画布数据，节省一次内存拷贝。
-   本接口用于获取区域对应的画布信息，在得到画布地址之后，用户可直接对画布进行操作，譬如：将bmp数据直接填写到该画布中。然后通过调用[ss\_mpi\_rgn\_update\_canvas](ss_mpi_rgn_update_canvas.md)接口，更新显示画布数据。
-   本接口与[ss\_mpi\_rgn\_set\_bmp](ss_mpi_rgn_set_bmp.md)接口互斥。如果已经使用了本接口，那么在调用[ss\_mpi\_rgn\_update\_canvas](ss_mpi_rgn_update_canvas.md)前，调用[ss\_mpi\_rgn\_set\_bmp](ss_mpi_rgn_set_bmp.md)不生效。
...
<p id="p2020mcpsimp"><a name="p2020mcpsimp"></a><a name="p2020mcpsimp"></a>取值范围：[0, <a href="OT_RGN_HANDLE_MAX.md">OT_RGN_HANDLE_MAX</a>)。</p>
</td>
...
-   区域必须已创建。
-   本接口配合[ss\_mpi\_rgn\_get\_canvas\_info](ss_mpi_rgn_get_canvas_info.md)使用。主要用于画布内存数据更新之后，进行画布切换显示。
-   每次调用本接口前，都必须先获取画布信息，然后在调用本接口进行更新。
-   本接口仅支持OVERLAY或OVERLAYEX类型的区域。

【举例】

请参见[ss\_mpi\_rgn\_get\_canvas\_info](ss_mpi_rgn_get_canvas_info.md)。
...
<p id="p2111mcpsimp"><a name="p2111mcpsimp"></a><a name="p2111mcpsimp"></a>数组长度取值范围：[2, <a href="OT_RGN_BATCH_HANDLE_MAX.md">OT_RGN_BATCH_HANDLE_MAX</a>)</p>
<p id="p2113mcpsimp"><a name="p2113mcpsimp"></a><a name="p2113mcpsimp"></a>数组元素取值范围：[0, <a href="OT_RGN_HANDLE_MAX.md">OT_RGN_HANDLE_MAX</a>)</p>
</td>
...
-   区域类型必须是OVERLAY类型。
-   必须与[ss\_mpi\_rgn\_batch\_end](ss_mpi_rgn_batch_end.md)成对出现。
-   此接口不支持多线程调用。
-   单buf模式下批处理不生效
-   批处理中不可以调用[ss\_mpi\_rgn\_set\_attr](ss_mpi_rgn_set_attr.md)，[ss\_mpi\_rgn\_destroy](ss_mpi_rgn_destroy.md)，[ss\_mpi\_rgn\_detach\_from\_chn](ss_mpi_rgn_detach_from_chn.md)，[ss\_mpi\_rgn\_update\_canvas](ss_mpi_rgn_update_canvas.md)接口。
...
[ss\_mpi\_rgn\_batch\_end](#ss_mpi_rgn_batch_end)
...
-   区域必须已创建。
-   区域批处理组号必须已获取。
-   本接口是配合[ss\_mpi\_rgn\_batch\_begin](#ss_mpi_rgn_batch_begin)
-   区域类型必须是OVERLAY类型。
-   此接口不支持多线程调用。

【举例】

请参见[ss\_mpi\_rgn\_batch\_begin](#ss_mpi_rgn_batch_begin)
...
-   [OT\_RGN\_CLUT\_NUM](OT_RGN_CLUT_NUM.md)：定义RGB颜色查表个数。
-   [OT\_RGN\_GRP\_MAX](OT_RGN_GRP_MAX.md)：定义批处理最大的组数。
-   [OT\_RGN\_HANDLE\_MAX](OT_RGN_HANDLE_MAX.md)：定义区域的最大句柄数。
-   [OT\_RGN\_BATCH\_HANDLE\_MAX](OT_RGN_BATCH_HANDLE_MAX.md)：定义一组批处理最多区域句柄数。
-   [OT\_RGN\_MAX\_BUF\_NUM](OT_RGN_MAX_BUF_NUM.md)：定义区域使用的最大内存数量。
-   [OT\_RGN\_MIN\_WIDTH](OT_RGN_MIN_WIDTH.md)：定义区域宽度的最小值。
-   [OT\_RGN\_MIN\_HEIGHT](OT_RGN_MIN_HEIGHT.md)：定义区域高度的最小值。
-   [OT\_RGN\_ALIGN](OT_RGN_ALIGN.md)：定义区域坐标宽高度对齐值。
-   [OT\_RGN\_COVER\_MIN\_X](OT_RGN_COVER_MIN_X.md)：定义COVER矩形区域起始位置以及任意四边形坐标点X坐标最小值。
-   [OT\_RGN\_COVER\_MIN\_Y](OT_RGN_COVER_MIN_Y.md)：定义COVER矩形区域起始位置以及任意四边形坐标点Y坐标最小值。
-   [OT\_RGN\_COVER\_MAX\_X](OT_RGN_COVER_MAX_X.md)：定义COVER矩形区域起始位置以及任意四边形坐标点X坐标最大值。
-   [OT\_RGN\_COVER\_MAX\_Y](OT_RGN_COVER_MAX_Y.md)：定义COVER矩形区域起始位置以及任意四边形坐标点Y坐标最大值。
-   [OT\_RGN\_COVER\_MAX\_WIDTH](OT_RGN_COVER_MAX_WIDTH.md)：定义COVER矩形区域的最大宽度。
-   [OT\_RGN\_COVER\_MAX\_HEIGHT](OT_RGN_COVER_MAX_HEIGHT.md)：定义COVER矩形区域的最大高度。
-   [OT\_RGN\_COVEREX\_MIN\_X](OT_RGN_COVEREX_MIN_X.md)：定义COVEREX矩形区域起始位置以及任意四边形坐标点X坐标最小值。
-   [OT\_RGN\_COVEREX\_MIN\_Y](OT_RGN_COVEREX_MIN_Y.md)：定义COVEREX矩形区域起始位置以及任意四边形坐标点Y坐标最小值。
-   [OT\_RGN\_COVEREX\_MAX\_X](OT_RGN_COVEREX_MAX_X.md)：定义COVEREX矩形区域起始位置以及任意四边形坐标点X坐标最大值。
-   [OT\_RGN\_COVEREX\_MAX\_Y](OT_RGN_COVEREX_MAX_Y.md)：定义COVEREX矩形区域起始位置以及任意四边形坐标点Y坐标最大值。
-   [OT\_RGN\_COVEREX\_MAX\_WIDTH](OT_RGN_COVEREX_MAX_WIDTH.md)：定义COVEREX矩形区域的最大宽度。
-   [OT\_RGN\_COVEREX\_MAX\_HEIGHT](OT_RGN_COVEREX_MAX_HEIGHT.md)：定义COVEREX矩形区域的最大高度。
-   [OT\_RGN\_COVEREX\_MIN\_THICK](OT_RGN_COVEREX_MIN_THICK.md)：定义能力集空心COVEREX任意四边形的最小线宽。
-   [OT\_RGN\_COVEREX\_MAX\_THICK](OT_RGN_COVEREX_MAX_THICK.md)：定义能力集空心COVEREX任意四边形的最大线宽。
-   [OT\_RGN\_OVERLAY\_MIN\_X](OT_RGN_OVERLAY_MIN_X.md)：定义OVERLAY区域起始位置X坐标最小值。
-   [OT\_RGN\_OVERLAY\_MIN\_Y](OT_RGN_OVERLAY_MIN_Y.md)：定义OVERLAY区域起始位置Y坐标最小值。
-   [OT\_RGN\_OVERLAY\_MAX\_X](OT_RGN_OVERLAY_MAX_X.md)：定义OVERLAY区域起始位置X坐标最大值。
-   [OT\_RGN\_OVERLAY\_MAX\_Y](OT_RGN_OVERLAY_MAX_Y.md)：定义OVERLAY区域起始位置Y坐标最大值。
-   [OT\_RGN\_OVERLAY\_MAX\_WIDTH](OT_RGN_OVERLAY_MAX_WIDTH.md)：定义OVERLAY区域的最大宽度。
-   [OT\_RGN\_OVERLAY\_MAX\_HEIGHT](OT_RGN_OVERLAY_MAX_HEIGHT.md)：定义OVERLAY区域的最大高度。
-   [OT\_RGN\_OVERLAYEX\_MIN\_X](OT_RGN_OVERLAYEX_MIN_X.md)：定义OVERLAYEX区域起始位置X坐标最小值。
-   [OT\_RGN\_OVERLAYEX\_MIN\_Y](OT_RGN_OVERLAYEX_MIN_Y.md)：定义OVERLAYEX区域起始位置Y坐标最小值。
-   [OT\_RGN\_OVERLAYEX\_MAX\_X](OT_RGN_OVERLAYEX_MAX_X.md)：定义OVERLAYEX区域起始位置X坐标最大值。
-   [OT\_RGN\_OVERLAYEX\_MAX\_Y](OT_RGN_OVERLAYEX_MAX_Y.md)：定义OVERLAYEX区域起始位置Y坐标最大值。
-   [OT\_RGN\_OVERLAYEX\_MAX\_WIDTH](OT_RGN_OVERLAYEX_MAX_WIDTH.md)：定义OVERLAYEX区域的最大宽度。
-   [OT\_RGN\_OVERLAYEX\_MAX\_HEIGHT](OT_RGN_OVERLAYEX_MAX_HEIGHT.md)：定义OVERLAYEX区域的最大高度。
-   [OT\_RGN\_LINE\_MIN\_X](OT_RGN_LINE_MIN_X.md)：定义LINE区域起点和终点位置坐标点X坐标最小值。
-   [OT\_RGN\_LINE\_MIN\_Y](OT_RGN_LINE_MIN_Y.md)：定义LINE区域起点和终点位置坐标点Y坐标最小值。
-   [OT\_RGN\_LINE\_MAX\_X](OT_RGN_LINE_MAX_X.md)：定义LINE区域起点和终点位置坐标点X坐标最大值。
-   [OT\_RGN\_LINE\_MAX\_Y](OT_RGN_LINE_MAX_Y.md)：定义LINE区域起点和终点位置坐标点Y坐标最大值。
-   [OT\_RGN\_LINE\_MIN\_THICK](OT_RGN_LINE_MIN_THICK.md)：定义LINE区域最小宽度。
-   [OT\_RGN\_LINE\_MAX\_THICK](OT_RGN_LINE_MAX_THICK.md)：定义LINE区域最大宽度。
-   [OT\_RGN\_LINE\_POINT\_NUM](OT_RGN_LINE_POINT_NUM.md)：定义LINE区域坐标点数量。
-   [OT\_RGN\_MOSAIC\_MIN\_X](OT_RGN_MOSAIC_MIN_X.md)：定义MOSAIC矩形区域起始位置坐标点X坐标最小值。
-   [OT\_RGN\_MOSAIC\_MIN\_Y](OT_RGN_MOSAIC_MIN_Y.md)：定义MOSAIC矩形区域起始位置坐标点Y坐标最小值。
-   [OT\_RGN\_MOSAIC\_MAX\_X](OT_RGN_MOSAIC_MAX_X.md)：定义MOSAIC矩形区域起始位置坐标点X坐标最大值。
-   [OT\_RGN\_MOSAIC\_MAX\_Y](OT_RGN_MOSAIC_MAX_Y.md)：定义MOSAIC矩形区域起始位置坐标点Y坐标最大值。
-   [OT\_RGN\_MOSAIC\_MIN\_WIDTH](OT_RGN_MOSAIC_MIN_WIDTH.md)：定义MOSAIC矩形区域的最小宽度。
-   [OT\_RGN\_MOSAIC\_MIN\_HEIGHT](OT_RGN_MOSAIC_MIN_HEIGHT.md)：定义MOSAIC矩形区域的最小高度。
-   [OT\_RGN\_MOSAIC\_MAX\_WIDTH](OT_RGN_MOSAIC_MAX_WIDTH.md)：定义MOSAIC矩形区域的最大宽度。
-   [OT\_RGN\_MOSAIC\_MAX\_HEIGHT](OT_RGN_MOSAIC_MAX_HEIGHT.md)：定义MOSAIC矩形区域的最大高度。
-   [OT\_RGN\_MOSAICEX\_MIN\_X](OT_RGN_MOSAICEX_MIN_X.md)：定义MOSAICEX矩形区域起始位置坐标点X坐标最小值。
-   [OT\_RGN\_MOSAICEX\_MIN\_Y](OT_RGN_MOSAICEX_MIN_Y.md)：定义MOSAICEX矩形区域起始位置坐标点Y坐标最小值。
-   [OT\_RGN\_MOSAICEX\_MAX\_X](OT_RGN_MOSAICEX_MAX_X.md)：定义MOSAICEX矩形区域起始位置坐标点X坐标最大值。
-   [OT\_RGN\_MOSAICEX\_MAX\_Y](OT_RGN_MOSAICEX_MAX_Y.md)：定义MOSAICEX矩形区域起始位置坐标点Y坐标最大值。
-   [OT\_RGN\_MOSAICEX\_MIN\_WIDTH](OT_RGN_MOSAICEX_MIN_WIDTH.md)：定义MOSAICEX矩形区域的最小宽度。
-   [OT\_RGN\_MOSAICEX\_MIN\_HEIGHT](OT_RGN_MOSAICEX_MIN_HEIGHT.md)：定义MOSAICEX矩形区域的最小高度。
-   [OT\_RGN\_MOSAICEX\_MAX\_WIDTH](OT_RGN_MOSAICEX_MAX_WIDTH.md)：定义MOSAICEX矩形区域的最大宽度。
-   [OT\_RGN\_MOSAICEX\_MAX\_HEIGHT](OT_RGN_MOSAICEX_MAX_HEIGHT.md)：定义MOSAICEX矩形区域的最大高度。
-   [OT\_RGN\_CORNER\_RECTEX\_MIN\_X](OT_RGN_CORNER_RECTEX_MIN_X.md)：定义CORNER\_RECTEX起始位置坐标点X坐标最小值。
-   [OT\_RGN\_CORNER\_RECTEX\_MIN\_Y](OT_RGN_CORNER_RECTEX_MIN_Y.md)：定义CORNER\_RECTEX起始位置坐标点Y坐标最小值。
-   [OT\_RGN\_CORNER\_RECTEX\_MAX\_X](OT_RGN_CORNER_RECTEX_MAX_X.md)：定义CORNER\_RECTEX起始位置坐标点X坐标最大值。
-   [OT\_RGN\_CORNER\_RECTEX\_MAX\_Y](OT_RGN_CORNER_RECTEX_MAX_Y.md)：定义CORNER\_RECTEX起始位置坐标点Y坐标最大值。
-   [OT\_RGN\_CORNER\_RECTEX\_MAX\_WIDTH](OT_RGN_CORNER_RECTEX_MAX_WIDTH.md)：定义CORNER\_RECTEX的最大宽度。
-   [OT\_RGN\_CORNER\_RECTEX\_MAX\_HEIGHT](OT_RGN_CORNER_RECTEX_MAX_HEIGHT.md)：定义CORNER\_RECTEX的最大高度。
-   [OT\_RGN\_CORNER\_RECTEX\_MIN\_HOR\_LEN](OT_RGN_CORNER_RECTEX_MIN_HOR_LEN.md)：定义CORNER\_RECTEX最小水平线长度。
-   [OT\_RGN\_CORNER\_RECTEX\_MIN\_VER\_LEN](OT_RGN_CORNER_RECTEX_MIN_VER_LEN.md)：定义CORNER\_RECTEX最小垂直线长度。
-   [OT\_RGN\_CORNER\_RECTEX\_MAX\_HOR\_LEN](OT_RGN_CORNER_RECTEX_MAX_HOR_LEN.md)：定义CORNER\_RECTEX的最大水平线长度。
-   [OT\_RGN\_CORNER\_RECTEX\_MAX\_VER\_LEN](OT_RGN_CORNER_RECTEX_MAX_VER_LEN.md)：定义CORNER\_RECTEX的最大垂直线长度。
-   [OT\_RGN\_CORNER\_RECTEX\_MIN\_THICK](OT_RGN_CORNER_RECTEX_MIN_THICK.md)：定义CORNER\_RECTEX的最小线宽。
-   [OT\_RGN\_CORNER\_RECTEX\_MAX\_THICK](OT_RGN_CORNER_RECTEX_MAX_THICK.md)：定义CORNER\_RECTEX的最大线宽。
-   [OT\_RGN\_VENC\_MAX\_OVERLAY\_NUM](OT_RGN_VENC_MAX_OVERLAY_NUM.md)：定义一个VENC通道最多可显示的叠加OVERLAY区域的个数。
-   [OT\_RGN\_PCIV\_MAX\_OVERLAYEX\_NUM](OT_RGN_PCIV_MAX_OVERLAYEX_NUM.md)：定义一个PCIV通道的最大扩展叠加区域的个数。
-   [OT\_RGN\_VPSS\_MAX\_OVERLAY\_NUM](OT_RGN_VPSS_MAX_OVERLAY_NUM.md)：定义一个VPSS组最多可显示的叠加OVERLAY区域的个数。
-   [OT\_RGN\_VPSS\_MAX\_OVERLAYEX\_NUM](OT_RGN_VPSS_MAX_OVERLAYEX_NUM.md)：定义一个VPSS通道最多可显示的叠加OVERLAYEX区域的个数。
-   [OT\_RGN\_VPSS\_MAX\_COVER\_NUM](OT_RGN_VPSS_MAX_COVER_NUM.md)：定义一个VPSS组最大遮挡区域的个数。
-   [OT\_RGN\_VPSS\_MAX\_COVEREX\_NUM](OT_RGN_VPSS_MAX_COVEREX_NUM.md)：定义一个VPSS通道最大叠加COVEREX区域的个数。
-   [OT\_RGN\_VPSS\_MAX\_LINE\_NUM](OT_RGN_VPSS_MAX_LINE_NUM.md)：定义一个VPSS通道最大叠加LINE区域的个数。
-   [OT\_RGN\_VPSS\_MAX\_MOSAIC\_NUM](OT_RGN_VPSS_MAX_MOSAIC_NUM.md)：定义一个VPSS组最大叠加MOSAIC区域的个数。
-   [OT\_RGN\_VPSS\_MAX\_MOSAICEX\_NUM](OT_RGN_VPSS_MAX_MOSAICEX_NUM.md)：定义一个VPSS通道最大叠加MOSAICEX区域的个数。
-   [OT\_RGN\_VPSS\_MAX\_CORNER\_RECTEX\_NUM](OT_RGN_VPSS_MAX_CORNER_RECTEX_NUM.md)：定义一个VPSS通道最大叠加CORNER\_RECTEX区域的个数。
-   [OT\_RGN\_VO\_MAX\_LINE\_NUM](OT_RGN_VO_MAX_LINE_NUM.md)：定义一个VO通道的最大叠加LINE区域的个数。
-   [OT\_RGN\_VO\_MAX\_OVERLAYEX\_NUM](OT_RGN_VO_MAX_OVERLAYEX_NUM.md)：定义一个VO通道的最大叠加OVERLAYEX区域的个数。
-   [OT\_RGN\_VO\_MAX\_COVEREX\_NUM](OT_RGN_VO_MAX_COVEREX_NUM.md)：定义一个VO通道的最大叠加COVEREX区域的个数。
-   [OT\_RGN\_VO\_MAX\_CORNER\_RECTEX\_NUM](OT_RGN_VO_MAX_CORNER_RECTEX_NUM.md)：定义一个VO通道最大叠加CORNER\_RECTEX区域的个数。
-   [OT\_RGN\_VI\_MAX\_COVER\_NUM](OT_RGN_VI_MAX_COVER_NUM.md)：定义一个VI通道的最大叠加COVER区域的个数。
-   [OT\_RGN\_VI\_MAX\_COVEREX\_NUM](OT_RGN_VI_MAX_COVEREX_NUM.md)：定义一个VI通道的最大叠加COVER区域的个数
-   [OT\_RGN\_VI\_MAX\_OVERLAYEX\_NUM](OT_RGN_VI_MAX_OVERLAYEX_NUM.md)：定义一个VI通道最多可显示的叠加OVERLAYEX区域的个数。
-   [OT\_RGN\_AVS\_MAX\_OVERLAYEX\_NUM](OT_RGN_AVS_MAX_OVERLAYEX_NUM.md)：定义一个AVS通道最多可显示的叠加OVERLAYEX区域的个数。
-   [OT\_RGN\_VO\_MAX\_CORNER\_RECTEX\_NUM](OT_RGN_VO_MAX_CORNER_RECTEX_NUM.md)：定义一个VO通道最大叠加CORNER\_RECTEX区域的个数。
-   [OT\_RGN\_MCF\_MAX\_OVERLAYEX\_NUM](OT_RGN_MCF_MAX_OVERLAYEX_NUM.md)：定义一个MCF通道最多可显示的叠加OVERLAYEX区域的个数。
-   [OT\_RGN\_MCF\_MAX\_COVER\_NUM](OT_RGN_MCF_MAX_COVER_NUM.md)：定义一个MCF组最大遮挡区域的个数。
-   [OT\_RGN\_MCF\_MAX\_COVEREX\_NUM](OT_RGN_MCF_MAX_COVEREX_NUM.md)：定义一个MCF通道最大叠加COVEREX区域的个数。
-   [OT\_RGN\_MCF\_MAX\_LINE\_NUM](OT_RGN_MCF_MAX_LINE_NUM.md)：定义一个MCF通道最大叠加LINE区域的个数。
-   [OT\_RGN\_MCF\_MAX\_MOSAIC\_NUM](OT_RGN_MCF_MAX_MOSAIC_NUM.md)：定义一个MCF组的最大叠加MOSAIC区域的个数。
-   [OT\_RGN\_MCF\_MAX\_MOSAICEX\_NUM](OT_RGN_MCF_MAX_MOSAICEX_NUM.md)：定义一个MCF通道最大叠加MOSAICEX区域的个数。
-   [OT\_RGN\_MCF\_MAX\_CORNER\_RECTEX\_NUM](OT_RGN_MCF_MAX_CORNER_RECTEX_NUM.md)：定义一个MCF通道最大叠加CORNER\_RECTEX区域的个数
-   [OT\_RGN\_COORD\_X\_MAX](OT_RGN_COORD_X_MAX.md)：定义COVER 相对x坐标最大值。
-   [OT\_RGN\_COORD\_Y\_MAX](OT_RGN_COORD_Y_MAX.md)：定义COVER 相对Y坐标最大值。
-   [OT\_RGN\_COORD\_WIDTH\_MAX](OT_RGN_COORD_WIDTH_MAX.md)：定义COVER 相对宽度最大值。
-   [OT\_RGN\_COORD\_HEIGHT\_MAX](OT_RGN_COORD_HEIGHT_MAX.md)：定义COVER 相对高度最大值。
-   [ot\_rgn\_handle](ot_rgn_handle.md)：定义区域句柄。
-   [ot\_rgn\_handle\_grp](ot_rgn_handle_grp.md)：定义批处理区域组号。
-   [ot\_rgn\_type](ot_rgn_type.md)：定义区域类型。
-   [ot\_rgn\_overlay\_qp\_info](ot_rgn_overlay_qp_info.md)：定义叠加区域qp属性结构体。
-   [ot\_rgn\_attach\_dst](ot_rgn_attach_dst.md)：定义叠加区域VENC图像的类型。
-   [ot\_rgn\_overlay\_attr](ot_rgn_overlay_attr.md)：定义叠加区域属性结构体。
-   [ot\_rgn\_overlay\_chn\_attr](ot_rgn_overlay_chn_attr.md)：定义叠加区域通道显示属性。
-   [ot\_rgn\_cover\_chn\_attr](ot_rgn_cover_chn_attr.md)：定义遮挡区域的通道显示属性。
-   [ot\_rgn\_coverex\_chn\_attr](ot_rgn_coverex_chn_attr.md)：定义扩展遮挡区域的通道显示属性。
-   [ot\_rgn\_line\_chn\_attr](ot_rgn_line_chn_attr.md)：定义线性遮挡区域的通道显示属性。
-   [ot\_rgn\_overlayex\_attr](ot_rgn_overlayex_attr.md)：定义扩展叠加区域属性结构体。
-   [ot\_rgn\_overlayex\_chn\_attr](ot_rgn_overlayex_chn_attr.md)：定义扩展叠加区域的通道显示属性。
-   [ot\_rgn\_type\_attr](ot_rgn_type_attr.md)：定义区域属性联合体。
-   [ot\_rgn\_type\_chn\_attr](ot_rgn_type_chn_attr.md)：定义区域通道显示属性联合体。
-   [ot\_rgn\_mosaic\_chn\_attr](ot_rgn_mosaic_chn_attr.md)：定义MOSAIC区域属性结构体。
-   [ot\_rgn\_mosaicex\_chn\_attr](ot_rgn_mosaicex_chn_attr.md)：定义MOSAICEX区域属性结构体。
-   [ot\_rgn\_corner\_rect\_chn\_attr](ot_rgn_corner_rect_chn_attr.md)：定义CORNER\_RECTEX区域属性结构体。
-   [ot\_rgn\_chn\_attr](ot_rgn_chn_attr.md)：定义区域通道显示属性结构体。
-   [ot\_rgn\_canvas\_info](ot_rgn_canvas_info.md)：定义区域显示画布属性结构体。
...
<td class="cellrowborder" valign="top" width="47%" headers="mcps1.1.3.1.2 "><p id="p4731mcpsimp"><a name="p4731mcpsimp"></a><a name="p4731mcpsimp"></a>角框区域，仅Hi3403V100支持。</p>
</td>
</tr>
</tbody>
</table>
...
<p id="p4874mcpsimp"><a name="p4874mcpsimp"></a><a name="p4874mcpsimp"></a>宽度：[2, <a href="OT_RGN_OVERLAY_MAX_WIDTH.md">OT_RGN_OVERLAY_MAX_WIDTH</a>]，要求以2对齐。</p>
<p id="p4876mcpsimp"><a name="p4876mcpsimp"></a><a name="p4876mcpsimp"></a>高度：[2, <a href="OT_RGN_OVERLAY_MAX_HEIGHT.md">OT_RGN_OVERLAY_MAX_HEIGHT</a>]，要求以2对齐。</p>
</td>
</tr>
...
<p id="p4884mcpsimp"><a name="p4884mcpsimp"></a><a name="p4884mcpsimp"></a>[1, <a href="OT_RGN_MAX_BUF_NUM.md">OT_RGN_MAX_BUF_NUM</a>]</p>
</td>
</tr>
<tr id="row4886mcpsimp"><td class="cellrowborder" valign="top" width="35%" headers="mcps1.1.3.1.1 "><p id="p4888mcpsimp"><a name="p4888mcpsimp"></a><a name="p4888mcpsimp"></a>clut[<a href="OT_RGN_CLUT_NUM.md">OT_RGN_CLUT_NUM</a>]</p>
</td>
...
<p id="p4892mcpsimp"><a name="p4892mcpsimp"></a><a name="p4892mcpsimp"></a>个数：<a href="OT_RGN_CLUT_NUM.md">OT_RGN_CLUT_NUM</a></p>
...
-   pixel\_format、size在调用[ss\_mpi\_rgn\_attach\_to\_chn](ss_mpi_rgn_attach_to_chn.md)后，[ss\_mpi\_rgn\_detach\_from\_chn](ss_mpi_rgn_detach_from_chn.md)之前为静态变量。
...
<p id="p4939mcpsimp"><a name="p4939mcpsimp"></a><a name="p4939mcpsimp"></a>水平位置X：[0, <a href="OT_RGN_OVERLAY_MAX_X.md">OT_RGN_OVERLAY_MAX_X</a>]，要求以2对齐。</p>
<p id="p4941mcpsimp"><a name="p4941mcpsimp"></a><a name="p4941mcpsimp"></a>垂直位置Y：[0, <a href="OT_RGN_OVERLAY_MAX_Y.md">OT_RGN_OVERLAY_MAX_Y</a>]，要求以2对齐。</p>
...
<p id="p4970mcpsimp"><a name="p4970mcpsimp"></a><a name="p4970mcpsimp"></a>取值范围：参见结构体<a href="ot_rgn_overlay_qp_info.md">ot_rgn_overlay_qp_info</a>。</p>
...
<p id="p4978mcpsimp"><a name="p4978mcpsimp"></a><a name="p4978mcpsimp"></a>取值范围：参见枚举<a href="ot_rgn_attach_dst.md">ot_rgn_attach_dst</a>。</p>
...
<img src="figures/OVERLAY的宽高非16对齐示意图.png" alt="" title="OVERLAY的宽高非16对齐示意图" />
...
<p id="p5033mcpsimp"><a name="p5033mcpsimp"></a><a name="p5033mcpsimp"></a>[<a href="OT_RGN_COVER_MIN_X.md">OT_RGN_COVER_MIN_X</a>, <a href="OT_RGN_COVER_MAX_X.md">OT_RGN_COVER_MAX_X</a>]，要求以2对齐。</p>
<p id="p5036mcpsimp"><a name="p5036mcpsimp"></a><a name="p5036mcpsimp"></a>垂直位置Y：</p>
<p id="p5037mcpsimp"><a name="p5037mcpsimp"></a><a name="p5037mcpsimp"></a>[<a href="OT_RGN_COVER_MIN_Y.md">OT_RGN_COVER_MIN_Y</a>, <a href="OT_RGN_COVER_MAX_Y.md">OT_RGN_COVER_MAX_Y</a>]，要求以2对齐。</p>
<p id="p5040mcpsimp"><a name="p5040mcpsimp"></a><a name="p5040mcpsimp"></a>宽高取值范围：</p>
<p id="p5041mcpsimp"><a name="p5041mcpsimp"></a><a name="p5041mcpsimp"></a>宽度：[2, <a href="OT_RGN_COVER_MAX_WIDTH.md">OT_RGN_COVER_MAX_WIDTH</a>]，要求以2对齐。</p>
<p id="p5043mcpsimp"><a name="p5043mcpsimp"></a><a name="p5043mcpsimp"></a>高度：[2, <a href="OT_RGN_COVER_MAX_HEIGHT.md">OT_RGN_COVER_MAX_HEIGHT</a>]，要求以2对齐。</p>
<p id="p5045mcpsimp"><a name="p5045mcpsimp"></a><a name="p5045mcpsimp"></a>若是相对坐标：</p>
<p id="p5046mcpsimp"><a name="p5046mcpsimp"></a><a name="p5046mcpsimp"></a>水平位置X：[0, <a href="OT_RGN_COORD_X_MAX.md">OT_RGN_COORD_X_MAX</a>]。</p>
<p id="p5048mcpsimp"><a name="p5048mcpsimp"></a><a name="p5048mcpsimp"></a>垂直位置Y：[0, <a href="OT_RGN_COORD_Y_MAX.md">OT_RGN_COORD_Y_MAX</a>]。</p>
<p id="p5050mcpsimp"><a name="p5050mcpsimp"></a><a name="p5050mcpsimp"></a>宽高取值范围：</p>
<p id="p5051mcpsimp"><a name="p5051mcpsimp"></a><a name="p5051mcpsimp"></a>宽度：[1,<a href="OT_RGN_COORD_WIDTH_MAX.md">OT_RGN_COORD_WIDTH_MAX</a>]。</p>
<p id="p5053mcpsimp"><a name="p5053mcpsimp"></a><a name="p5053mcpsimp"></a>高度：[1, <a href="OT_RGN_COORD_HEIGHT_MAX.md">OT_RGN_COORD_HEIGHT_MAX</a>]。</p>
</td>
</tr>
</tbody>
</table>
...
<p id="p5093mcpsimp"><a name="p5093mcpsimp"></a><a name="p5093mcpsimp"></a>[<a href="OT_RGN_COVEREX_MIN_X.md">OT_RGN_COVEREX_MIN_X</a>, <a href="OT_RGN_COVEREX_MAX_X.md">OT_RGN_COVEREX_MAX_X</a>]，要求以2对齐。</p>
<p id="p5096mcpsimp"><a name="p5096mcpsimp"></a><a name="p5096mcpsimp"></a>垂直位置Y：</p>
<p id="p5097mcpsimp"><a name="p5097mcpsimp"></a><a name="p5097mcpsimp"></a>[<a href="OT_RGN_COVEREX_MIN_Y.md">OT_RGN_COVEREX_MIN_Y</a>, <a href="OT_RGN_COVEREX_MAX_Y.md">OT_RGN_COVEREX_MAX_Y</a>]，要求以2对齐。</p>
<p id="p5100mcpsimp"><a name="p5100mcpsimp"></a><a name="p5100mcpsimp"></a>宽高取值范围：</p>
<p id="p5101mcpsimp"><a name="p5101mcpsimp"></a><a name="p5101mcpsimp"></a>宽度：[2, <a href="OT_RGN_COVEREX_MAX_WIDTH.md">OT_RGN_COVEREX_MAX_WIDTH</a>]，要求以2对齐。</p>
<p id="p5103mcpsimp"><a name="p5103mcpsimp"></a><a name="p5103mcpsimp"></a>高度：[2, <a href="OT_RGN_COVEREX_MAX_HEIGHT.md">OT_RGN_COVEREX_MAX_HEIGHT</a>]，要求以2对齐。</p>
<p id="p5105mcpsimp"><a name="p5105mcpsimp"></a><a name="p5105mcpsimp"></a>若是相对坐标：</p>
<p id="p5106mcpsimp"><a name="p5106mcpsimp"></a><a name="p5106mcpsimp"></a>水平位置X：[0, <a href="OT_RGN_COORD_X_MAX.md">OT_RGN_COORD_X_MAX</a>]</p>
<p id="p5108mcpsimp"><a name="p5108mcpsimp"></a><a name="p5108mcpsimp"></a>垂直位置Y：[0, <a href="OT_RGN_COORD_Y_MAX.md">OT_RGN_COORD_Y_MAX</a>]</p>
<p id="p5110mcpsimp"><a name="p5110mcpsimp"></a><a name="p5110mcpsimp"></a>宽高取值范围：</p>
<p id="p5111mcpsimp"><a name="p5111mcpsimp"></a><a name="p5111mcpsimp"></a>宽度：[1, <a href="OT_RGN_COORD_WIDTH_MAX.md">OT_RGN_COORD_WIDTH_MAX</a>]</p>
<p id="p5113mcpsimp"><a name="p5113mcpsimp"></a><a name="p5113mcpsimp"></a>高度：[1, <a href="OT_RGN_COORD_HEIGHT_MAX.md">OT_RGN_COORD_HEIGHT_MAX</a>]</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="53%" headers="mcps1.1.3.1.2 "><p id="p5164mcpsimp"><a name="p5164mcpsimp"></a><a name="p5164mcpsimp"></a>线宽，最小为<a href="OT_RGN_LINE_MIN_THICK.md">OT_RGN_LINE_MIN_THICK</a>，最大为<a href="OT_RGN_LINE_MAX_THICK.md">OT_RGN_LINE_MAX_THICK</a>，实际生效的值（显示效果、PROC信息、LOG信息）是设置值的两倍</p>
</td>
</tr>
...
<tr id="row5172mcpsimp"><td class="cellrowborder" valign="top" width="47%" headers="mcps1.1.3.1.1 "><p id="p5174mcpsimp"><a name="p5174mcpsimp"></a><a name="p5174mcpsimp"></a>points[<a href="OT_RGN_LINE_POINT_NUM.md">OT_RGN_LINE_POINT_NUM</a>]</p>
</td>
...
<p id="p5178mcpsimp"><a name="p5178mcpsimp"></a><a name="p5178mcpsimp"></a>X坐标范围是[<a href="OT_RGN_LINE_MIN_X.md">OT_RGN_LINE_MIN_X</a>，<a href="OT_RGN_LINE_MAX_X.md">OT_RGN_LINE_MAX_X</a>],</p>
<p id="p5181mcpsimp"><a name="p5181mcpsimp"></a><a name="p5181mcpsimp"></a>Y坐标范围是[<a href="OT_RGN_LINE_MIN_Y.md">OT_RGN_LINE_MIN_Y</a>，<a href="OT_RGN_LINE_MAX_Y.md">OT_RGN_LINE_MAX_Y</a>],</p>
</td>
</tr>
</tbody>
</table>
...
[OT\_RGN\_LINE\_POINT\_NUM](#OT_RGN_LINE_POINT_NUM)
...
<p id="p3606mcpsimp"><a name="p3606mcpsimp"></a><a name="p3606mcpsimp"></a>宽度：[2, <a href="OT_RGN_OVERLAYEX_MAX_WIDTH.md">OT_RGN_OVERLAYEX_MAX_WIDTH</a>]，要求以2对齐。</p>
<p id="p3608mcpsimp"><a name="p3608mcpsimp"></a><a name="p3608mcpsimp"></a>高度：[2, <a href="OT_RGN_OVERLAYEX_MAX_HEIGHT.md">OT_RGN_OVERLAYEX_MAX_HEIGHT</a>]，要求以2对齐。</p>
</td>
</tr>
...
<p id="p3615mcpsimp"><a name="p3615mcpsimp"></a><a name="p3615mcpsimp"></a>取值范围：[1, <a href="OT_RGN_MAX_BUF_NUM.md">OT_RGN_MAX_BUF_NUM</a>]</p>
</td>
</tr>
<tr id="row3617mcpsimp"><td class="cellrowborder" valign="top" width="35%" headers="mcps1.1.3.1.1 "><p id="p3619mcpsimp"><a name="p3619mcpsimp"></a><a name="p3619mcpsimp"></a>clut[<a href="OT_RGN_CLUT_NUM.md">OT_RGN_CLUT_NUM</a>]</p>
</td>
...
<p id="p3623mcpsimp"><a name="p3623mcpsimp"></a><a name="p3623mcpsimp"></a>个数：<a href="OT_RGN_CLUT_NUM.md">OT_RGN_CLUT_NUM</a></p>
...
-   pixel\_format、size只在调用[ss\_mpi\_rgn\_attach\_to\_chn](ss_mpi_rgn_attach_to_chn.md)后，[ss\_mpi\_rgn\_detach\_from\_chn](ss_mpi_rgn_detach_from_chn.md)之前为静态变量。
-   OVERLAYEX的宽高与该区域分配的内存大小相关，建议按实际需要设置区域的宽高，以免造成内存浪费。
-   clut\[[OT\_RGN\_CLUT\_NUM](OT_RGN_CLUT_NUM.md)\]，RGB颜色查表属性，用于OVERLAYEX设置RGN颜色值。
...
<p id="p3666mcpsimp"><a name="p3666mcpsimp"></a><a name="p3666mcpsimp"></a>水平位置X：[<a href="OT_RGN_OVERLAYEX_MIN_X.md">OT_RGN_OVERLAYEX_MIN_X</a>, <a href="OT_RGN_OVERLAYEX_MAX_X.md">OT_RGN_OVERLAYEX_MAX_X</a>]，要求以2对齐。</p>
<p id="p3669mcpsimp"><a name="p3669mcpsimp"></a><a name="p3669mcpsimp"></a>垂直位置Y：[<a href="OT_RGN_OVERLAYEX_MIN_Y.md">OT_RGN_OVERLAYEX_MIN_Y</a>, <a href="OT_RGN_OVERLAYEX_MAX_Y.md">OT_RGN_OVERLAYEX_MAX_Y</a>]，要求以2对齐。</p>
...
<td class="cellrowborder" valign="top" width="68%" headers="mcps1.1.3.1.2 "><p id="p3813mcpsimp"><a name="p3813mcpsimp"></a><a name="p3813mcpsimp"></a>扩展的角框显示属性（仅Hi3403V100支持）</p>
</td>
</tr>
</tbody>
</table>
