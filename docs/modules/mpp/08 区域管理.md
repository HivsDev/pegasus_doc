...
-   pixel\_format、size在调用`ss_mpi_rgn_attach_to_chn`后，`ss_mpi_rgn_detach_from_chn`之前为静态变量。
...
-   pixel\_format、size只在调用`ss_mpi_rgn_attach_to_chn`后，`ss_mpi_rgn_detach_from_chn`之前为静态变量。
