...
-   只有在SS528V100/SS625V100/SS524V100/SS522V101采用HDMI（含HDMI同源）接口输出时设备前置分频属性需要进行配置，其他接口输出时前置分频属性配置为1（不分频）；而对于SS626V100/SS928V100的所有接口，前置分频属性必须配置为1（不分频）。——因此，从[图1]可以知道，对于同一个时序，HDMI接口（含HDMI同源）输出与非HDMI接口输出需要分别配置出两套不同的时钟源参数。根据像素时钟频率所在的范围来配置前置分频属性值。由于有前置分频的存在，锁相环配置时钟时需要相应的增加倍数。
