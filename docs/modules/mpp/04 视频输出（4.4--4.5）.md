...
<img src="figures/时序信息结构示意图.png" alt="" title="时序信息结构示意图" />
...
-   支持多接口同源输出，解决方案各设备支持情况见表1。同时使用多个接口类型示意：intf\_type = OT\_VO\_INTF\_BT1120 | OT\_VO\_INTF\_HDMI。
...
-   只有在SS528V100/SS625V100/SS524V100/SS522V101采用HDMI（含HDMI同源）接口输出时设备前置分频属性需要进行配置，其他接口输出时前置分频属性配置为1（不分频）；而对于SS626V100/Hi3403V100的所有接口，前置分频属性必须配置为1（不分频）。——因此，从[图1](#fig82411535184119)可以知道，对于同一个时序，HDMI接口（含HDMI同源）输出与非HDMI接口输出需要分别配置出两套不同的时钟源参数。根据像素时钟频率所在的范围来配置前置分频属性值。由于有前置分频的存在，锁相环配置时钟时需要相应的增加倍数。
