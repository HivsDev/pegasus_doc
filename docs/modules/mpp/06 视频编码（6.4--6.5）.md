...
-   此结构体定义是为了ss\_mpi\_venc\_query\_status接口中使用。编码特征信息的内容与ss\_mpi\_venc\_get\_stream获取到的编码特征信息一致，请参考结构体`ot_venc_stream`。与ss\_mpi\_venc\_get\_stream获取信息不同的是，ss\_mpi\_venc\_query\_status不必读取实际编码出来的码流，可以在不同的进程中使用。
...
> **说明：** 
>-   图像中每16 \* 16的块由2个bit表示，2个bit可以表示0\~3共4个档位，用以表示该块的降码率等级，等级越高降码率相对越多，图像质量损失也相对越多，与设置JPEG ROI属性接口ss\_mpi\_venc\_set\_jpeg\_roi\_attr参数`ot_venc_jpeg_roi_attr`中的level意义相同。
