...
<img src="figures/录音和播放示意图.png" alt="" title="录音和播放示意图" />
...
<img src="figures/I2S-1-2-4-8-16-20路接收.png" alt="" title="I2S-1-2-4-8-16-20路接收" />
...
-   Hi3403V100如[表2](#_Ref47711415)所示。

**表 2**  Hi3403V100 AIO最大支持AI、AO通道数
...
<img src="figures/PCM时序发送示意图.png" alt="" title="PCM时序发送示意图" />

**图 2**  I2S时序多线发送示意图<a name="fig5981314020"></a>  
<img src="figures/I2S时序多线发送示意图.png" alt="" title="I2S时序多线发送示意图" />
...
音频输入和音频输出模块支持对音频数据实施重采样。如果启用AI重采样功能，则在[ss\_mpi\_ai\_get\_frame](ss_mpi_ai_get_frame.md)获取数据返回前，内部将会先执行重采样处理，再返回处理后的数据。如果启用了AO重采样功能，则音频数据在发送给AO之前，内部先执行重采样处理，处理完成后再发送给AO通道进行播放。
...
-   如果AI的数据需要送到AENC进行编码且AI启动了重采样，则重采样后的音频帧长out\_point\_num\_per\_frame必须满足：CPU软件编码时out\_point\_num\_per\_frame小于等于编码通道属性的point\_num\_per\_frame；VOIE编码时out\_point\_num\_per\_frame必须是80、160、240、320、480中的一个值。out\_point\_num\_per\_frame、重采样前的音频帧长in\_point\_num\_per\_frame、重采样前的采样率in\_sample\_rate、重采样后的采样率out\_sample\_rate之间的换算关系为：out\_point\_num\_per\_frame=out\_sample\_rate \* in\_point\_num\_per\_frame / in\_sample\_rate。SS528V100/SS524V100不支持VOIE编码。
    ...
    >-   非系统绑定方式下，用户可以通过[ss\_mpi\_ai\_get\_frame](ss_mpi_ai_get_frame.md)接口获取重采样处理后的AI音频帧，并发送给AENC/AO，以建立AI-AENC或AI-AO的数据传输，此时AI或AO的重采样有效。
    >-   当AI通道模式设为OT\_AI\_CHN\_MODE\_FAST时，启用AI的重采样功能后，建议不要在调用[ss\_mpi\_ai\_get\_frame](ss_mpi_ai_get_frame.md)获取该AI通道的数据后将同一个AI通道绑定到AENC通道，否则会导致重采样处理异常。
    >-   重采样输入采样率配置为64kHz时，仅支持输出采样率为8kHz或16kHz。
    >-   当AI通道模式设为OT\_AI\_CHN\_MODE\_NORMAL时，在启用AI的重采样后，支持同一个AI通道绑定多个AENC通道（重采样输出采样率需一致），也支持在AI绑定AENC通道后再调用[ss\_mpi\_ai\_get\_frame](ss_mpi_ai_get_frame.md)获取同一个AI通道的数据。
    >-   ADEC-AO的数据传输方式无上述限制，当为系统绑定方式时，AO的重采样仍有效。
    >-   接口[ss\_mpi\_ai\_enable\_resample](ss_mpi_ai_enable_resample.md)和[ss\_mpi\_ai\_disable\_resample](ss_mpi_ai_disable_resample.md)使用的是声音质量增强功能中的RES模块。
...
-   AI上行通路的VQE最多支持TalkVQE、HIFIVQE、RecordVQE三种方式，其功能包含回声抵消、语音降噪、自动增益、高通滤波、录音噪声消除、均衡器、动态压缩、参量均衡器、高动态范围九个处理模块，如[图1](#fig10208037175514)所示。AI上行通路的VQE功能在[ss\_mpi\_ai\_get\_frame](ss_mpi_ai_get_frame.md)接口内实现，并返回处理后的数据。
-   AO下行通路的VQE支持TalkVQE方式，其功能包含语音降噪、自动增益、高通滤波、均衡器四个处理模块，如[图2](#fig1292512568568)所示。AO下行通路的VQE功能在[ss\_mpi\_ao\_send\_frame](ss_mpi_ao_send_frame.md)接口内实现。
...
> **须知：** 
>-   AI上行音效处理的相关接口为[ss\_mpi\_ai\_set\_talk\_vqe\_attr](ss_mpi_ai_set_talk_vqe_attr.md)、[ss\_mpi\_ai\_get\_talk\_vqe\_attr](ss_mpi_ai_get_talk_vqe_attr.md)、[ss\_mpi\_ai\_set\_talk\_vqe\_v2\_attr](ss_mpi_ai_set_talk_vqe_v2_attr.md)、[ss\_mpi\_ai\_get\_talk\_vqe\_v2\_attr](ss_mpi_ai_get_talk_vqe_v2_attr.md)、[ss\_mpi\_ai\_set\_record\_vqe\_attr](ss_mpi_ai_set_record_vqe_attr.md)、[ss\_mpi\_ai\_get\_record\_vqe\_attr](ss_mpi_ai_get_record_vqe_attr.md)、[ss\_mpi\_ai\_enable\_vqe](ss_mpi_ai_enable_vqe.md)、[ss\_mpi\_ai\_disable\_vqe](ss_mpi_ai_disable_vqe.md)。其中，[ss\_mpi\_ai\_set\_talk\_vqe\_attr](ss_mpi_ai_set_talk_vqe_attr.md)和[ss\_mpi\_ai\_get\_talk\_vqe\_attr](ss_mpi_ai_get_talk_vqe_attr.md)、[ss\_mpi\_ai\_set\_talk\_vqe\_v2\_attr](ss_mpi_ai_set_talk_vqe_v2_attr.md)和[ss\_mpi\_ai\_get\_talk\_vqe\_v2\_attr](ss_mpi_ai_get_talk_vqe_v2_attr.md)、[ss\_mpi\_ai\_set\_record\_vqe\_attr](ss_mpi_ai_set_record_vqe_attr.md)和[ss\_mpi\_ai\_get\_record\_vqe\_attr](ss_mpi_ai_get_record_vqe_attr.md)是三对接口，它们只能配对调用，例如：如果调用[ss\_mpi\_ai\_set\_talk\_vqe\_attr](ss_mpi_ai_set_talk_vqe_attr.md)设置了参数，则只能使用[ss\_mpi\_ai\_get\_talk\_vqe\_attr](ss_mpi_ai_get_talk_vqe_attr.md)获取设置的参数，调用[ss\_mpi\_ai\_get\_record\_vqe\_attr](ss_mpi_ai_get_record_vqe_attr.md)或[ss\_mpi\_ai\_get\_talk\_vqe\_v2\_attr](ss_mpi_ai_get_talk_vqe_v2_attr.md)获取参数会返回错误。在调用其中一个set接口设置参数后，在调用[ss\_mpi\_ai\_enable\_vqe](ss_mpi_ai_enable_vqe.md)使能vqe之前，可以调用另外一个set接口设置参数；调用[ss\_mpi\_ai\_enable\_vqe](ss_mpi_ai_enable_vqe.md)后，如果想设置参数，则须先调用[ss\_mpi\_ai\_disable\_vqe](ss_mpi_ai_disable_vqe.md)禁止使能vqe，然后再调用其中的一个set接口设置参数。各解决方案对这三对接口的支持情况表，如Vqe、HiFiVqe、TalkVqe和RecordVqe接口支持情况表中分别用TalkVqe、RecordVqe和TalkVqeV2代替这三对接口名字。
>-   AO下行音效处理的相关接口为[ss\_mpi\_ao\_set\_vqe\_attr](ss_mpi_ao_set_vqe_attr.md)、[ss\_mpi\_ao\_get\_vqe\_attr](ss_mpi_ao_get_vqe_attr.md)、[ss\_mpi\_ao\_enable\_vqe](ss_mpi_ao_enable_vqe.md)、[ss\_mpi\_ao\_disable\_vqe](ss_mpi_ao_disable_vqe.md)。
>-   与libvqe\_res.so有关联的接口还有[ss\_mpi\_ai\_enable\_resample](ss_mpi_ai_enable_resample.md)、[ss\_mpi\_ai\_disable\_resample](ss_mpi_ai_disable_resample.md)、[ss\_mpi\_ao\_enable\_resample](ss_mpi_ao_enable_resample.md)、[ss\_mpi\_ao\_disable\_resample](ss_mpi_ao_disable_resample.md)。
>-   libvqe\_res.so仅在不使能重采样，且ai和ao设置的采样率与vqe工作采样率一致的情况下可剪裁。
>-   libvqe\_common.so在不使用AEC、AGC、ANR、EQ功能模块时可剪裁。
>-   libvqe\_record.so仅在[ss\_mpi\_ai\_set\_record\_vqe\_attr](ss_mpi_ai_set_record_vqe_attr.md)设置属性之后使用，该库涵盖了HPF、HDR、RNR、DRC、EQ、AGC等功能，因此调用[ss\_mpi\_ai\_set\_record\_vqe\_attr](ss_mpi_ai_set_record_vqe_attr.md)接口设置vqe属性启动vqe时，在无需重采样的情况下只需要libvqe\_ record.so库\(在需要重采样的情况下还需要libvqe\_res.so及相关库\)，不再需要libvqe\_common.so、libvqe\_hpf.so等其他库。
>-   libvqe\_talkv2.so仅在[ss\_mpi\_ai\_set\_talk\_vqe\_v2\_attr](ss_mpi_ai_set_talk_vqe_v2_attr.md)设置属性之后使用，该库涵盖了PNR、NR、EQ、AGC、DRC、LIMITER、DEREVERB、FMP等功能，因此调用[ss\_mpi\_ai\_set\_talk\_vqe\_v2\_attr](ss_mpi_ai_set_talk_vqe_v2_attr.md)接口设置vqe属性启动vqe时，在无需重采样及降风噪的情况下只需要libvqe\_talkv2.so库\(在需要重采样的情况下还需要libvqe\_res.so及相关库，在需要降风噪的情况下还需要libvqe\_wnr.so及相关库\)，不再需要libvqe\_common.so、libvqe\_eq.so等其他库。
...
<tr id="row11560mcpsimp"><td class="cellrowborder" valign="top" width="22%" headers="mcps1.2.7.1.1 "><p id="p11562mcpsimp"><a name="p11562mcpsimp"></a><a name="p11562mcpsimp"></a>Hi3403V100</p>
</td>
...
<img src="figures/UpVQE处理流程图.png" alt="" title="UpVQE处理流程图" />

DnVQE是负责对AO数据进行处理的调度逻辑。在AO下行通路中，DnVQE的处理流程如[图2](#fig1292512568568)所示：

**图 2**  DnVQE处理流程图<a name="fig1292512568568"></a>  
<img src="figures/DnVQE处理流程图.png" alt="" title="DnVQE处理流程图" />

> **须知：** 
>-   当AI通道模式设为OT\_AI\_CHN\_MODE\_NORMAL时，在AI内部的处理线程中完成VQE处理；当AI通道模式设为OT\_AI\_CHN\_MODE\_FAST时，在[ss\_mpi\_ai\_get\_frame](ss_mpi_ai_get_frame.md)接口内部完成VQE处理。
>-   当AI通道模式设为OT\_AI\_CHN\_MODE\_FAST且AI-AENC/AO的数据传输方式为系统绑定方式时，使能AI的任何VQE功能均不起作用。
>-   当AI-AO的数据传输方式为系统绑定方式时，使能AO的VQE功能不起作用。
>-   当AI通道模式设为OT\_AI\_CHN\_MODE\_FAST且AI-AENC/AO的数据传输方式为非系统绑定方式时，使能AI上行通路的VQE功能后，VQE功能在[ss\_mpi\_ai\_get\_frame](ss_mpi_ai_get_frame.md)接口内部进行处理，用户可以通过该MPI接口获取VQE处理后的AI音频帧，然后发送给AENC/AO，以建立AI-AENC或AI-AO的数据传输。
>-   当AI通道模式设为OT\_AI\_CHN\_MODE\_FAST时，使能AI上行通路的VQE功能后，建议不要在调用[ss\_mpi\_ai\_get\_frame](ss_mpi_ai_get_frame.md)获取该AI通道的数据之后再将同一个AI通道绑定到AENC通道，否则会导致VQE处理异常。
>-   当AI通道模式设为OT\_AI\_CHN\_MODE\_NORMAL时，支持同一个AI通道绑定多个AENC通道，也支持在AI绑定AENC通道后再调用[ss\_mpi\_ai\_get\_frame](ss_mpi_ai_get_frame.md)获取同一个AI通道的数据。
>-   当AI通道模式需要设为OT\_AI\_CHN\_MODE\_FAST及需要使能AI上行通路的VQE功能时，建议AI设备的帧长不要设置为太小的值，否则在调用[ss\_mpi\_ai\_get\_frame](ss_mpi_ai_get_frame.md)接口获取前几帧数据会导致出现VQE处理异常的情况。
...
<img src="figures/回声抵消示意图.png" alt="" title="回声抵消示意图" />
...
-   Hi3403V100的AIO中AIP/AOP与AI、AO关系如[图2](#fig49271258143119)所示。
...
<img src="figures/SS528V100-SS524V100-AIO中AIP-AOP与AI-AO关系示意图.png" alt="" title="SS528V100-SS524V100-AIO中AIP-AOP与AI-AO关系示意图" />

**图 2**  Hi3403V100 AIO中AIP/AOP与AI、AO关系示意图<a name="fig49271258143119"></a>  
<img src="figures/Hi3403V100-AIO中AIP-AOP与AI-AO关系示意图.png" alt="" title="Hi3403V100-AIO中AIP-AOP与AI-AO关系示意图" />

**图 3**  SS626V100 AIO中AIP/AOP与AI、AO关系示意图<a name="fig8364724193214"></a>  
<img src="figures/SS626V100-AIO中AIP-AOP与AI-AO关系示意图.png" alt="" title="SS626V100-AIO中AIP-AOP与AI-AO关系示意图" />
...
<tr id="row10737mcpsimp"><td class="cellrowborder" valign="top" width="44.554455445544555%" headers="mcps1.2.4.1.1 "><p id="p10739mcpsimp"><a name="p10739mcpsimp"></a><a name="p10739mcpsimp"></a>Hi3403V100</p>
</td>
...
<tr id="row15784mcpsimp"><td class="cellrowborder" valign="top" width="21%" headers="mcps1.2.4.1.1 "><p id="p15786mcpsimp"><a name="p15786mcpsimp"></a><a name="p15786mcpsimp"></a>Hi3403V100</p>
</td>
...
SS625V100/Hi3403V100/SS626V100提供一个内置的Audio Codec（其中，SS625V100采用V770，Hi3403V100/SS626V100采用RemixV100），用于对接AIP0/AOP0实现声音的播放和录制。AIP0/AOP0接口可以选择对接内置的Audio Codec或外接的Audio Codec，进行声音的播放及录制。因为内置Audio Codec不能发送同步时钟，所以AIP0/AOP0接口对接内置Codec时只能配置为I<sup>2</sup>S时序的主模式，用户仍需要正确配置AIP0/AOP0和内置Audio Codec对接的时序才可接收或发送音频数据。
...
<a name="ul5729mcpsimp"></a><a name="ul5729mcpsimp"></a><ul id="ul5729mcpsimp"><li><a href="OT_ACODEC_SOFT_RESET_CTRL.md">OT_ACODEC_SOFT_RESET_CTRL</a>：将内置Codec恢复为默认设置</li><li><a href="OT_ACODEC_SET_I2S1_FS.md">OT_ACODEC_SET_I2S1_FS</a>：设置I<sup id="sup5734mcpsimp"><a name="sup5734mcpsimp"></a><a name="sup5734mcpsimp"></a>2</sup>S1接口采样率</li><li><a href="OT_ACODEC_SET_INPUT_VOLUME.md">OT_ACODEC_SET_INPUT_VOLUME</a>：输入总音量控制</li><li><a href="OT_ACODEC_GET_INPUT_VOLUME.md">OT_ACODEC_GET_INPUT_VOLUME</a>：获取输入总音量</li><li><a href="OT_ACODEC_SET_OUTPUT_VOLUME.md">OT_ACODEC_SET_OUTPUT_VOLUME</a>：输出总音量控制</li><li><a href="OT_ACODEC_GET_OUTPUT_VOLUME.md">OT_ACODEC_GET_OUTPUT_VOLUME</a>：获取输出总音量</li><li><a href="OT_ACODEC_SET_MIXER_MIC.md">OT_ACODEC_SET_MIXER_MIC</a>：输入方式选择</li><li><a href="OT_ACODEC_SET_GAIN_MICL.md">OT_ACODEC_SET_GAIN_MICL</a>：左声道输入的模拟增益控制</li><li><a href="OT_ACODEC_SET_GAIN_MICR.md">OT_ACODEC_SET_GAIN_MICR</a>：右声道输入的模拟增益控制</li><li><a href="OT_ACODEC_SET_DACL_VOLUME.md">OT_ACODEC_SET_DACL_VOLUME</a>：左声道输出音量控制</li><li><a href="OT_ACODEC_SET_DACR_VOLUME.md">OT_ACODEC_SET_DACR_VOLUME</a>：右声道输出音量控制</li><li><a href="OT_ACODEC_SET_ADCL_VOLUME.md">OT_ACODEC_SET_ADCL_VOLUME</a>：左声道输入音量控制</li><li><a href="OT_ACODEC_SET_ADCR_VOLUME.md">OT_ACODEC_SET_ADCR_VOLUME</a>：右声道输入音量控制</li><li><a href="OT_ACODEC_SET_MICL_MUTE.md">OT_ACODEC_SET_MICL_MUTE</a>：左声道输入静音控制</li><li><a href="OT_ACODEC_SET_MICR_MUTE.md">OT_ACODEC_SET_MICR_MUTE</a>：右声道输入静音控制</li><li><a href="OT_ACODEC_SET_DACL_MUTE.md">OT_ACODEC_SET_DACL_MUTE</a>：左声道输出静音控制</li><li><a href="OT_ACODEC_SET_DACR_MUTE.md">OT_ACODEC_SET_DACR_MUTE</a>：右声道输出静音控制</li><li><a href="OT_ACODEC_DAC_SOFT_MUTE.md">OT_ACODEC_DAC_SOFT_MUTE</a>：输出软静音控制</li><li><a href="OT_ACODEC_DAC_SOFT_UNMUTE.md">OT_ACODEC_DAC_SOFT_UNMUTE</a>：输出软撤销静音控制</li><li><a href="OT_ACODEC_ENABLE_BOOSTL.md">OT_ACODEC_ENABLE_BOOSTL</a>：左声道boost模拟增益控制</li><li><a href="OT_ACODEC_ENABLE_BOOSTR.md">OT_ACODEC_ENABLE_BOOSTR</a>：右声道boost模拟增益控制</li><li><a href="OT_ACODEC_GET_GAIN_MICL.md">OT_ACODEC_GET_GAIN_MICL</a>：获取模拟左声道输入的增益</li><li><a href="OT_ACODEC_GET_GAIN_MICR.md">OT_ACODEC_GET_GAIN_MICR</a>：获取模拟右声道输入的增益</li><li><a href="OT_ACODEC_GET_DACL_VOLUME.md">OT_ACODEC_GET_DACL_VOLUME</a>：获取左声道输出的音量控制</li><li><a href="OT_ACODEC_GET_DACR_VOLUME.md">OT_ACODEC_GET_DACR_VOLUME</a>：获取右声道输出的音量控制</li><li><a href="OT_ACODEC_GET_ADCL_VOLUME.md">OT_ACODEC_GET_ADCL_VOLUME</a>：获取左声道输入的音量控制</li><li><a href="OT_ACODEC_GET_ADCR_VOLUME.md">OT_ACODEC_GET_ADCR_VOLUME</a>：获取右声道输入的音量控制</li><li><a href="OT_ACODEC_SET_PD_DACL.md">OT_ACODEC_SET_PD_DACL</a>：左声道输出的下电控制</li><li><a href="OT_ACODEC_SET_PD_DACR.md">OT_ACODEC_SET_PD_DACR</a>：右声道输出的下电控制</li><li><a href="OT_ACODEC_SET_PD_ADCL.md">OT_ACODEC_SET_PD_ADCL</a>：左声道输入的下电控制</li><li><a href="OT_ACODEC_SET_PD_ADCR.md">OT_ACODEC_SET_PD_ADCR</a>：右声道输入的下电控制</li><li><a href="OT_ACODEC_SET_PD_LINEINL.md">OT_ACODEC_SET_PD_LINEINL</a>：左声道LINEIN输入的下电控制</li><li><a href="OT_ACODEC_SET_PD_LINEINR.md">OT_ACODEC_SET_PD_LINEINR</a>：右声道LINEIN输入的下电控制</li><li><a href="OT_ACODEC_SELECT_DAC_CLK.md">OT_ACODEC_SELECT_DAC_CLK</a>：设置DAC的时钟沿同沿或反沿</li><li><a href="OT_ACODEC_SELECT_ADC_CLK.md">OT_ACODEC_SELECT_ADC_CLK</a>：设置ADC的时钟沿同沿或反沿</li><li><a href="OT_ACODEC_SELECT_ANA_MCLK.md">OT_ACODEC_SELECT_ANA_MCLK</a>：设置模拟部分和数字部分的时钟沿同沿或反沿</li><li><a href="OT_ACODEC_DACL_SELECT_TRACK.md">OT_ACODEC_DACL_SELECT_TRACK</a>：设置DACL选择左声道或右声道</li><li><a href="OT_ACODEC_DACR_SELECT_TRACK.md">OT_ACODEC_DACR_SELECT_TRACK</a>：设置DACR选择左声道或右声道</li><li><a href="OT_ACODEC_ADCL_SELECT_TRACK.md">OT_ACODEC_ADCL_SELECT_TRACK</a>：设置ADCL选择左声道或右声道</li><li><a href="OT_ACODEC_ADCR_SELECT_TRACK.md">OT_ACODEC_ADCR_SELECT_TRACK</a>：设置ADCR选择左声道或右声道</li><li><a href="OT_ACODEC_SET_DAC_DE_EMPHASIS.md">OT_ACODEC_SET_DAC_DE_EMPHASIS</a>：DAC的去加重滤波控制</li><li><a href="OT_ACODEC_SET_ADC_HP_FILTER.md">OT_ACODEC_SET_ADC_HP_FILTER</a>：ADC的高通滤波控制</li><li><a href="OT_ACODEC_DAC_SOFT_MUTE_RATE.md">OT_ACODEC_DAC_SOFT_MUTE_RATE</a>：DAC软静音速率控制</li><li><a href="OT_ACODEC_SET_I2S1_DATA_WIDTH.md">OT_ACODEC_SET_I2S1_DATA_WIDTH</a>：I<sup id="sup5819mcpsimp"><a name="sup5819mcpsimp"></a><a name="sup5819mcpsimp"></a>2</sup>S1数据接口位宽</li></ul>
</td>
...
<img src="figures/输入音量调节示意图.png" alt="" title="输入音量调节示意图" />
...
<td class="cellrowborder" valign="top" width="83%" headers="mcps1.2.3.1.2 "><a name="ul3586mcpsimp"></a><a name="ul3586mcpsimp"></a><ul id="ul3586mcpsimp"><li><a href="OT_ACODEC_SET_INPUT_VOLUME.md">OT_ACODEC_SET_INPUT_VOLUME</a></li></ul>
<a name="ul3589mcpsimp"></a><a name="ul3589mcpsimp"></a><ul id="ul3589mcpsimp"><li><a href="OT_ACODEC_SET_GAIN_MICL.md">OT_ACODEC_SET_GAIN_MICL</a></li><li><a href="OT_ACODEC_SET_GAIN_MICR.md">OT_ACODEC_SET_GAIN_MICR</a></li></ul>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="83%" headers="mcps1.2.3.1.2 "><a name="ul3598mcpsimp"></a><a name="ul3598mcpsimp"></a><ul id="ul3598mcpsimp"><li><a href="OT_ACODEC_SET_INPUT_VOLUME.md">OT_ACODEC_SET_INPUT_VOLUME</a></li><li><a href="OT_ACODEC_ENABLE_BOOSTL.md">OT_ACODEC_ENABLE_BOOSTL</a></li><li><a href="OT_ACODEC_ENABLE_BOOSTR.md">OT_ACODEC_ENABLE_BOOSTR</a></li></ul>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="83%" headers="mcps1.2.3.1.2 "><a name="ul3609mcpsimp"></a><a name="ul3609mcpsimp"></a><ul id="ul3609mcpsimp"><li><a href="OT_ACODEC_SET_INPUT_VOLUME.md">OT_ACODEC_SET_INPUT_VOLUME</a></li><li><a href="OT_ACODEC_SET_ADCL_VOLUME.md">OT_ACODEC_SET_ADCL_VOLUME</a></li><li><a href="OT_ACODEC_SET_ADCR_VOLUME.md">OT_ACODEC_SET_ADCR_VOLUME</a></li></ul>
</td>
</tr>
</tbody>
</table>
...
-   用户在初始化时，需要先调用ss\_mpi\_sys\_init，再调用音频相关的接口（[ss\_mpi\_audio\_register\_vqe\_mod](ss_mpi_audio_register_vqe_mod.md)接口除外）初始化；
...
-   [ss\_mpi\_audio\_set\_mod\_param](ss_mpi_audio_set_mod_param.md)：设置Audio模块参数属性。
-   [ss\_mpi\_audio\_get\_mod\_param](ss_mpi_audio_get_mod_param.md)：获取Audio模块参数属性。
-   [ss\_mpi\_audio\_register\_vqe\_mod](ss_mpi_audio_register_vqe_mod.md)：注册声音质量增强及重采样模块。
...
-   [ss\_mpi\_ai\_set\_pub\_attr](ss_mpi_ai_set_pub_attr.md)：设置AI设备属性。
-   [ss\_mpi\_ai\_get\_pub\_attr](ss_mpi_ai_get_pub_attr.md)：获取AI设备属性。
-   [ss\_mpi\_ai\_enable](ss_mpi_ai_enable.md)：启用AI设备。
-   [ss\_mpi\_ai\_disable](ss_mpi_ai_disable.md)：禁用AI设备。
-   [ss\_mpi\_ai\_enable\_chn](ss_mpi_ai_enable_chn.md)：启用AI通道。
-   [ss\_mpi\_ai\_disable\_chn](ss_mpi_ai_disable_chn.md)：禁用AI通道。
-   [ss\_mpi\_ai\_get\_frame](ss_mpi_ai_get_frame.md)：获取音频帧。
-   [ss\_mpi\_ai\_release\_frame](ss_mpi_ai_release_frame.md)：释放音频帧。
-   [ss\_mpi\_ai\_set\_chn\_param](ss_mpi_ai_set_chn_param.md)：设置AI通道参数。
-   [ss\_mpi\_ai\_get\_chn\_param](ss_mpi_ai_get_chn_param.md)：获取AI通道参数。
-   [ss\_mpi\_ai\_enable\_resample](ss_mpi_ai_enable_resample.md)：启用AI重采样。
-   [ss\_mpi\_ai\_disable\_resample](ss_mpi_ai_disable_resample.md)：禁用AI重采样。
-   [ss\_mpi\_ai\_set\_talk\_vqe\_attr](ss_mpi_ai_set_talk_vqe_attr.md)：设置AI的声音质量增强功能（Talk）相关属性。
-   [ss\_mpi\_ai\_get\_talk\_vqe\_attr](ss_mpi_ai_get_talk_vqe_attr.md)：获取AI的声音质量增强功能（Talk）相关属性。
-   [ss\_mpi\_ai\_set\_record\_vqe\_attr](ss_mpi_ai_set_record_vqe_attr.md)：设置AI的声音质量增强功能（Record）相关属性。
-   [ss\_mpi\_ai\_get\_record\_vqe\_attr](ss_mpi_ai_get_record_vqe_attr.md)：获取AI的声音质量增强功能（Record）相关属性。
-   [ss\_mpi\_ai\_set\_talk\_vqe\_v2\_attr](ss_mpi_ai_set_talk_vqe_v2_attr.md)：设置AI的声音质量增强功能（Talk V2）相关属性。
-   [ss\_mpi\_ai\_get\_talk\_vqe\_v2\_attr](ss_mpi_ai_get_talk_vqe_v2_attr.md)：获取AI的声音质量增强功能（Talk V2）相关属性。
-   [ss\_mpi\_ai\_enable\_vqe](ss_mpi_ai_enable_vqe.md)：使能AI的声音质量增强功能。
-   [ss\_mpi\_ai\_disable\_vqe](ss_mpi_ai_disable_vqe.md)：禁用AI的声音质量增强功能。
-   [ss\_mpi\_ai\_set\_track\_mode](ss_mpi_ai_set_track_mode.md)：设置声道模式。
-   [ss\_mpi\_ai\_get\_track\_mode](ss_mpi_ai_get_track_mode.md)：获取声道模式。
-   [ss\_mpi\_ai\_get\_fd](ss_mpi_ai_get_fd.md)：获取AI通道对应设备文件句柄。
-   [ss\_mpi\_ai\_clr\_pub\_attr](ss_mpi_ai_clr_pub_attr.md)：清除AI设备属性。
-   [ss\_mpi\_ai\_save\_file](ss_mpi_ai_save_file.md)：开启音频输入保存文件功能。
-   [ss\_mpi\_ai\_query\_file\_status](ss_mpi_ai_query_file_status.md)：查询音频输入通道是否处于存文件的状态。
-   [ss\_mpi\_ai\_enable\_aec\_ref\_frame](ss_mpi_ai_enable_aec_ref_frame.md)：在AEC不打开的情况下也使用户能获取到AEC参考帧。
-   [ss\_mpi\_ai\_disable\_aec\_ref\_frame](ss_mpi_ai_disable_aec_ref_frame.md)：在AEC不打开的情况下禁止获取AEC参考帧。
-   [ss\_mpi\_ai\_set\_chn\_attr](ss_mpi_ai_set_chn_attr.md)：设置AI通道属性。
-   [ss\_mpi\_ai\_get\_chn\_attr](ss_mpi_ai_get_chn_attr.md)：获取AI通道属性。
...
<p id="p16113mcpsimp"><a name="p16113mcpsimp"></a><a name="p16113mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
-   时钟选择
    ...
    -   Hi3403V100的clk\_share对应关系如[表1](#table194181745310)所示。

        **表 1**  Hi3403V100的clk\_share对应关系
...
<p id="p12685mcpsimp"><a name="p12685mcpsimp"></a><a name="p12685mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p12204mcpsimp"><a name="p12204mcpsimp"></a><a name="p12204mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
请参见[ss\_mpi\_ai\_set\_pub\_attr](ss_mpi_ai_set_pub_attr.md)的举例。
...
<p id="p4657mcpsimp"><a name="p4657mcpsimp"></a><a name="p4657mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p9513mcpsimp"><a name="p9513mcpsimp"></a><a name="p9513mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p2120mcpsimp"><a name="p2120mcpsimp"></a><a name="p2120mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p2129mcpsimp"><a name="p2129mcpsimp"></a><a name="p2129mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p14468mcpsimp"><a name="p14468mcpsimp"></a><a name="p14468mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
-   AI模块会缓存音频帧数据，用于用户态获取。缓存的深度通过[ss\_mpi\_ai\_set\_chn\_param](ss_mpi_ai_set_chn_param.md)接口设定，默认为0。
...
<p id="p1295mcpsimp"><a name="p1295mcpsimp"></a><a name="p1295mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p1522mcpsimp"><a name="p1522mcpsimp"></a><a name="p1522mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
-   建议先调用[ss\_mpi\_ai\_get\_chn\_param](ss_mpi_ai_get_chn_param.md)接口获取默认配置，再调用本接口修改配置，以便于后续扩展。
...
<p id="p5524mcpsimp"><a name="p5524mcpsimp"></a><a name="p5524mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p9184mcpsimp"><a name="p9184mcpsimp"></a><a name="p9184mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p12860mcpsimp"><a name="p12860mcpsimp"></a><a name="p12860mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p12869mcpsimp"><a name="p12869mcpsimp"></a><a name="p12869mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p12027mcpsimp"><a name="p12027mcpsimp"></a><a name="p12027mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p12036mcpsimp"><a name="p12036mcpsimp"></a><a name="p12036mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p12045mcpsimp"><a name="p12045mcpsimp"></a><a name="p12045mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p12054mcpsimp"><a name="p12054mcpsimp"></a><a name="p12054mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p6811mcpsimp"><a name="p6811mcpsimp"></a><a name="p6811mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p6820mcpsimp"><a name="p6820mcpsimp"></a><a name="p6820mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p15119mcpsimp"><a name="p15119mcpsimp"></a><a name="p15119mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p15128mcpsimp"><a name="p15128mcpsimp"></a><a name="p15128mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p13545mcpsimp"><a name="p13545mcpsimp"></a><a name="p13545mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p13554mcpsimp"><a name="p13554mcpsimp"></a><a name="p13554mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p13351mcpsimp"><a name="p13351mcpsimp"></a><a name="p13351mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p13361mcpsimp"><a name="p13361mcpsimp"></a><a name="p13361mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p13371mcpsimp"><a name="p13371mcpsimp"></a><a name="p13371mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p13381mcpsimp"><a name="p13381mcpsimp"></a><a name="p13381mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p9106mcpsimp"><a name="p9106mcpsimp"></a><a name="p9106mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p9115mcpsimp"><a name="p9115mcpsimp"></a><a name="p9115mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p10587mcpsimp"><a name="p10587mcpsimp"></a><a name="p10587mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p10596mcpsimp"><a name="p10596mcpsimp"></a><a name="p10596mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p2827mcpsimp"><a name="p2827mcpsimp"></a><a name="p2827mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p2836mcpsimp"><a name="p2836mcpsimp"></a><a name="p2836mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p3797mcpsimp"><a name="p3797mcpsimp"></a><a name="p3797mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p3511mcpsimp"><a name="p3511mcpsimp"></a><a name="p3511mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p14188mcpsimp"><a name="p14188mcpsimp"></a><a name="p14188mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p14197mcpsimp"><a name="p14197mcpsimp"></a><a name="p14197mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p4986mcpsimp"><a name="p4986mcpsimp"></a><a name="p4986mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p14578mcpsimp"><a name="p14578mcpsimp"></a><a name="p14578mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p14587mcpsimp"><a name="p14587mcpsimp"></a><a name="p14587mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p15456mcpsimp"><a name="p15456mcpsimp"></a><a name="p15456mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p15465mcpsimp"><a name="p15465mcpsimp"></a><a name="p15465mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
此接口用于查询音频输入通道是否处于存文件的状态，当用户调用[ss\_mpi\_ai\_save\_file](ss_mpi_ai_save_file.md)存储文件后，可调用此接口查询存储的文件是否达到了指定的大小，如果file\_status的saving为TD\_TRUE，说明还没有达到指定大小，为TD\_FALSE则已经达到指定大小。
...
<p id="p8750mcpsimp"><a name="p8750mcpsimp"></a><a name="p8750mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p8767mcpsimp"><a name="p8767mcpsimp"></a><a name="p8767mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p8776mcpsimp"><a name="p8776mcpsimp"></a><a name="p8776mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
-   此接口仅用于在AEC功能关闭的情况下使能获取AEC参考帧，AEC参考帧通过接口[ss\_mpi\_ai\_get\_frame](ss_mpi_ai_get_frame.md)返回，供用户在上层自己做AEC处理。
...
<p id="p12764mcpsimp"><a name="p12764mcpsimp"></a><a name="p12764mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p12773mcpsimp"><a name="p12773mcpsimp"></a><a name="p12773mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p1953mcpsimp"><a name="p1953mcpsimp"></a><a name="p1953mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
-   建议先调用[ss\_mpi\_ai\_get\_chn\_attr](ss_mpi_ai_get_chn_attr.md)接口获取默认配置，再调用本接口修改配置，以便于后续扩展。
...
<p id="p8244mcpsimp"><a name="p8244mcpsimp"></a><a name="p8244mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
-   [ss\_mpi\_ao\_set\_pub\_attr](ss_mpi_ao_set_pub_attr.md)：设置AO设备属性。
-   [ss\_mpi\_ao\_get\_pub\_attr](ss_mpi_ao_get_pub_attr.md)：获取AO设备属性。
-   [ss\_mpi\_ao\_enable](ss_mpi_ao_enable.md)：启用AO设备。
-   [ss\_mpi\_ao\_disable](ss_mpi_ao_disable.md)：禁用AO设备。
-   [ss\_mpi\_ao\_enable\_chn](ss_mpi_ao_enable_chn.md)：启用AO通道。
-   [ss\_mpi\_ao\_disable\_chn](ss_mpi_ao_disable_chn.md)：禁用AO通道。
-   [ss\_mpi\_ao\_send\_frame](ss_mpi_ao_send_frame.md)：发送AO音频帧
-   [ss\_mpi\_ao\_enable\_resample](ss_mpi_ao_enable_resample.md)：启用AO重采样。
-   [ss\_mpi\_ao\_disable\_resample](ss_mpi_ao_disable_resample.md)：禁用AO重采样。
-   [ss\_mpi\_ao\_pause\_chn](ss_mpi_ao_pause_chn.md)：暂停AO通道。
-   [ss\_mpi\_ao\_resume\_chn](ss_mpi_ao_resume_chn.md)：恢复AO通道。
-   [ss\_mpi\_ao\_clr\_chn\_buf](ss_mpi_ao_clr_chn_buf.md)：清除AO通道中当前的音频数据缓存。
-   [ss\_mpi\_ao\_query\_chn\_stat](ss_mpi_ao_query_chn_stat.md)：查询AO通道中当前的音频数据缓存状态。
-   [ss\_mpi\_ao\_set\_track\_mode](ss_mpi_ao_set_track_mode.md)：设置AO设备声道模式。
-   [ss\_mpi\_ao\_get\_track\_mode](ss_mpi_ao_get_track_mode.md)：获取AO设备声道模式。
-   [ss\_mpi\_ao\_set\_volume](ss_mpi_ao_set_volume.md)：设置AO设备音量大小。
-   [ss\_mpi\_ao\_get\_volume](ss_mpi_ao_get_volume.md)：获取AO设备音量大小。
-   [ss\_mpi\_ao\_set\_mute](ss_mpi_ao_set_mute.md)：设置AO设备静音状态。
-   [ss\_mpi\_ao\_get\_mute](ss_mpi_ao_get_mute.md)：获取AO设备静音状态。
-   [ss\_mpi\_ao\_get\_fd](ss_mpi_ao_get_fd.md)：获取AO通道对应的设备文件句柄。
-   [ss\_mpi\_ao\_clr\_pub\_attr](ss_mpi_ao_clr_pub_attr.md)：清除AO设备属性。
-   [ss\_mpi\_ao\_set\_vqe\_attr](ss_mpi_ao_set_vqe_attr.md)：设置AO的声音质量增强功能相关属性。
-   [ss\_mpi\_ao\_get\_vqe\_attr](ss_mpi_ao_get_vqe_attr.md)：获取AO的声音质量增强功能相关属性。
-   [ss\_mpi\_ao\_enable\_vqe](ss_mpi_ao_enable_vqe.md)：使能AO的声音质量增强功能。
-   [ss\_mpi\_ao\_disable\_vqe](ss_mpi_ao_disable_vqe.md)：禁用AO的声音质量增强功能。
-   [ss\_mpi\_ao\_get\_chn\_delay](ss_mpi_ao_get_chn_delay.md)：获取AO通道中当前的音频延时大小。
-   [ss\_mpi\_ao\_save\_file](ss_mpi_ao_save_file.md)：开启音频输出保存文件功能。
-   [ss\_mpi\_ao\_query\_file\_status](ss_mpi_ao_query_file_status.md)：查询音频输出通道是否处于存文件的状态。
...
<p id="p15906mcpsimp"><a name="p15906mcpsimp"></a><a name="p15906mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p17713mcpsimp"><a name="p17713mcpsimp"></a><a name="p17713mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p985mcpsimp"><a name="p985mcpsimp"></a><a name="p985mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p13623mcpsimp"><a name="p13623mcpsimp"></a><a name="p13623mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p10661mcpsimp"><a name="p10661mcpsimp"></a><a name="p10661mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p3187mcpsimp"><a name="p3187mcpsimp"></a><a name="p3187mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p3196mcpsimp"><a name="p3196mcpsimp"></a><a name="p3196mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p12401mcpsimp"><a name="p12401mcpsimp"></a><a name="p12401mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p14938mcpsimp"><a name="p14938mcpsimp"></a><a name="p14938mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p14370mcpsimp"><a name="p14370mcpsimp"></a><a name="p14370mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p14379mcpsimp"><a name="p14379mcpsimp"></a><a name="p14379mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p6330mcpsimp"><a name="p6330mcpsimp"></a><a name="p6330mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
-   通过[ss\_mpi\_ao\_send\_frame](ss_mpi_ao_send_frame.md)接口向AO通道发送音频帧数据时，不允许调用此接口暂停该AO通道。
...
<p id="p2987mcpsimp"><a name="p2987mcpsimp"></a><a name="p2987mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p5308mcpsimp"><a name="p5308mcpsimp"></a><a name="p5308mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
-   在AO通道成功启用后再调用此接口。
-   为完全清除解码回放通路上所有缓存数据，此接口还应该与[ss\_mpi\_adec\_clr\_chn\_buf](ss_mpi_adec_clr_chn_buf.md)接口配合使用。
...
<p id="p12932mcpsimp"><a name="p12932mcpsimp"></a><a name="p12932mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p3425mcpsimp"><a name="p3425mcpsimp"></a><a name="p3425mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p2710mcpsimp"><a name="p2710mcpsimp"></a><a name="p2710mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p14750mcpsimp"><a name="p14750mcpsimp"></a><a name="p14750mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p3357mcpsimp"><a name="p3357mcpsimp"></a><a name="p3357mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p5062mcpsimp"><a name="p5062mcpsimp"></a><a name="p5062mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p16689mcpsimp"><a name="p16689mcpsimp"></a><a name="p16689mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p17396mcpsimp"><a name="p17396mcpsimp"></a><a name="p17396mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p17405mcpsimp"><a name="p17405mcpsimp"></a><a name="p17405mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p12268mcpsimp"><a name="p12268mcpsimp"></a><a name="p12268mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p1871mcpsimp"><a name="p1871mcpsimp"></a><a name="p1871mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p1880mcpsimp"><a name="p1880mcpsimp"></a><a name="p1880mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p16610mcpsimp"><a name="p16610mcpsimp"></a><a name="p16610mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p16619mcpsimp"><a name="p16619mcpsimp"></a><a name="p16619mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p585mcpsimp"><a name="p585mcpsimp"></a><a name="p585mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p594mcpsimp"><a name="p594mcpsimp"></a><a name="p594mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p4857mcpsimp"><a name="p4857mcpsimp"></a><a name="p4857mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p4866mcpsimp"><a name="p4866mcpsimp"></a><a name="p4866mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p13818mcpsimp"><a name="p13818mcpsimp"></a><a name="p13818mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p9285mcpsimp"><a name="p9285mcpsimp"></a><a name="p9285mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p9294mcpsimp"><a name="p9294mcpsimp"></a><a name="p9294mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
<p id="p6708mcpsimp"><a name="p6708mcpsimp"></a><a name="p6708mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备号范围.md#_Ref321753705">表1</a>。</p>
</td>
...
<p id="p6717mcpsimp"><a name="p6717mcpsimp"></a><a name="p6717mcpsimp"></a>取值范围：请参见<a href="音频的输入输出设备的通道数范围.md#_Ref504668216">表1</a>。</p>
</td>
...
-   此接口用于查询音频输出通道是否处于存文件的状态，当用户调用[ss\_mpi\_ao\_save\_file](ss_mpi_ao_save_file.md)存储文件后，可调用此接口查询存储的文件是否达到了指定的大小。
...
-   [ss\_mpi\_aenc\_create\_chn](ss_mpi_aenc_create_chn.md)：创建音频编码通道。
-   [ss\_mpi\_aenc\_destroy\_chn](ss_mpi_aenc_destroy_chn.md)：销毁音频编码通道。
-   [ss\_mpi\_aenc\_send\_frame](ss_mpi_aenc_send_frame.md)：发送音频编码音频帧
-   [ss\_mpi\_aenc\_get\_stream](ss_mpi_aenc_get_stream.md)：获取音频编码码流。
-   [ss\_mpi\_aenc\_release\_stream](ss_mpi_aenc_release_stream.md)：释放音频编码码流。
-   [ss\_mpi\_aenc\_get\_fd](ss_mpi_aenc_get_fd.md)：获取音频编码通道号对应的设备文件句柄.
...
-   [ss\_mpi\_aenc\_get\_stream\_buf\_info](ss_mpi_aenc_get_stream_buf_info.md)：获取音频码流buffer相关信息。
-   [ss\_mpi\_aenc\_set\_mute](ss_mpi_aenc_set_mute.md)：设置音频编码通道静音状态。
-   [ss\_mpi\_aenc\_get\_mute](ss_mpi_aenc_get_mute.md)：获取音频编码通道静音状态。
-   [ss\_mpi\_aenc\_clr\_chn\_buf](ss_mpi_aenc_clr_chn_buf.md)：清除音频编码通道中当前的音频数据缓存。
...
-   协议类型指定该通道的编码协议，目前支持G711、G726、ADPCM，具体内容如[表1](音频编解码协议.md#_Ref289967849)所示。
-   [表1](音频编解码协议.md#_Ref289967849)中列举的编解码协议只支持16bit线性PCM音频数据处理，如果输入的是8bit采样精度的数据，AENC内部会将其扩展为16bit；建议配置AI设备公共属性时将扩展标志置为1，使得AI数据由8bit自动扩展到16bit。
-   语音帧结构如[表1](语音帧结构.md#_Ref224978456)所示。
...
请参见[ss\_mpi\_aenc\_create\_chn](ss_mpi_aenc_create_chn.md)的举例。
...
-   [ss\_mpi\_adec\_create\_chn](ss_mpi_adec_create_chn.md)：创建音频解码通道。
-   [ss\_mpi\_adec\_destroy\_chn](ss_mpi_adec_destroy_chn.md)：销毁音频解码通道。
-   [ss\_mpi\_adec\_send\_stream](ss_mpi_adec_send_stream.md)：发送音频码流到音频解码通道。
-   [ss\_mpi\_adec\_clr\_chn\_buf](ss_mpi_adec_clr_chn_buf.md)：清除ADEC通道中当前的音频数据缓存。
...
-   [ss\_mpi\_adec\_get\_frame](ss_mpi_adec_get_frame.md)：获取音频解码帧数据。
-   [ss\_mpi\_adec\_release\_frame](ss_mpi_adec_release_frame.md)：释放音频解码帧数据。
-   [ss\_mpi\_adec\_send\_end\_of\_stream](ss_mpi_adec_send_end_of_stream.md)：向解码器发送码流结束标识符，并清除码流buffer。
-   [ss\_mpi\_adec\_query\_chn\_status](ss_mpi_adec_query_chn_status.md)：查询ADEC通道中当前的音频数据缓存状态。
-   [ss\_mpi\_adec\_get\_frame\_info](ss_mpi_adec_get_frame_info.md)：获取音频解码帧信息。
-   [ss\_mpi\_adec\_set\_chn\_param](ss_mpi_adec_set_chn_param.md)：设置音频解码通道参数。
-   [ss\_mpi\_adec\_get\_chn\_param](ss_mpi_adec_get_chn_param.md)：获取音频解码通道参数。
...
请参见[ss\_mpi\_adec\_create\_chn](ss_mpi_adec_create_chn.md)的举例。
...
-   必须在ADEC通道创建之后调用。
-   本接口必须与接口[ss\_mpi\_adec\_get\_frame](ss_mpi_adec_get_frame.md)配合使用。
...
-   在ADEC通道成功启用后再调用此接口。
-   查询解码码流结束状态时，此接口还应该与[ss\_mpi\_adec\_send\_end\_of\_stream](ss_mpi_adec_send_end_of_stream.md)接口配合使用，否则查询到的码流结束状态可能会不正确。
...
-   [OT\_ACODEC\_SOFT\_RESET\_CTRL](OT_ACODEC_SOFT_RESET_CTRL.md)：将Codec恢复为默认设置。
-   [OT\_ACODEC\_SET\_I2S1\_FS](OT_ACODEC_SET_I2S1_FS.md)：设置I<sup>2</sup>S1接口采样率。
-   [OT\_ACODEC\_SET\_INPUT\_VOLUME](OT_ACODEC_SET_INPUT_VOLUME.md)：输入总音量控制。
-   [OT\_ACODEC\_GET\_INPUT\_VOLUME](OT_ACODEC_GET_INPUT_VOLUME.md)：获取输入总音量。
-   [OT\_ACODEC\_SET\_OUTPUT\_VOLUME](OT_ACODEC_SET_OUTPUT_VOLUME.md)：输出总音量控制。
-   [OT\_ACODEC\_GET\_OUTPUT\_VOLUME](OT_ACODEC_GET_OUTPUT_VOLUME.md)：获取输出总音量。
-   [OT\_ACODEC\_SET\_MIXER\_MIC](OT_ACODEC_SET_MIXER_MIC.md)：输入声道选择。
-   [OT\_ACODEC\_SET\_GAIN\_MICL](OT_ACODEC_SET_GAIN_MICL.md)：左声道输入的模拟增益控制。
-   [OT\_ACODEC\_SET\_GAIN\_MICR](OT_ACODEC_SET_GAIN_MICR.md)：右声道输入的模拟增益控制。
-   [OT\_ACODEC\_SET\_DACL\_VOLUME](OT_ACODEC_SET_DACL_VOLUME.md)：左声道输出音量控制。
-   [OT\_ACODEC\_SET\_DACR\_VOLUME](OT_ACODEC_SET_DACR_VOLUME.md)：右声道输出音量控制。
-   [OT\_ACODEC\_SET\_ADCL\_VOLUME](OT_ACODEC_SET_ADCL_VOLUME.md)：左声道输入音量控制。
-   [OT\_ACODEC\_SET\_ADCR\_VOLUME](OT_ACODEC_SET_ADCR_VOLUME.md)：右声道输入音量控制。
-   [OT\_ACODEC\_SET\_MICL\_MUTE](OT_ACODEC_SET_MICL_MUTE.md)：左声道输入静音控制。
-   [OT\_ACODEC\_SET\_MICR\_MUTE](OT_ACODEC_SET_MICR_MUTE.md)：右声道输入静音控制。
-   [OT\_ACODEC\_SET\_DACL\_MUTE](OT_ACODEC_SET_DACL_MUTE.md)：左声道输出静音控制。
-   [OT\_ACODEC\_SET\_DACR\_MUTE](OT_ACODEC_SET_DACR_MUTE.md)：右声道输出静音控制。
-   [OT\_ACODEC\_DAC\_SOFT\_MUTE](OT_ACODEC_DAC_SOFT_MUTE.md)：输出软静音控制。
-   [OT\_ACODEC\_DAC\_SOFT\_UNMUTE](OT_ACODEC_DAC_SOFT_UNMUTE.md)：输出软撤销静音控制。
-   [OT\_ACODEC\_ENABLE\_BOOSTL](OT_ACODEC_ENABLE_BOOSTL.md)：左声道boost模拟增益控制。
-   [OT\_ACODEC\_ENABLE\_BOOSTR](OT_ACODEC_ENABLE_BOOSTR.md)：右声道boost模拟增益控制。
-   [OT\_ACODEC\_GET\_GAIN\_MICL](OT_ACODEC_GET_GAIN_MICL.md)：获取模拟左声道输入的增益。
-   [OT\_ACODEC\_GET\_GAIN\_MICR](OT_ACODEC_GET_GAIN_MICR.md)：获取模拟右声道输入的增益。
-   [OT\_ACODEC\_GET\_DACL\_VOLUME](OT_ACODEC_GET_DACL_VOLUME.md)：获取左声道输出的音量控制。
-   [OT\_ACODEC\_GET\_DACR\_VOLUME](OT_ACODEC_GET_DACR_VOLUME.md)：获取右声道输出的音量控制。
-   [OT\_ACODEC\_GET\_ADCL\_VOLUME](OT_ACODEC_GET_ADCL_VOLUME.md)：获取左声道输入的音量控制。
-   [OT\_ACODEC\_GET\_ADCR\_VOLUME](OT_ACODEC_GET_ADCR_VOLUME.md)：获取右声道输入的音量控制。
-   [OT\_ACODEC\_SET\_PD\_DACL](OT_ACODEC_SET_PD_DACL.md)：左声道输出的下电控制。
-   [OT\_ACODEC\_SET\_PD\_DACR](OT_ACODEC_SET_PD_DACR.md)：右声道输出的下电控制。
-   [OT\_ACODEC\_SET\_PD\_ADCL](OT_ACODEC_SET_PD_ADCL.md)：左声道输入的下电控制。
-   [OT\_ACODEC\_SET\_PD\_ADCR](OT_ACODEC_SET_PD_ADCR.md)：右声道输入的下电控制。
-   [OT\_ACODEC\_SET\_PD\_LINEINL](OT_ACODEC_SET_PD_LINEINL.md)：左声道LINEIN输入的下电控制。
-   [OT\_ACODEC\_SET\_PD\_LINEINR](OT_ACODEC_SET_PD_LINEINR.md)：右声道LINEIN输入的下电控制。
...
-   Hi3403V100/SS626V100输入音量范围为\[-78, 86\]，包括模拟增益和数字增益，赋值越大，音量越大。赋值为86时，音量最大，为86dB，赋值为-78时，音量最小，为静音。调节时左右声道一起生效。建议调节范围限制为\[0, 36\]中3的整数倍值，此范围只调节模拟增益，这样引入的噪声最小，能够更好的保证声音质量。
...
该接口须与[OT\_ACODEC\_SET\_INPUT\_VOLUME](OT_ACODEC_SET_INPUT_VOLUME.md)接口搭配使用，不支持获取通过其它音量设置接口配置之后的音量。
...
SS625V100/Hi3403V100/SS626V100的arg取值范围\[1, 2\]，默认选择IN1单端输入。
...
-   Hi3403V100/SS626V100的模拟增益范围为\[0, 12\]，其中0到12按3db递增，0对应0db，12对应最大增益36db。
...
<td class="cellrowborder" valign="top" width="39.57%" headers="mcps1.1.4.1.2 "><p id="p8428mcpsimp"><a name="p8428mcpsimp"></a><a name="p8428mcpsimp"></a><a href="zh-cn_topic_0000002408099670.md">ot_acodec_volume_ctrl</a>结构体指针</p>
</td>
...
<td class="cellrowborder" valign="top" width="40.78000000000001%" headers="mcps1.1.4.1.2 "><p id="p2924mcpsimp"><a name="p2924mcpsimp"></a><a name="p2924mcpsimp"></a><a href="zh-cn_topic_0000002408099670.md">ot_acodec_volume_ctrl</a>结构体指针</p>
</td>
...
-   [OT\_ACODEC\_SELECT\_DAC\_CLK](OT_ACODEC_SELECT_DAC_CLK.md)：设置DAC的时钟沿同沿或反沿。
-   [OT\_ACODEC\_SELECT\_ADC\_CLK](OT_ACODEC_SELECT_ADC_CLK.md)：设置ADC的时钟沿同沿或反沿。
-   [OT\_ACODEC\_SELECT\_ANA\_MCLK](OT_ACODEC_SELECT_ANA_MCLK.md)：设置模拟部分和数字部分的时钟沿同沿或反沿。
-   [OT\_ACODEC\_DACL\_SELECT\_TRACK](OT_ACODEC_DACL_SELECT_TRACK.md)：设置DACL选择左声道或右声道。
-   [OT\_ACODEC\_DACR\_SELECT\_TRACK](OT_ACODEC_DACR_SELECT_TRACK.md)：设置DACR选择左声道或右声道。
-   [OT\_ACODEC\_ADCL\_SELECT\_TRACK](OT_ACODEC_ADCL_SELECT_TRACK.md)：设置ADCL选择左声道或右声道。
-   [OT\_ACODEC\_ADCR\_SELECT\_TRACK](OT_ACODEC_ADCR_SELECT_TRACK.md)：设置ADCR选择左声道或右声道。
-   [OT\_ACODEC\_SET\_DAC\_DE\_EMPHASIS](OT_ACODEC_SET_DAC_DE_EMPHASIS.md)：DAC的去加重滤波控制。
-   [OT\_ACODEC\_SET\_ADC\_HP\_FILTER](OT_ACODEC_SET_ADC_HP_FILTER.md)：ADC的高通滤波控制。
-   [OT\_ACODEC\_DAC\_SOFT\_MUTE\_RATE](OT_ACODEC_DAC_SOFT_MUTE_RATE.md)：DAC软静音速率控制。
-   [OT\_ACODEC\_SET\_I2S1\_DATA\_WIDTH](OT_ACODEC_SET_I2S1_DATA_WIDTH.md)：I2S1数据接口位宽。
...
-   [ss\_resample\_create](ss_resample_create.md)：创建一个重采样模块。
-   [ss\_resample\_process](ss_resample_process.md)：重采样模块数据处理。
-   [ss\_resample\_destroy](ss_resample_destroy.md)：销毁重采样模块。
-   [ss\_resample\_get\_max\_output\_num](ss_resample_get_max_output_num.md)：计算重采样最大输出数据。
...
参考[ss\_resample\_create](#ss_resample_create)
