...
-   Hi3403V100 DEV和时序输入接口的约束关系如表1所示。
...
<p xml:lang="sv-SE" id="p8802mcpsimp"><a name="p8802mcpsimp"></a><a name="p8802mcpsimp"></a><span xml:lang="en-US" id="ph8803mcpsimp"><a name="ph8803mcpsimp"></a><a name="ph8803mcpsimp"></a>取值范围：`ss_mpi_vi_release_chn_frame`  接口释放其内存。
...
<p xml:lang="sv-SE" id="p8913mcpsimp"><a name="p8913mcpsimp"></a><a name="p8913mcpsimp"></a><span xml:lang="en-US" id="ph8914mcpsimp"><a name="ph8914mcpsimp"></a><a name="ph8914mcpsimp"></a>取值范围：`ss_mpi_vi_get_chn_frame`配对使用。
-   用户必须保证frame\_info结构中的信息与获取时一致，否则可能造成释放不成功。
...
<p xml:lang="sv-SE" id="p9368mcpsimp"><a name="p9368mcpsimp"></a><a name="p9368mcpsimp"></a><span xml:lang="en-US" id="ph9369mcpsimp"><a name="ph9369mcpsimp"></a><a name="ph9369mcpsimp"></a>取值范围：`ss_mpi_vi_chn_detach_vb_pool`后，销毁创建的VB之前，需要保证VB没有被VI后端绑定的模块使用，可以通过sleep或清除后端模块通道缓存的方式先把VB都释放，再销毁缓存VB池。
-   需要配合`ss_mpi_vi_chn_set_vb_src`接口使用，可以动态切换。
    -   使用绑定用户VB池的方式，必须设置vb\_src为OT\_VB\_SRC\_USER。
    -   使用公共VB池的方式，必须设置vb\_src为OT\_VB\_SRC\_COMMON。
...
-   `OT_VI_MAX_AD_CHN_NUM`：VI设备属性中ad\_chn\_id的个数。
...
-   对于crop\_ratio参数：
    -   scale为TRUE时，crop\_ratio可以调用接口`ss_mpi_vi_set_chn_dis_cfg`动态修改。
    -   scale为FALSE时，crop\_ratio不可以动态修改，必须先关闭DIS，再动态修改crop\_ratio值。

-   对于参数未有任何修改，但调用`ss_mpi_vi_set_chn_dis_cfg`重复设置时，接口返回成功。
