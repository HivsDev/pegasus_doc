...
-   [前言](前言.md)：介绍文档概述和读者对象。
-   [使用约束](使用约束.md)：介绍SVP ACL的总体使用约束。
-   [新手指引](新手指引.md)：介绍文档结构、接口命名规则。
-   [简介](简介.md)：介绍SVP（Smart Vision Platform）ACL（Advanced Computing Language）的基础知识，包括功能、基本概念、各概念之间的关系、如何查看日志等。
-   [接口调用流程介绍](接口调用流程介绍.md)：介绍各场景下的SVP ACL接口调用流程。
-   [开发流程](开发流程.md)：介绍使用SVP ACL提供的接口开发应用的基本步骤。
-   [准备环境](准备环境.md)：介绍准备开发环境和板端环境时需要参考的文档。
-   [开发首个应用](开发首个应用.md)：以开发图片分类应用（不包含抠图、缩放、解码等数据预处理的开发）为例，按照开发流程，结合示例代码，介绍各步骤的基本原理。
-   [开发典型功能点的介绍](开发典型功能点的介绍.md)：SVP ACL各功能点的详细介绍。
-   [SVP ACL API参考](SVP-ACL-API参考.md)：介绍API的功能、原型、参数等。
-   [SVP ACL样例使用指导](SVP-ACL样例使用指导.md)：介绍如何使用SVP ACL提供的样例。
...
<img src="figures/逻辑架构图.png" alt="" title="逻辑架构图" />
...
<a name="ul3987mcpsimp"></a><a name="ul3987mcpsimp"></a><ul id="ul3987mcpsimp"><li>默认Context：调用<a href="svp_acl_rt_set_device.md">svp_acl_rt_set_device</a>接口指定用于运算的Device时，系统会自动隐式创建一个默认Context，一个Device对应一个默认Context，默认Context不能通过<a href="svp_acl_rt_destroy_context.md">svp_acl_rt_destroy_context</a>接口来释放。</li><li>显式创建的Context：<strong id="b3992mcpsimp"><a name="b3992mcpsimp"></a><a name="b3992mcpsimp"></a>推荐，</strong>在进程或线程中调用<a href="svp_acl_rt_create_context.md">svp_acl_rt_create_context</a>接口显式创建一个Context。</li></ul>
</td>
</tr>
...
<a name="ul4000mcpsimp"></a><a name="ul4000mcpsimp"></a><ul id="ul4000mcpsimp"><li>默认Stream：调用<a href="svp_acl_rt_set_device.md">svp_acl_rt_set_device</a>接口指定用于运算的Device时，系统会自动隐式创建一个默认Stream，一个Device对应一个默认Stream，默认Stream不能通过<a href="svp_acl_rt_destroy_stream.md">svp_acl_rt_destroy_stream</a>接口来释放。</li><li>显式创建的Stream：<strong id="b4005mcpsimp"><a name="b4005mcpsimp"></a><a name="b4005mcpsimp"></a>推荐，</strong>在进程或线程中调用<a href="svp_acl_rt_create_stream.md">svp_acl_rt_create_stream</a>接口显式创建一个Stream。</li></ul>
</td>
</tr>
...
-   可以通过[svp\_acl\_rt\_set\_current\_context](svp_acl_rt_set_current_context.md)进行Device的快速切换。示例代码如下，仅供参考，不可以直接拷贝编译运行：
...
-   进程内创建的多个Context，可以通过[svp\_acl\_rt\_set\_current\_context](svp_acl_rt_set_current_context.md)设置当前需要使用的Context。

<img src="figures/2.png" alt="" />
...
-   默认Context不允许用户执行[svp\_acl\_rt\_get\_current\_context](svp_acl_rt_get_current_context.md)或[svp\_acl\_rt\_set\_current\_context](svp_acl_rt_set_current_context.md)操作，也不允许执行[svp\_acl\_rt\_destroy\_context](svp_acl_rt_destroy_context.md)操作。
...
2.  内存池管理内存，用户一次性申请一块较大内存，并在使用时从这块较大内存中二次分配所需内存。
    ...
    <tbody><tr id="row4595mcpsimp"><td class="cellrowborder" valign="top" width="25%" headers="mcps1.1.4.1.1 "><p id="p4597mcpsimp"><a name="p4597mcpsimp"></a><a name="p4597mcpsimp"></a><a href="svp_acl_rt_malloc.md">svp_acl_rt_malloc</a></p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="55.00000000000001%" headers="mcps1.1.4.1.3 "><a name="ul4603mcpsimp"></a><a name="ul4603mcpsimp"></a><ul id="ul4603mcpsimp"><li>使用<a href="svp_acl_rt_malloc.md">svp_acl_rt_malloc</a>接口申请的内存，需要通过<a href="svp_acl_rt_free.md">svp_acl_rt_free</a>接口释放内存。</li><li>频繁调用<a href="svp_acl_rt_malloc.md">svp_acl_rt_malloc</a>接口申请内存、调用<a href="svp_acl_rt_free.md">svp_acl_rt_free</a>接口释放内存，会损耗性能，建议用户提前做内存预先分配或二次管理，避免频繁申请/释放内存。</li></ul>
    </td>
    </tr>
    <tr id="row4612mcpsimp"><td class="cellrowborder" valign="top" width="25%" headers="mcps1.1.4.1.1 "><p id="p4614mcpsimp"><a name="p4614mcpsimp"></a><a name="p4614mcpsimp"></a><a href="svp_acl_rt_malloc_host.md">svp_acl_rt_malloc_host</a></p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="55.00000000000001%" headers="mcps1.1.4.1.3 "><a name="ul4620mcpsimp"></a><a name="ul4620mcpsimp"></a><ul id="ul4620mcpsimp"><li>使用<a href="svp_acl_rt_malloc_host.md">svp_acl_rt_malloc_host</a>接口申请的内存，需要通过<a href="svp_acl_rt_free_host.md">svp_acl_rt_free_host</a>接口释放内存。</li><li>如果没有Host端，调用该接口会获取Device侧内存，也可调用<a href="svp_acl_rt_free.md">svp_acl_rt_free</a>释放。</li><li>频繁调用<a href="svp_acl_rt_malloc_host.md">svp_acl_rt_malloc_host</a>接口申请内存、调用<a href="svp_acl_rt_free_host.md">svp_acl_rt_free_host</a>接口释放内存，会损耗性能，建议用户提前做内存预先分配或二次管理，避免频繁申请/释放内存。</li></ul>
    </td>
    </tr>
    </tbody>
    </table>
...
<td class="cellrowborder" valign="top" width="37%" headers="mcps1.2.4.1.3 "><p id="p3940mcpsimp"><a name="p3940mcpsimp"></a><a name="p3940mcpsimp"></a><a href="基于Caffe-ResNet-50网络实现图片分类（同步推理）.md">基于Caffe ResNet-50网络实现图片分类（同步推理）</a></p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="37%" headers="mcps1.2.4.1.3 "><p id="p3948mcpsimp"><a name="p3948mcpsimp"></a><a name="p3948mcpsimp"></a><a href="基于Caffe-ResNet-50网络实现图片分类（异步推理）.md">基于Caffe ResNet-50网络实现图片分类（异步推理）</a></p>
</td>
</tr>
</tbody>
</table>
...
<img src="figures/接口调用流程图.png" alt="" title="接口调用流程图" />

上图根据应用开发中的典型功能抽象出主要的接口调用流程，例如，如果需要实现模型推理的功能，则需要先加载模型，模型推理结束后，则需要卸载模型；如果模型推理后，需要从推理结果中查找最大置信度的类别标识对图片分类，则需要数据后处理。

1.  SVP ACL初始化：调用[svp\_acl\_init](svp_acl_init.md)接口实现初始化SVP ACL。
2.  运行管理资源申请：依次申请运行管理资源：[Device](基本概念.md)、[Context](基本概念.md)、[Stream](基本概念.md)。具体流程，请参见“[运行管理资源申请](运行管理资源申请.md)”。
3.  模型推理。
    1.  生成模型om文件：需使用ATC工具将第三方网络（例如，Caffe ResNet-50网络）转换为适配SoC的离线模型（\*.om文件），请参见《ATC工具使用指南》。
    2.  模型加载：模型推理前，需要先将对应的模型加载到系统中。具体流程，请参见“[模型加载](模型加载.md)”。
    3.  模型执行：使用模型实现图片分类、目标识别等功能，目前SVP ACL提供同步推理接口和异步推理接口，支持动态Batch、动态分辨率等场景。具体流程，请参见“[模型执行](模型执行.md)”。
    4.  （可选）数据后处理：处理模型推理的结果，此处根据用户的实际需求来处理推理结果，例如用户可以将获取到的推理结果写入文件、从推理结果中找到每张图片最大置信度的类别标识等。
    5.  模型卸载：调用[svp\_acl\_mdl\_unload](svp_acl_mdl_unload.md)接口卸载模型。

4.  运行管理资源释放：所有数据处理都结束后，需要依次释放运行管理资源：[Stream](基本概念.md)、[Context](基本概念.md)、[Device](基本概念.md)。具体流程，请参见“[运行管理资源释放](运行管理资源释放.md)”。
5.  SVP ACL去初始化：调用[svp\_acl\_finalize](svp_acl_finalize.md)接口实现SVP ACL去初始化。
...
您需要依次申请运行管理资源，包括：[Device](基本概念.md)、[Context](基本概念.md)、[Stream](基本概念.md)。其中创建Context、Stream的方式分为隐式创建和显式创建。
...
<img src="figures/运行管理资源申请流程.png" alt="" title="运行管理资源申请流程" />

1.  申请运行管理资源时，需按顺序依次申请：[Device](基本概念.md)、[Context](基本概念.md)、[Stream](基本概念.md)。
    -   调用[svp\_acl\_rt\_set\_device](svp_acl_rt_set_device.md)接口显式指定用于运算的Device。
        -   调用[svp\_acl\_rt\_create\_context](svp_acl_rt_create_context.md)接口显式创建Context，调用[svp\_acl\_rt\_create\_stream](svp_acl_rt_create_stream.md)接口显式创建Stream。
        -   不显式创建Context和Stream，系统会使用默认Context、默认Stream，该默认Context、默认Stream是在调用[svp\_acl\_rt\_set\_device](svp_acl_rt_set_device.md)接口时隐式创建的。

            默认Stream作为接口入参时，直接传NULL。

    -   不显式指定用于运算的Device。

        调用[svp\_acl\_rt\_create\_context](svp_acl_rt_create_context.md)接口显式创建Context，调用[svp\_acl\_rt\_create\_stream](svp_acl_rt_create_stream.md)接口显式创建Stream。系统在显式创建Context时，系统内部会调用[svp\_acl\_rt\_set\_device](svp_acl_rt_set_device.md)接口指定运行的Device，Device ID通过[svp\_acl\_rt\_create\_context](svp_acl_rt_create_context.md)接口传入。

2.  （可选）调用[svp\_acl\_rt\_get\_run\_mode](svp_acl_rt_get_run_mode.md)接口获取软件栈的运行模式，根据运行模式来判断后续的内存申请接口调用逻辑，如果查询结果为SVP\_ACL\_HOST，则数据传输时涉及申请Host上的内存；如果查询结果为SVP\_ACL\_DEVICE，则数据传输时不涉及申请Host上的内存，仅需申请Device上的内存。

## 模型加载<a name="ZH-CN_TOPIC_0000002408581558"></a>

**图 1**  模型加载流程<a name="fig15136105912515"></a>  
<img src="figures/模型加载流程.png" alt="" title="模型加载流程" />

-   在模型加载前，需使用ATC工具将第三方网络（例如，Caffe ResNet-50网络）转换为适配SoC的离线模型（\*.om文件），请参见《ATC工具使用指南》。
-   支持以下方式加载模型，模型加载成功后，返回标识模型的模型ID：

    [svp\_acl\_mdl\_load\_from\_mem](svp_acl_mdl_load_from_mem.md)：从内存加载离线模型数据，由系统内部管理内存。
...
<img src="figures/基本的模型推理流程.png" alt="" title="基本的模型推理流程" />

模型执行的关键流程说明如下：

1.  调用[svp\_acl\_mdl\_create\_desc](svp_acl_mdl_create_desc.md)接口创建描述模型基本信息的数据类型。
2.  调用[svp\_acl\_mdl\_get\_desc](svp_acl_mdl_get_desc.md)接口根据“[模型加载](模型加载.md)”中返回的模型ID获取模型基本信息。
3.  准备模型执行的输入、输出数据，具体流程，请参见“[准备模型执行的输入/输出数据](准备模型执行的输入-输出数据.md)”。
4.  设置动态Batch的具体流程，请参见“[设置动态Batch和Total\_t](设置动态Batch和Total_t.md)”。
5.  执行模型推理。

    当前系统支持模型的同步推理和异步推理：

    -   同步推理时调用[svp\_acl\_mdl\_execute](svp_acl_mdl_execute.md)接口
    -   异步推理时调用[svp\_acl\_mdl\_execute\_async](svp_acl_mdl_execute_async.md)接口

        对于异步接口，还需调用[svp\_acl\_rt\_synchronize\_stream](svp_acl_rt_synchronize_stream.md)接口阻塞Host运行，直到指定Stream中的所有任务都完成。

        如果同时需要实现Callback功能，请参见“[Callback场景](Callback场景.md)”。
...
7.  释放内存。
    ...
    调用[svp\_acl\_rt\_free](svp_acl_rt_free.md)接口释放Device上的内存。

8.  释放相关数据类型的数据。

    在模型推理结束后，需及时调用[svp\_acl\_destroy\_data\_buffer](svp_acl_destroy_data_buffer.md)接口和[svp\_acl\_mdl\_destroy\_dataset](svp_acl_mdl_destroy_dataset.md)接口释放描述模型输入的数据，且先调用[svp\_acl\_destroy\_data\_buffer](svp_acl_destroy_data_buffer.md)接口，再调用[svp\_acl\_mdl\_destroy\_dataset](svp_acl_mdl_destroy_dataset.md)接口。如果存在多个输入、输出，需调用多次[svp\_acl\_destroy\_data\_buffer](svp_acl_destroy_data_buffer.md)接口。
...
1.  准备模型推理的动态Batch输入的数据，详细流程请参见“[准备模型执行的输入/输出数据](准备模型执行的输入-输出数据.md)”。

    1.  设置动态batch之前需要先调用[svp\_acl\_mdl\_get\_input\_index\_by\_name](svp_acl_mdl_get_input_index_by_name.md)接口根据输入名称获取模型中标识动态Batch输入的index，动态Batch输入的名称固定为SVP\_ACL\_DYNAMIC\_TENSOR\_NAME，然后再调用[svp\_acl\_mdl\_set\_dynamic\_batch\_size](svp_acl_mdl_set_dynamic_batch_size.md)接口根据index设置动态batch。
    2.  申请动态Batch输入对应的内存前，需要先调用[svp\_acl\_mdl\_get\_num\_inputs](svp_acl_mdl_get_num_inputs.md)获取输入个数，然后调用[svp\_acl\_mdl\_get\_input\_size\_by\_index](svp_acl_mdl_get_input_size_by_index.md)接口根据index获取单batch所需内存大小，乘以用户要设置的动态batch值得到需要的内存大小\(task\_buf和work\_buf不需要乘以动态batch值，详细请参见“[准备模型执行的输入/输出数据](准备模型执行的输入-输出数据.md)”\)。
...
2.  在成功加载模型之后，执行模型之前，设置动态Batch数或者设置Total\_t。
    1.  设置动态Batch数
        ...
        调用[svp\_acl\_mdl\_set\_dynamic\_batch\_size](svp_acl_mdl_set_dynamic_batch_size.md)接口设置动态Batch数。

    2.  设置Total\_t

        调用[svp\_acl\_mdl\_set\_total\_t](svp_acl_mdl_set_total_t.md)接口设置Recurrent网络推理的帧数。

### 设置动态分辨率<a name="ZH-CN_TOPIC_0000002442021097"></a>

-   准备模型推理的动态分辨率输入的数据，详细流程请参见“[准备模型执行的输入/输出数据](准备模型执行的输入-输出数据.md)”。

    -   设置动态分辨率之前需要先调用[svp\_acl\_mdl\_get\_input\_index\_by\_name](svp_acl_mdl_get_input_index_by_name.md)接口根据输入名称获取模型中标识动态分辨率输入的index，动态分辨率输入的名称固定为SVP\_ACL\_DYNAMIC\_TENSOR\_NAME。
    -   设置动态分辨率之前，调用[svp\_acl\_mdl\_get\_input\_size\_by\_index](svp_acl_mdl_get_input_size_by_index.md)或者[svp\_acl\_mdl\_get\_output\_size\_by\_index](svp_acl_mdl_get_output_size_by_index.md)接口根据index获取的是默认分辨率\(最大分辨率\)的size。
    -   设置动态分辨率之前，调用[svp\_acl\_mdl\_get\_input\_default\_stride](svp_acl_mdl_get_input_default_stride.md)或者[svp\_acl\_mdl\_get\_output\_default\_stride](svp_acl_mdl_get_output_default_stride.md)接口根据index获取的是默认分辨率\(最大分辨率\)的stride。
    -   设置动态分辨率之前，调用[svp\_acl\_mdl\_get\_input\_dims](svp_acl_mdl_get_input_dims.md)或者[svp\_acl\_mdl\_get\_output\_dims](svp_acl_mdl_get_output_dims.md)接口根据index获取的是默认分辨率\(最大分辨率\)的dims。
...
-   在成功加载模型之后，执行模型之前，设置动态分辨率大小。
    -   调用[svp\_acl\_mdl\_get\_dynamic\_hw](svp_acl_mdl_get_dynamic_hw.md)接口获取支持的动态分辨率个数和大小。
    -   调用[svp\_acl\_mdl\_set\_dynamic\_hw\_size](svp_acl_mdl_set_dynamic_hw_size.md)接口根据index设置动态分辨率大小。

### 准备模型执行的输入/输出数据<a name="ZH-CN_TOPIC_0000002408421658"></a>

**图 1**  模型输入/输出数据<a name="fig11885192314108"></a>  
<img src="figures/模型输入-输出数据.png" alt="" title="模型输入-输出数据" />

准备输入内存时需要额外输入两块内存，分别为task\_buf，work\_buf，对应的输入索引为M-2和M-1（M是通过[svp\_acl\_mdl\_get\_num\_inputs](svp_acl_mdl_get_num_inputs.md)接口获取的输入个数）。在申请动态batch内存时，这两块内存不需要乘以batch值。
...
<img src="figures/模型执行的输入-输出数据准备流程.png" alt="" title="模型执行的输入-输出数据准备流程" />

使用[svp\_acl\_mdl\_desc](svp_acl_mdl_desc.md)类型的数据描述模型基本信息（例如输入/输出的个数、名称、数据类型、Format、维度信息等），使用[svp\_acl\_mdl\_dataset](svp_acl_mdl_dataset.md)类型的数据描述模型的输入/输出数据，模型可能存在多个输入、多个输出，每个输入/输出的内存地址、内存大小用[svp\_acl\_data\_buffer](svp_acl_data_buffer.md)类型的数据来描述。

**图 3** [svp\_acl\_mdl\_dataset](svp_acl_mdl_dataset.md)类型与[svp\_acl\_data\_buffer](svp_acl_data_buffer.md)类型的关系<a name="fig147991655103120"></a>  
<img src="figures/svp_acl_mdl_dataset类型与svp_acl_data_buffer类型的关系.png" alt="" title="svp_acl_mdl_dataset类型与svp_acl_data_buffer类型的关系" />

模型存在多个输入、输出时，用户在向[svp\_acl\_mdl\_dataset](svp_acl_mdl_dataset.md)中添加[svp\_acl\_create\_data\_buffer](svp_acl_create_data_buffer.md)时，为避免顺序出错，可以先获取输入、输出的名称，根据输入、输出名称所对应的index的顺序添加。

> **说明：** 
>在创建[svp\_acl\_data\_buffer](svp_acl_data_buffer.md)类型的数据时，内存大小可以通过调用[svp\_acl\_mdl\_get\_input\_size\_by\_index](svp_acl_mdl_get_input_size_by_index.md)接口根据index获取。

**图 4**  模型输入/输出数据排布（通道为2，batch为2示意图）<a name="fig186181219123517"></a>  
<img src="figures/模型输入-输出数据排布（通道为2-batch为2示意图）.png" alt="" title="模型输入-输出数据排布（通道为2-batch为2示意图）" />
...
<img src="figures/YVU420SP-YUV420SP数据排布（通道为2-frame为2示意图）.png" alt="" title="YVU420SP-YUV420SP数据排布（通道为2-frame为2示意图）" />
...
<img src="figures/检测网阈值输入数据排布.png" alt="" title="检测网阈值输入数据排布" />

**图 7**  检测框结果数据排布（通道为2，chn为2示意图）<a name="fig2039910144816"></a>  
<img src="figures/检测框结果数据排布（通道为2-chn为2示意图）.png" alt="" title="检测框结果数据排布（通道为2-chn为2示意图）" />
...
<img src="figures/同步等待流程多Device场景.png" alt="" title="同步等待流程多Device场景" />

-   在多Device时，利用Context切换（调用[svp\_acl\_rt\_set\_current\_context](svp_acl_rt_set_current_context.md)接口）来切换Device，比使用[svp\_acl\_rt\_set\_device](svp_acl_rt_set_device.md)接口效率高。
-   调用[svp\_acl\_rt\_synchronize\_device](svp_acl_rt_synchronize_device.md)接口等待Device上的计算任务结束。
-   模型加载的流程请参见“[模型加载](模型加载.md)”，模型执行的流程请参见“[模型执行](模型执行.md)”。

### 多Stream场景<a name="ZH-CN_TOPIC_0000002408581866"></a>

**图 1**  同步等待流程多Stream场景<a name="fig151761723115714"></a>  
<img src="figures/同步等待流程多Stream场景.png" alt="" title="同步等待流程多Stream场景" />
...
<img src="figures/同步等待流程Callback场景模型推理.png" alt="" title="同步等待流程Callback场景模型推理" />

Callback流程中关键流程说明如下：

-   回调函数fn需由用户提前创建，用于获取并处理模型推理或算子执行的结果。
-   线程tid2需由用户提前创建，并自定义线程函数，在线程函数内调用[svp\_acl\_rt\_process\_report](svp_acl_rt_process_report.md)接口，等待指定时间后，触发回调函数。
-   调用[svp\_acl\_rt\_subscribe\_report](svp_acl_rt_subscribe_report.md)接口：指定处理Stream上回调函数的线程，线程与tid2保持一致。
-   调用[svp\_acl\_rt\_launch\_callback](svp_acl_rt_launch_callback.md)接口：在Stream的任务队列中增加一个需要在板端环境上执行的回调函数，回调函数与fn保持一致。
-   调用[svp\_acl\_rt\_unsubscribe\_report](svp_acl_rt_unsubscribe_report.md)接口：取消线程注册（Stream上的回调函数不再由指定线程处理）。
-   如果是异步推理Callback场景，为确保Stream中所有任务都完成、模型推理的结果数据都经过Callback函数处理，在Stream销毁前，需要调用一次[svp\_acl\_rt\_synchronize\_device](svp_acl_rt_synchronize_device.md)接口。

## 运行管理资源释放<a name="ZH-CN_TOPIC_0000002441980865"></a>

**图 1**  运行管理资源释放流程<a name="fig19418163914186"></a>  
<img src="figures/运行管理资源释放流程.png" alt="" title="运行管理资源释放流程" />

释放运行管理资源时，需按顺序依次释放：[Stream](基本概念.md)、[Context](基本概念.md)、[Device](基本概念.md)。

-   显式创建Context和Stream时，需调用[svp\_acl\_rt\_destroy\_stream](svp_acl_rt_destroy_stream.md)接口释放Stream，再调用[svp\_acl\_rt\_destroy\_context](svp_acl_rt_destroy_context.md)接口释放Context。若显式调用[svp\_acl\_rt\_set\_device](svp_acl_rt_set_device.md)接口指定运算的Device时，还需调用[svp\_acl\_rt\_reset\_device](svp_acl_rt_reset_device.md)接口释放Device上的资源。
-   不显式创建Context和Stream时，仅需调用[svp\_acl\_rt\_reset\_device](svp_acl_rt_reset_device.md)接口释放Device上的资源。

# 开发流程<a name="ZH-CN_TOPIC_0000002408421570"></a>

**图 1**  开发流程（不包含数据预处理）<a name="fig8148143272210"></a>  
<img src="figures/开发流程（不包含数据预处理）.png" alt="" title="开发流程（不包含数据预处理）" />

1.  准备环境，包括开发环境和板端环境。
2.  开发场景分析。

    根据开发场景分析涉及哪些功能（例如，数据传输、模型推理等）的开发，确定功能后，再明确涉及的命令或接口，请参见“[开发场景分析](开发场景分析.md)”。

3.  创建代码目录。

    在开发应用前，您需要先创建目录，存放代码文件、编译脚本、测试图片数据、模型文件等，请参见“[创建代码目录](创建代码目录.md)”。

4.  开发应用。
    1.  资源初始化，包括SVP ACL初始化、运行管理资源申请、模型推理资源申请等，请参见“[资源初始化](资源初始化.md)”。

        使用SVP ACL接口开发应用时，必须先调用[svp\_acl\_init](svp_acl_init.md)接口进行SVP ACL初始化，否则可能会导致后续系统内部资源初始化出错，进而导致其它业务异常。

    2.  数据传输，请参见“[读入图片数据](读入图片数据.md)”。
    3.  执行模型推理。请参见“[模型推理](模型推理.md)”。
...
5.  编译运行应用，包括模型转换、编译代码、运行应用，请参见“[编译及运行应用](编译及运行应用.md)”。
...
**基本原理**：必须调用[svp\_acl\_init](svp_acl_init.md)接口初始化SVP ACL，配置文件内容为json格式，当前支持以下配置。
...
**示例代码**：可以从[acl\_resnet50](基于Caffe-ResNet-50网络实现图片分类（同步推理）.md)样例中查看完整样例代码。
...
多线程、多Stream的场景请参见[Stream管理](Stream管理.md)。

**基本原理**：需要按顺序依次申请如下资源：[Device](基本概念.md)、[Context](基本概念.md)、[Stream](基本概念.md)，确保可以使用这些资源执行运算、管理任务。

-   关于Context和Stream
    -   显式创建Context时，调用[svp\_acl\_rt\_create\_context](svp_acl_rt_create_context.md)接口，此时需显式调用[svp\_acl\_rt\_destroy\_context](svp_acl_rt_destroy_context.md)接口释放。
    -   显式创建Stream时，调用[svp\_acl\_rt\_create\_stream](svp_acl_rt_create_stream.md)接口，此时需显式调用[svp\_acl\_rt\_destroy\_stream](svp_acl_rt_destroy_stream.md)接口释放。
    -   如果不显式创建Context和Stream，您可以使用[svp\_acl\_rt\_set\_device](svp_acl_rt_set_device.md)接口隐式创建的默认Context和默认Stream，但默认Context和默认Stream存在如下限制：
        -   一个Device对应一个默认Context，默认Context不能通过[svp\_acl\_rt\_destroy\_context](svp_acl_rt_destroy_context.md)接口来释放。
        -   一个Device对应一个默认Stream，默认Stream不能通过[svp\_acl\_rt\_destroy\_stream](svp_acl_rt_destroy_stream.md)接口来释放。默认Stream作为接口入参时，直接传NULL。
...
-   关于模型的输入、输出数据
    ...
    [svp\_acl\_mdl\_dataset](svp_acl_mdl_dataset.md)主要用于描述模型推理时的输入数据/输出数据，模型可能存在多个输入/多个输出，每个输入/输出的内存地址、内存大小用[svp\_acl\_data\_buffer](svp_acl_data_buffer.md)类型的数据来描述。
...
-   根据“[运行管理资源申请（单进程+单线程+单Stream）](运行管理资源申请（单进程+单线程+单Stream）.md)”中[svp\_acl\_rt\_get\_run\_mode](svp_acl_rt_get_run_mode.md)接口的查询结果（SVP\_ACL\_DEVICE，表示软件栈运行在SoC的图像分析引擎上），在自定义函数ReadBinFile内部，直接调用[svp\_acl\_rt\_malloc](svp_acl_rt_malloc.md)申请Device上的内存，再调用C++标准库的read函数将图片数据读入Device的内存中。
...
-   关于模型推理
    ...
    调用[svp\_acl\_mdl\_execute](svp_acl_mdl_execute.md)接口实现同步模型推理，该接口的入参之一是模型ID，在[模型推理资源申请](模型推理资源申请.md)阶段，如果加载模型成功，则会返回标识模型的ID。

-   关于模型输出数据的处理

    [示例代码](示例代码.md)中的代码逻辑是：获取模型推理的输出数据，将输出数据转换为float类型后，再输出其中top5置信度的类别编号。

-   关于资源释放
    -   [svp\_acl\_mdl\_dataset](svp_acl_mdl_dataset.md)主要用于描述模型推理时的输入数据、输出数据，模型可能存在多个输入、多个输出，每个输入/输出的内存地址、内存大小用[svp\_acl\_data\_buffer](svp_acl_data_buffer.md)类型的数据来描述。在模型推理结束后，需及时调用[svp\_acl\_destroy\_data\_buffer](svp_acl_destroy_data_buffer.md)接口和[svp\_acl\_mdl\_destroy\_dataset](svp_acl_mdl_destroy_dataset.md)接口释放描述模型输入的数据，且先调用[svp\_acl\_destroy\_data\_buffer](svp_acl_destroy_data_buffer.md)接口，再调用[svp\_acl\_mdl\_destroy\_dataset](svp_acl_mdl_destroy_dataset.md)接口。如果存在多个输入，需调用多次[svp\_acl\_destroy\_data\_buffer](svp_acl_destroy_data_buffer.md)接口。
    -   在模型推理结束后，还需要通过[svp\_acl\_mdl\_unload](svp_acl_mdl_unload.md)接口卸载模型。


#### 示例代码<a name="ZH-CN_TOPIC_0000002441981121"></a>

您可以从[acl\_resnet50](基于Caffe-ResNet-50网络实现图片分类（同步推理）.md)样例中查看完整样例代码。
...
所有数据处理都结束后，需要释放运行管理资源，包括[Stream](基本概念.md)、[Context](基本概念.md)、[Device](基本概念.md)。释放资源时，需要按顺序释放，先释放Stream，再释放Context，最后再释放Device。

-   如果使用默认Stream，则不能通过[svp\_acl\_rt\_destroy\_stream](svp_acl_rt_destroy_stream.md)接口来释放。
-   如果使用默认Context，则不能通过[svp\_acl\_rt\_destroy\_context](svp_acl_rt_destroy_context.md)接口来释放。
-   默认Context、默认Stream，是在调用[svp\_acl\_rt\_reset\_device](svp_acl_rt_reset_device.md)接口后自动释放。
-   调用[svp\_acl\_finalize](svp_acl_finalize.md)接口实现SVP ACL去初始化。
...
编译运行应用的步骤，请参见[基于Caffe ResNet-50网络实现图片分类（同步推理）](基于Caffe-ResNet-50网络实现图片分类（同步推理）.md)。
...
-   编译代码时，编译脚本的修改点如下。
    ...
    您可以从[acl\_resnet50](基于Caffe-ResNet-50网络实现图片分类（同步推理）.md)样例中获取编译脚本CMakeLists.txt，在该编译脚本的基础上修改如下参数。
...
-   Stream内任务的同步等待：调用[svp\_acl\_rt\_synchronize\_stream](svp_acl_rt_synchronize_stream.md)接口，阻塞应用程序运行，直到指定Stream中的所有任务都完成。
-   Device的同步等待：调用[svp\_acl\_rt\_synchronize\_device](svp_acl_rt_synchronize_device.md)接口，阻塞应用程序运行，直到正在运算中的Device完成运算。多Device场景下，调用该接口等待的是当前Context对应的Device。
...
2.  在模型推理前，需要从离线模型文件（适配SoC的离线模型）中加载模型数据到内存中，并创建[svp\_acl\_mdl\_dataset](svp_acl_mdl_dataset.md)类型的数据描述模型的输出。请参见[模型推理资源申请](模型推理资源申请.md)。
3.  加载模型后，再同步执行模型推理。请参见[模型推理](模型推理.md)。
...
1.  调用[svp\_acl\_init](svp_acl_init.md)接口初始化SVP ACL。
2.  按顺序调用[svp\_acl\_rt\_set\_device](svp_acl_rt_set_device.md)**、**[svp\_acl\_rt\_create\_context](svp_acl_rt_create_context.md)、[svp\_acl\_rt\_create\_stream](svp_acl_rt_create_stream.md)接口依次申请运行管理资源：[Device](基本概念.md)、[Context](基本概念.md)、[Stream](基本概念.md)，确保可以使用这些资源执行运算、管理任务。
...
3.  申请模型推理资源。
    1.  加载模型。
        ...
        [svp\_acl\_mdl\_load\_from\_mem](svp_acl_mdl_load_from_mem.md)：从内存加载离线模型数据。

    2.  <a name="li923473118332"></a>调用[svp\_acl\_mdl\_get\_desc](svp_acl_mdl_get_desc.md)接口获取成功加载的模型的描述信息。
    3.  初始化内存，用于存放模型推理的输入数据、输出数据。

        此处需要用户可自行定义函数实现以下关键点：

        1.  调用[svp\_acl\_mdl\_create\_dataset](svp_acl_mdl_create_dataset.md)接口创建[svp\_acl\_mdl\_dataset](svp_acl_mdl_dataset.md)类型的数据（描述模型的输入/输出）。
        2.  模型可能存在多个输入/输出，调用[svp\_acl\_create\_data\_buffer](svp_acl_create_data_buffer.md)接口创建[svp\_acl\_data\_buffer](svp_acl_data_buffer.md)类型的数据（描述每个输入/输出数据的内存地址、内存大小）。

            模型输入数据的内存大小，根据实际读入的图片数据的大小来确定。

            模型输出数据的内存大小，可调用[svp\_acl\_mdl\_get\_output\_size\_by\_index](svp_acl_mdl_get_output_size_by_index.md)接口根据[3.b](#li923473118332)中模型描述信息获取每个输出需占用的内存大小。

        3.  调用[svp\_acl\_mdl\_add\_dataset\_buffer](svp_acl_mdl_add_dataset_buffer.md)接口向[svp\_acl\_mdl\_dataset](svp_acl_mdl_dataset.md)中增加[svp\_acl\_data\_buffer](svp_acl_data_buffer.md)。

4.  执行模型异步推理和callback（用于处理模型推理的结果）。

    此处需要用户可自行定义函数实现以下关键点：

    1.  创建新线程（例如t1），在线程函数内调用[svp\_acl\_rt\_process\_report](svp_acl_rt_process_report.md)接口，等待指定时间后，触发回调函数（例如CallBackFunc，用于处理模型推理结果）。
    2.  调用[svp\_acl\_rt\_subscribe\_report](svp_acl_rt_subscribe_report.md)接口，指定处理Stream上回调函数（CallBackFunc）的线程（t1），回调函数需要提前创建，用于处理模型推理的结果（例如，将推理结果写入文件、从推理结果中获取topn置信度的类别标识等）。
    3.  调用[svp\_acl\_mdl\_execute\_async](svp_acl_mdl_execute_async.md)接口执行异步模型推理。
    4.  调用[svp\_acl\_rt\_launch\_callback](svp_acl_rt_launch_callback.md)接口，在Stream的任务队列中增加一个需要在板端环境上执行的回调函数（CallBackFunc）。
    5.  调用[svp\_acl\_rt\_synchronize\_stream](svp_acl_rt_synchronize_stream.md)接口，阻塞应用程序运行，直到指定Stream中的所有任务都完成。
    6.  调用[svp\_acl\_rt\_unsubscribe\_report](svp_acl_rt_unsubscribe_report.md)接口，取消线程注册，Stream上的回调函数（CallBackFunc）不再由指定线程（t1）处理。
    7.  模型推理结束后，调用[svp\_acl\_mdl\_unload](svp_acl_mdl_unload.md)接口卸载模型。

5.  所有数据处理结束后，按顺序调用[svp\_acl\_rt\_destroy\_stream](svp_acl_rt_destroy_stream.md)、[svp\_acl\_rt\_destroy\_context](svp_acl_rt_destroy_context.md)、[svp\_acl\_rt\_reset\_device](svp_acl_rt_reset_device.md)接口依次释放运行管理资源，包括[Stream](基本概念.md)、[Context](基本概念.md)、[Device](基本概念.md)。
6.  调用[svp\_acl\_finalize](svp_acl_finalize.md)接口实现SVP ACL去初始化。

    **示例代码**：

    您可以从[acl\_resnet50\_async](基于Caffe-ResNet-50网络实现图片分类（异步推理）.md)样例中查看完整样例代码，以下是关键步骤的代码示例，不可以直接拷贝编译运行，仅供参考。

    在[acl\_resnet50\_async](基于Caffe-ResNet-50网络实现图片分类（异步推理）.md)样例中：
...
多模型推理的基本流程与单模型类似，请参见[单Batch+固定shape+单模型同步推理](单Batch+固定shape+单模型同步推理.md)。
...
-   关于模型推理，如果涉及多个模型，需调用多次模型推理接口。
    ...
    调用[svp\_acl\_mdl\_execute](svp_acl_mdl_execute.md)接口实现同步模型推理。

## 带cache属性的内存管理<a name="ZH-CN_TOPIC_0000002441981237"></a>

**基本原理**：

通过[svp\_acl\_rt\_malloc\_cached](svp_acl_rt_malloc_cached.md)接口申请的内存支持cache缓存，对于频繁使用的内存，建议使用本接口分配内存，这样可以提高cpu的读写效率，提升系统性能，但需要用户处理cpu与硬件设备（如图像分析引擎）之间的数据一致性问题，SVP ACL提供了[svp\_acl\_rt\_mem\_flush](svp_acl_rt_mem_flush.md)接口将cache中的数据刷新到ddr中，从而确保硬件设备（如图像分析引擎）可以从ddr中获取最新数据进行处理，SVP ACL还提供了[svp\_acl\_rt\_mem\_invalidate](svp_acl_rt_mem_invalidate.md)接口将cache中的数据置为无效（如果cache被CPU写操作，调用该接口功能会将数据刷到ddr），从而确保cpu可以获取到ddr中的经图像分析引擎处理后的结果数据。

如果任务（例如，模型加载）不涉及cpu与硬件设备（如图像分析引擎）之间的数据交互，则不涉及两者之间的数据一致性问题，因此就不需要调用[svp\_acl\_rt\_mem\_flush](svp_acl_rt_mem_flush.md)接口和[svp\_acl\_rt\_mem\_invalidate](svp_acl_rt_mem_invalidate.md)接口。

**示例代码**：

调用接口后，需增加异常处理的分支，示例代码中不一一列举。以下是关键步骤的代码示例，不可以直接拷贝编译运行，仅供参考。

示例中，运行管理资源申请与释放请参见[运行管理资源申请](运行管理资源申请.md)和[运行管理资源释放](运行管理资源释放.md)，模型加载的接口调用流程请参见[模型加载](模型加载.md)，模型推理的接口调用流程、准备模型推理的输入/输出数据的接口调用流程请参见[模型执行](模型执行.md)。
...
SoC推理场景下，如果模型中有模式识别 Core不支持的算子，需要通过模式识别CPU算子来实现，这时，异步推理场景下可以调用[svp\_acl\_ext\_process\_aacpu\_task](svp_acl_ext_process_aacpu_task.md)接口来接收模式识别CPU算子并在CPU上执行，同步推理场景下无需用户做特殊处理。下面就异步推理关于该接口的使用方式进行进一步介绍。
...
2.  需要用户在模式识别CPU工作线程内，调用[svp\_acl\_ext\_process\_aacpu\_task](svp_acl_ext_process_aacpu_task.md)接口来接收模式识别CPU算子并执行。
3.  用户场景差异比较大，需要根据自己业务需求来部署模式识别CPU工作线程，例如线程数量，是否绑核。
    ...
    **示例代码**：
    ...
    示例中，异步推理流程请参见[异步推理+callback回调处理](异步推理+callback回调处理.md)  。
...
实现方式详细说明请参考“[功能及约束说明](功能及约束说明.md)”。

示例代码1：

调用接口后，需增加异常处理的分支，并记录报错日志、提示日志，此处不一一列举。以下是关键步骤的代码示例，不可以直接拷贝编译运行，仅供参考。

示例中，运行管理资源申请与释放、模型加载的接口调用流程、模型推理的接口调用流程、准备模型推理的输入/输出数据的接口调用流程请参见[异步推理+callback回调处理](异步推理+callback回调处理.md)。
...
-   一个进程内只能调用一次[svp\_acl\_init](svp_acl_init.md)接口。
-   使用SVP ACL接口开发应用时，必须先调用[svp\_acl\_init](svp_acl_init.md)接口，进行内部资源初始化，否则可能会导致业务异常。
-   进程退出时需要调用[svp\_acl\_finalize](svp_acl_finalize.md)接口释放内部资源。
...
-   如果多次调用svp\_acl\_rt\_set\_device接口而不调用[svp\_acl\_rt\_reset\_device](svp_acl_rt_reset_device.md)接口释放本进程使用的Device资源，功能上不会有问题，因为在进程退出时也会释放本进程使用的Device资源。建议svp\_acl\_rt\_set\_device接口和[svp\_acl\_rt\_reset\_device](svp_acl_rt_reset_device.md)接口配对使用，在不使用Device上资源时，通过调用[svp\_acl\_rt\_reset\_device](svp_acl_rt_reset_device.md)接口及时释放本进程使用的Device资源。

    支持以下使用场景：

    -   在不同进程或线程中可指定同一个Device用于运算。
    -   在某一进程中指定Device，该进程内的多个线程可共用此Device显式创建Context（[svp\_acl\_rt\_create\_context](svp_acl_rt_create_context.md)接口）。
    -   多Device场景下，可在进程中通过svp\_acl\_rt\_set\_device接口切换到其它Device。但利用Context切换（调用[svp\_acl\_rt\_set\_current\_context](svp_acl_rt_set_current_context.md)接口）来切换Device，比使用svp\_acl\_rt\_set\_device接口效率高。
...
<p id="p2416mcpsimp"><a name="p2416mcpsimp"></a><a name="p2416mcpsimp"></a>用户调用<a href="svp_acl_rt_get_device_count.md">svp_acl_rt_get_device_count</a>接口获取可用的Device数量后，这个Device ID的取值范围：[0, (可用的Device数量-1)]</p>
</td>
</tr>
</tbody>
</table>
...
-   接口调用顺序：调用[svp\_acl\_rt\_destroy\_stream](svp_acl_rt_destroy_stream.md)接口释放显式创建的Stream--\>调用[svp\_acl\_rt\_destroy\_context](svp_acl_rt_destroy_context.md)释放显式创建的Context--\>调用svp\_acl\_rt\_reset\_device接口
...
约束说明：如果没有调用[svp\_acl\_rt\_set\_device](svp_acl_rt_set_device.md)接口显式指定Device或没有调用[svp\_acl\_rt\_create\_context](svp_acl_rt_create_context.md)接口隐式指定Device，则调用本接口时，返回错误。
...
-   若不调用svp\_acl\_rt\_create\_context接口显式创建Context，那系统会使用默认Context，该默认Context是在调用[svp\_acl\_rt\_set\_device](svp_acl_rt_set_device.md)接口时隐式创建的。
    -   隐式创建Context：适合简单、无复杂交互逻辑的应用，但缺点在于，在多线程编程中，执行结果取决于线程调度的顺序。
    -   显式创建Context：**推荐显式**，适合大型、复杂交互逻辑的应用，且便于提高程序的可读性、可维护性。

-   若在某一进程内创建多个Context（Context的数量与Stream相关，Stream数量有限制，请参见[svp\_acl\_rt\_create\_stream](svp_acl_rt_create_stream.md)），当前线程在同一时刻内只能使用其中一个Context，建议通过[svp\_acl\_rt\_set\_current\_context](svp_acl_rt_set_current_context.md)接口明确指定当前线程的Context，增加程序的可维护性。
...
函数功能：销毁一个Context，释放Context的资源，同步接口。只能销毁通过[svp\_acl\_rt\_create\_context](svp_acl_rt_create_context.md)接口创建的Context。
...
-   如果在某线程（例如：thread1）中调用[svp\_acl\_rt\_create\_context](svp_acl_rt_create_context.md)接口显式创建一个Context（例如：ctx1），则可以不调用svp\_acl\_rt\_set\_current\_context接口指定该线程的Context，系统默认将ctx1作为thread1的Context。
-   如果没有调用[svp\_acl\_rt\_create\_context](svp_acl_rt_create_context.md)接口显式创建Context，则系统将默认Context作为线程的Context，此时，不能通过[svp\_acl\_rt\_destroy\_context](svp_acl_rt_destroy_context.md)接口来释放默认Context。
...
-   推荐在某一线程中创建的Context，在该线程中使用。若在线程A中调用[svp\_acl\_rt\_create\_context](svp_acl_rt_create_context.md)接口创建Context，在线程B中使用该Context，则需由用户自行保证两个线程中同一个Context下同一个Stream中任务执行的顺序。
...
-   获取线程的Context，同步接口。
-   如果用户多次调用[svp\_acl\_rt\_set\_current\_context](svp_acl_rt_set_current_context.md)接口设置当前线程的Context，则获取的是最后一次设置的Context。
...
-   每个Context对应一个默认Stream，该默认Stream是调用[svp\_acl\_rt\_set\_device](svp_acl_rt_set_device.md)接口或[svp\_acl\_rt\_create\_context](svp_acl_rt_create_context.md)接口隐式创建的。推荐调用svp\_acl\_rt\_create\_stream接口显式创建Stream。
...
函数功能：销毁指定Stream，只能销毁通过[svp\_acl\_rt\_create\_stream](svp_acl_rt_create_stream.md)接口创建的Stream，同步接口。

约束说明：

-   在调用svp\_acl\_rt\_destroy\_stream接口销毁指定Stream前，需要先调用[svp\_acl\_rt\_synchronize\_stream](svp_acl_rt_synchronize_stream.md)接口确保Stream中的任务都已完成。
...
-   在timeout时间内，触发回调处理，由[svp\_acl\_rt\_subscribe\_report](svp_acl_rt_subscribe_report.md)接口指定的线程处理回调。同步接口。
...
-   通过该接口申请的Device内存不支持cache缓存，如果用户需要申请支持cache缓存的内存，请参见[svp\_acl\_rt\_malloc\_cached](svp_acl_rt_malloc_cached.md)。

约束说明：

-   使用svp\_acl\_rt\_malloc接口申请的内存，需要通过[svp\_acl\_rt\_free](svp_acl_rt_free.md)接口释放内存。
-   频繁调用svp\_acl\_rt\_malloc接口申请内存、调用[svp\_acl\_rt\_free](svp_acl_rt_free.md)接口释放内存，会损耗性能，建议用户提前做内存预先分配或二次管理，避免频繁申请/释放内存。
...
-   在Device上申请size大小的线性内存，通过\*dev\_ptr返回已分配内存的虚拟地址指针。
-   通过本接口分配的内存支持cache缓存，对于频繁使用的内存，建议使用本接口分配内存，这样可以提高cpu的读写效率，提升系统性能。当cpu访问此接口申请的内存时，会将内存中的数据放在cache中，而硬件设备（如图像分析引擎）只能访问物理内存，不能访问cache中内容，对于这种cpu和硬件会共同操作的内存，需要调用[svp\_acl\_rt\_mem\_flush](svp_acl_rt_mem_flush.md)接口做好数据同步。

约束说明：其它约束与[svp\_acl\_rt\_malloc](svp_acl_rt_malloc.md)接口相同，但使用本接口申请的支持cache缓存的内存，需要用户显式处理cpu与图像分析引擎之间cache的一致性。
...
-   此接口应与[svp\_acl\_rt\_malloc\_cached](svp_acl_rt_malloc_cached.md)接口配套使用。
...
-   释放Device上的内存，同步接口。
-   本接口只能释放通过[svp\_acl\_rt\_malloc](svp_acl_rt_malloc.md)接口或[svp\_acl\_rt\_malloc\_cached](svp_acl_rt_malloc_cached.md)接口申请的内存。如果应用在Device侧使用[svp\_acl\_rt\_malloc\_host](svp_acl_rt_malloc_host.md)接口申请内存，也可以通过本接口释放。
...
-   使用svp\_acl\_rt\_malloc\_host接口申请的内存，需要通过[svp\_acl\_rt\_free\_host](svp_acl_rt_free_host.md)接口释放内存。
-   应用在Host上运行时，调用该接口申请的是Host内存，由系统保证内存首地址64字节对齐。应用在Device上运行时，调用该接口申请的是Device内存，由系统保证内存首地址64字节对齐。
-   频繁调用svp\_acl\_rt\_malloc\_host接口申请内存、调用[svp\_acl\_rt\_free\_host](svp_acl_rt_free_host.md)接口释放内存，会损耗性能，建议用户提前做内存预先分配或二次管理，避免频繁申请/释放内存。
...
-   应用在Host侧，svp\_acl\_rt\_free\_host接口只能释放通过[svp\_acl\_rt\_malloc\_host](svp_acl_rt_malloc_host.md)接口申请的内存。应用在Device侧，svp\_acl\_rt\_free\_host接口既可以释放[svp\_acl\_rt\_malloc\_host](svp_acl_rt_malloc_host.md)接口申请的内存，也可以释放[svp\_acl\_rt\_malloc](svp_acl_rt_malloc.md)接口申请的内存。
...
-   输入/输出默认stride按照生成.om文件时配置的\[internal\_stride\]的值对齐后不能超过0xFFFFFFFF（详细输入/输出默认stride计算方法请参见[svp\_acl\_mdl\_get\_input\_default\_stride](svp_acl_mdl_get_input_default_stride.md)和[svp\_acl\_mdl\_get\_output\_default\_stride](svp_acl_mdl_get_output_default_stride.md)）。
-   输入/输出按照默认stride计算得到的单batch内存大小不能超过0xFFFFFFFF（详细输入/输出内存计算方法请参见[svp\_acl\_mdl\_get\_input\_size\_by\_index](svp_acl_mdl_get_input_size_by_index.md)和[svp\_acl\_mdl\_get\_output\_size\_by\_index](svp_acl_mdl_get_output_size_by_index.md)）。
...
<td class="cellrowborder" valign="top" width="46%" headers="mcps1.1.4.1.3 "><p id="p1466mcpsimp"><a name="p1466mcpsimp"></a><a name="p1466mcpsimp"></a>模型数据的内存地址。用户要保证内存中存储的模型数据的完整性和正确性，在调用<a href="svp_acl_mdl_unload.md">svp_acl_mdl_unload</a>接口卸载模型之前内存不能被释放且内存中的模型数据不能被修改。</p>
</td>
</tr>
...
-   input中的task\_buf（见[准备模型执行的输入/输出数据](准备模型执行的输入-输出数据.md)）在任务模型推理完成前不能被改写也不能被其他任务使用。
-   input中的work\_buf（见[准备模型执行的输入/输出数据](准备模型执行的输入-输出数据.md)）不能被其他stream上的任务使用。
-   在模型推理完成之前，不能调用[svp\_acl\_mdl\_destroy\_dataset](svp_acl_mdl_destroy_dataset.md)接口释放input或output，不能调用[svp\_acl\_update\_data\_buffer](svp_acl_update_data_buffer.md)接口更新input或者output。
...
<p id="p6218mcpsimp"><a name="p6218mcpsimp"></a><a name="p6218mcpsimp"></a>调用<a href="svp_acl_mdl_load_from_mem.md">svp_acl_mdl_load_from_mem</a>接口加载模型成功后，会返回模型ID。</p>
</td>
</tr>
...
<p id="p6228mcpsimp"><a name="p6228mcpsimp"></a><a name="p6228mcpsimp"></a>若用户使用<a href="svp_acl_rt_malloc.md">svp_acl_rt_malloc</a>或<a href="svp_acl_rt_malloc_host.md">svp_acl_rt_malloc_host</a>接口申请大块内存并自行划分、管理内存时，用户在管理内存时，模型输入数据的内存有对齐和补齐要求，首地址和stride需要16字节对齐。</p>
</td>
</tr>
...
<p id="p6240mcpsimp"><a name="p6240mcpsimp"></a><a name="p6240mcpsimp"></a>若用户使用<a href="svp_acl_rt_malloc.md">svp_acl_rt_malloc</a>接口申请大块内存并自行划分、管理内存时，用户在管理内存时，模型输出数据的内存有对齐和补齐要求，首地址和stride需要16字节对齐。</p>
</td>
</tr>
</tbody>
</table>
...
<p id="p7349mcpsimp"><a name="p7349mcpsimp"></a><a name="p7349mcpsimp"></a>调用<a href="svp_acl_mdl_load_from_mem.md">svp_acl_mdl_load_from_mem</a>接口加载模型成功后，会返回模型ID。</p>
</td>
</tr>
...
<p id="p7359mcpsimp"><a name="p7359mcpsimp"></a><a name="p7359mcpsimp"></a>若用户使用<a href="svp_acl_rt_malloc.md">svp_acl_rt_malloc</a>或<a href="svp_acl_rt_malloc_host.md">svp_acl_rt_malloc_host</a>接口申请大块内存并自行划分、管理内存时，用户在管理内存时，模型输入数据的内存有对齐和补齐要求，首地址和stride需要16字节对齐。</p>
</td>
</tr>
...
<p id="p7371mcpsimp"><a name="p7371mcpsimp"></a><a name="p7371mcpsimp"></a>若用户使用<a href="svp_acl_rt_malloc.md">svp_acl_rt_malloc</a>接口申请大块内存并自行划分、管理内存时，用户在管理内存时，模型输出数据的内存有对齐和补齐要求，首地址和stride需要16字节对齐。</p>
</td>
</tr>
...
<p id="p2484mcpsimp"><a name="p2484mcpsimp"></a><a name="p2484mcpsimp"></a>调用<a href="svp_acl_mdl_load_from_mem.md">svp_acl_mdl_load_from_mem</a>接口加载模型成功后，会返回模型ID。</p>
</td>
</tr>
...
<p id="p2494mcpsimp"><a name="p2494mcpsimp"></a><a name="p2494mcpsimp"></a>使用<a href="svp_acl_mdl_dataset.md">svp_acl_mdl_dataset</a>类型的数据描述模型推理时的输入数据，输入的内存地址、内存大小用<a href="svp_acl_data_buffer.md">svp_acl_data_buffer</a>类型的数据来描述。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="47%" headers="mcps1.1.4.1.3 "><p id="p2503mcpsimp"><a name="p2503mcpsimp"></a><a name="p2503mcpsimp"></a>标识动态Batch输入的输入index，需调用<a href="svp_acl_mdl_get_input_index_by_name.md">svp_acl_mdl_get_input_index_by_name</a>接口获取，输入名称固定为SVP_ACL_DYNAMIC_TENSOR_NAME。</p>
</td>
</tr>
...
<p id="p2633mcpsimp"><a name="p2633mcpsimp"></a><a name="p2633mcpsimp"></a>调用<a href="svp_acl_mdl_load_from_mem.md">svp_acl_mdl_load_from_mem</a>接口加载模型成功后，会返回模型ID。</p>
</td>
</tr>
...
<p id="p2643mcpsimp"><a name="p2643mcpsimp"></a><a name="p2643mcpsimp"></a>使用svp_acl_mdl_dataset类型的数据描述模型推理时的输入数据，输入的内存地址、内存大小用<a href="svp_acl_data_buffer.md">svp_acl_data_buffer</a>类型的数据来描述。</p>
</td>
</tr>
...
-   Dump初始化。
-   svp\_acl\_mdl\_init\_dump接口需要与[svp\_acl\_mdl\_set\_dump](svp_acl_mdl_set_dump.md)接口、[svp\_acl\_mdl\_finalize\_dump](svp_acl_mdl_finalize_dump.md)接口配合使用，用于配置Dump信息。
-   接口的调用顺序如下，以配置两个模型的Dump信息为例：[svp\_acl\_init](svp_acl_init.md)接口--\>svp\_acl\_mdl\_init\_dump接口--\>[svp\_acl\_mdl\_set\_dump](svp_acl_mdl_set_dump.md)接口--\>模型1加载--\>[svp\_acl\_mdl\_finalize\_dump](svp_acl_mdl_finalize_dump.md)接口--\>svp\_acl\_mdl\_init\_dump接口--\>[svp\_acl\_mdl\_set\_dump](svp_acl_mdl_set_dump.md)接口--\>模型2加载--\>[svp\_acl\_mdl\_finalize\_dump](svp_acl_mdl_finalize_dump.md)接口--\>执行其它任务--\>模型卸载--\>[svp\_acl\_finalize](svp_acl_finalize.md)接口

约束说明：

-   如果已经通过[svp\_acl\_init](svp_acl_init.md)接口配置了dump信息，则调用svp\_acl\_mdl\_init\_dump接口时会返回失败。
-   必须在调用[svp\_acl\_init](svp_acl_init.md)接口之后、模型加载接口之前调用svp\_acl\_mdl\_init\_dump接口。
...
-   设置dump参数。
-   svp\_acl\_mdl\_set\_dump接口需要与[svp\_acl\_mdl\_init\_dump](svp_acl_mdl_init_dump.md)接口、[svp\_acl\_mdl\_finalize\_dump](svp_acl_mdl_finalize_dump.md)接口配合使用，用于配置Dump信息。
-   接口的调用顺序如下，以配置两个模型的Dump信息为例：[svp\_acl\_init](svp_acl_init.md)接口--\>[svp\_acl\_mdl\_init\_dump](svp_acl_mdl_init_dump.md)接口--\>svp\_acl\_mdl\_set\_dump接口--\>模型1加载--\>[svp\_acl\_mdl\_finalize\_dump](svp_acl_mdl_finalize_dump.md)接口--\>[svp\_acl\_mdl\_init\_dump](svp_acl_mdl_init_dump.md)接口--\>svp\_acl\_mdl\_set\_dump接口--\>模型2加载--\>[svp\_acl\_mdl\_finalize\_dump](svp_acl_mdl_finalize_dump.md)接口--\>执行其它任务--\>模型卸载--\>[svp\_acl\_finalize](svp_acl_finalize.md)接口
...
-   只有在调用本接口之后加载模型，配置的Dump信息有效。在调用本接口之前已经加载的模型不受影响，除非用户在调用本接口后重新加载该模型。
    ...
    [svp\_acl\_mdl\_init\_dump](svp_acl_mdl_init_dump.md)接口--\>模型1加载--\>svp\_acl\_mdl\_set\_dump接口--\>模型2加载--\>[svp\_acl\_mdl\_finalize\_dump](svp_acl_mdl_finalize_dump.md)接口

-   多次调用本接口对同一个模型配置了Dump信息，系统内处理时会采用覆盖策略。

    例如以下接口调用顺序中，第二次调用本接口配置的Dump信息会覆盖第一次配置的Dump信息：

    [svp\_acl\_mdl\_init\_dump](svp_acl_mdl_init_dump.md)接口--\>svp\_acl\_mdl\_set\_dump接口--\>svp\_acl\_mdl\_set\_dump接口--\>模型1加载--\>[svp\_acl\_mdl\_finalize\_dump](svp_acl_mdl_finalize_dump.md)接口
...
-   Dump去初始化。
-   svp\_acl\_mdl\_finalize\_dump接口需要与[svp\_acl\_mdl\_init\_dump](svp_acl_mdl_init_dump.md)接口、[svp\_acl\_mdl\_set\_dump](svp_acl_mdl_set_dump.md)接口配合使用，用于配置Dump信息。
-   接口的调用顺序如下，以配置两个模型的Dump信息为例：[svp\_acl\_init](svp_acl_init.md)接口--\>[svp\_acl\_mdl\_init\_dump](svp_acl_mdl_init_dump.md)接口--\>[svp\_acl\_mdl\_set\_dump](svp_acl_mdl_set_dump.md)接口--\>模型1加载--\>svp\_acl\_mdl\_finalize\_dump接口--\>[svp\_acl\_mdl\_init\_dump](svp_acl_mdl_init_dump.md)接口--\>[svp\_acl\_mdl\_set\_dump](svp_acl_mdl_set_dump.md)接口--\>模型2加载--\>svp\_acl\_mdl\_finalize\_dump接口--\>执行其它任务--\>模型卸载--\>[svp\_acl\_finalize](svp_acl_finalize.md)接口
...
1.  调用[svp\_acl\_mdl\_create\_config\_handle](svp_acl_mdl_create_config_handle.md)接口创建模型加载的配置对象。
2.  多次调用svp\_acl\_mdl\_set\_config\_opt接口设置配置对象中每个属性的值。
3.  调用[svp\_acl\_mdl\_load\_with\_config](svp_acl_mdl_load_with_config.md)接口指定模型加载时需要的配置信息，并进行模型加载。
4.  模型加载成功后，调用[svp\_acl\_mdl\_destroy\_config\_handle](svp_acl_mdl_destroy_config_handle.md)接口销毁。
5.  值得注意的是通过[svp\_acl\_mdl\_load\_with\_config](svp_acl_mdl_load_with_config.md)接口加载的模型同样需要调用svp\_acl\_mdl\_unload接口进行模型卸载。
...
<td class="cellrowborder" valign="top" width="57.31573157315732%" headers="mcps1.1.4.1.3 "><p id="p59101418145717"><a name="p59101418145717"></a><a name="p59101418145717"></a>指定模型加载的配置对象。需提前调用<a href="svp_acl_mdl_create_config_handle.md">svp_acl_mdl_create_config_handle</a>接口创建该对象。</p>
</td>
</tr>
...
1.  调用[svp\_acl\_mdl\_create\_config\_handle](svp_acl_mdl_create_config_handle.md)接口创建模型加载的配置对象。
2.  多次调用[svp\_acl\_mdl\_set\_config\_opt](svp_acl_mdl_set_config_opt.md)接口设置配置对象中每个属性的值。
...
<td class="cellrowborder" valign="top" width="65.85658565856586%" headers="mcps1.1.4.1.3 "><p id="p3831534105919"><a name="p3831534105919"></a><a name="p3831534105919"></a>指定模型加载的配置对象。需提前调用<a href="svp_acl_mdl_create_config_handle.md">svp_acl_mdl_create_config_handle</a>接口创建该对象，与<a href="svp_acl_mdl_set_config_opt.md">svp_acl_mdl_set_config_opt</a>中的handle保持一致。</p>
</td>
</tr>
...
使用本接口获取模型中_AA_PP的信息，只能获取[svp\_acl\_aapp\_info](svp_acl_aapp_info.md)结构体中inputFormat的值，其他的值无法获取。
...
<td class="cellrowborder" valign="top" width="65.85658565856586%" headers="mcps1.1.4.1.3 "><p id="p3831534105919"><a name="p3831534105919"></a><a name="p3831534105919"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="65.85658565856586%" headers="mcps1.1.4.1.3 "><p id="p2503mcpsimp"><a name="p2503mcpsimp"></a><a name="p2503mcpsimp"></a>标识动态HW输入的输入index，需调用<a href="svp_acl_mdl_get_input_index_by_name.md">svp_acl_mdl_get_input_index_by_name</a>接口获取，输入名称固定为SVP_ACL_DYNAMIC_TENSOR_NAME。</p>
</td>
</tr>
...
<p id="p862315919430"><a name="p862315919430"></a><a name="p862315919430"></a>可以调用<a href="svp_acl_mdl_get_dynamic_hw.md">svp_acl_mdl_get_dynamic_hw</a>接口获取指定模型支持的分辨率档位数以及每一档中的宽、高。</p>
</td>
</tr>
...
<p id="p11324174884611"><a name="p11324174884611"></a><a name="p11324174884611"></a>可以调用<a href="svp_acl_mdl_get_dynamic_hw.md">svp_acl_mdl_get_dynamic_hw</a>接口获取指定模型支持的分辨率档位数以及每一档中的宽、高。</p>
</td>
</tr>
</tbody>
</table>
...
-   方式一：将采集到的Profiling数据写入文件，再使用Profiling工具解析该文件，并展示性能分析数据
    ...
    -   [svp\_acl\_prof\_init](svp_acl_prof_init.md)接口、[svp\_acl\_prof\_start](svp_acl_prof_start.md)接口、[svp\_acl\_prof\_stop](svp_acl_prof_stop.md)接口、[svp\_acl\_prof\_finalize](svp_acl_prof_finalize.md)接口配合使用，实现该方式的性能数据采集。该方式可获取SVP ACL的接口性能数据、_模式识别_Core上算子的执行时间、模式识别CPU算子的执行时间等。目前这些接口为进程级控制，表示在进程内任意线程调用该接口，其它线程都会生效。

        一个进程内，可以根据需求多次调用这些接口，基于不同的Profiling采集配置，采集数据。

    -   调用[svp\_acl\_init](svp_acl_init.md)接口，在SVP ACL初始化阶段，通过\*.json文件传入要采集的Profiling数据。该方式可获取SVP ACL的接口性能数据、模式识别Core上算子的执行时间、模式识别CPU算子的执行时间等。

        一个进程内，只能调用一次[svp\_acl\_init](svp_acl_init.md)接口，如果要修改Profiling采集配置，需修改\*.json文件中的配置。详细使用说明请参见接口处的说明，不在本章节描述。

-   方式二：将采集到的Profiling数据解析后写入管道，由用户读入内存，再由用户调用SVP ACL的接口获取性能数据

    [svp\_acl\_prof\_model\_subscribe](svp_acl_prof_model_subscribe.md)接口、svp\_acl\_prof\_get\*接口、[svp\_acl\_prof\_model\_unsubscribe](svp_acl_prof_model_unsubscribe.md)接口配合使用，实现该方式的性能数据采集，当前支持获取网络模型中模式识别Core算子的性能数据，包括算子名称、算子类型名称、算子执行时间等。

总体约束：两种方式的Profiling性能数据采集接口不能交叉调用。

-   [svp\_acl\_prof\_init](svp_acl_prof_init.md)接口和[svp\_acl\_prof\_finalize](svp_acl_prof_finalize.md)接口之间不能调用[svp\_acl\_prof\_model\_subscribe](svp_acl_prof_model_subscribe.md)接口、svp\_acl\_prof\_get\*接口、[svp\_acl\_prof\_model\_unsubscribe](svp_acl_prof_model_unsubscribe.md)接口；
-   [svp\_acl\_prof\_model\_subscribe](svp_acl_prof_model_subscribe.md)接口和[svp\_acl\_prof\_model\_unsubscribe](svp_acl_prof_model_unsubscribe.md)接口之间不能调用[svp\_acl\_prof\_init](svp_acl_prof_init.md)接口、[svp\_acl\_prof\_start](svp_acl_prof_start.md)接口、[svp\_acl\_prof\_stop](svp_acl_prof_stop.md)接口、[svp\_acl\_prof\_finalize](svp_acl_prof_finalize.md)接口。
...
-   **调用接口要求**：
    -   [svp\_acl\_prof\_init](svp_acl_prof_init.md)接口必须在svp\_acl\_init接口之后、模型加载之前调用。

        如果已经通过[svp\_acl\_init](svp_acl_init.md)接口配置了Profiling信息，则调用[svp\_acl\_prof\_init](svp_acl_prof_init.md)接口、[svp\_acl\_prof\_start](svp_acl_prof_start.md)接口、[svp\_acl\_prof\_stop](svp_acl_prof_stop.md)接口、[svp\_acl\_prof\_finalize](svp_acl_prof_finalize.md)时，会返回报错。

        如果没有调用[svp\_acl\_prof\_init](svp_acl_prof_init.md)接口，调用[svp\_acl\_prof\_start](svp_acl_prof_start.md)接口、[svp\_acl\_prof\_stop](svp_acl_prof_stop.md)接口、[svp\_acl\_prof\_finalize](svp_acl_prof_finalize.md)时，会返回报错。

    -   [svp\_acl\_prof\_start](svp_acl_prof_start.md)接口在模型执行之前调用，若在模型执行过程中调用[svp\_acl\_prof\_start](svp_acl_prof_start.md)接口，Profling采集到的数据为调用[svp\_acl\_prof\_start](svp_acl_prof_start.md)接口之后的数据，可能导致数据不完整。

        调用[svp\_acl\_prof\_start](svp_acl_prof_start.md)接口时，可以指定从一个Device上采集性能数据，也可以指定从多个Device上采集性能数据。

        一个用户APP进程内，如果连续调用多次[svp\_acl\_prof\_start](svp_acl_prof_start.md)接口，指定重复的Profiling配置，或指定的Device重复，会返回报错。

    -   在用户APP的进程生命周期内，[svp\_acl\_prof\_init](svp_acl_prof_init.md)接口与[svp\_acl\_prof\_finalize](svp_acl_prof_finalize.md)接口配对使用，建议只调用一次，如该组合多次调用可以改变保存性能数据的文件的路径。
    -   [svp\_acl\_prof\_start](svp_acl_prof_start.md)接口与[svp\_acl\_prof\_stop](svp_acl_prof_stop.md)接口需配对使用。

-   **接口调用顺序：**
    -   **建议的接口调用顺序如下**，以“一个用户APP进程内采集多个模型推理时的性能数据”为例：

        [svp\_acl\_init](svp_acl_init.md)接口--\>[svp\_acl\_prof\_init](svp_acl_prof_init.md)接口--\>[svp\_acl\_prof\_start](svp_acl_prof_start.md)**接口**--\>模型1加载--\>模型1执行--\>模型2加载--\>模型2执行--\>模型卸载--\>[svp\_acl\_prof\_stop](svp_acl_prof_stop.md)**接口**\(与[svp\_acl\_prof\_start](svp_acl_prof_start.md)接口的[svp\_acl\_prof\_config](svp_acl_prof_config.md)数据保持一致\)--\>[svp\_acl\_prof\_finalize](svp_acl_prof_finalize.md)接口--\>执行其它任务--\>[svp\_acl\_finalize](svp_acl_finalize.md)接口

    -   **错误的接口调用顺序示例如下**，以“一个用户APP进程内，如果连续调用多次[svp\_acl\_prof\_start](svp_acl_prof_start.md)接口，指定的Device重复”为例：

        [svp\_acl\_init](svp_acl_init.md)接口--\>[svp\_acl\_prof\_init](svp_acl_prof_init.md)接口--\>[svp\_acl\_prof\_start](svp_acl_prof_start.md)**接口**\(指定Device 0\)--\>[svp\_acl\_prof\_start](svp_acl_prof_start.md)**接口**\(指定**Device 0**\)--\>模型1加载--\>模型1执行--\>模型2加载--\>模型2执行--\>[svp\_acl\_prof\_stop](svp_acl_prof_stop.md)**接口**--\>[svp\_acl\_prof\_stop](svp_acl_prof_stop.md)**接口**--\>[svp\_acl\_prof\_finalize](svp_acl_prof_finalize.md)--\>执行其它任务--\>模型卸载--\>[svp\_acl\_finalize](svp_acl_finalize.md)接口

方式二的接口约束说明

-   接口调用要求：
    -   [svp\_acl\_prof\_model\_subscribe](svp_acl_prof_model_subscribe.md)接口在模型执行之前调用，若在模型执行过程中调用[svp\_acl\_prof\_model\_subscribe](svp_acl_prof_model_subscribe.md)接口，Profling采集到的数据为调用[svp\_acl\_prof\_model\_subscribe](svp_acl_prof_model_subscribe.md)接口之后的数据，可能导致数据不完整。
    -   [svp\_acl\_prof\_model\_subscribe](svp_acl_prof_model_subscribe.md)接口需与[svp\_acl\_prof\_model\_unsubscribe](svp_acl_prof_model_unsubscribe.md)接口配对使用，不能在调用接口前，多次调用

        [svp\_acl\_prof\_model\_subscribe](svp_acl_prof_model_subscribe.md)接口重复订阅相同的模型。

    -   不能调用[svp\_acl\_prof\_model\_subscribe](svp_acl_prof_model_subscribe.md)接口订阅不存在的模型ID。
    -   不能调用[svp\_acl\_prof\_model\_unsubscribe](svp_acl_prof_model_unsubscribe.md)接口取消订阅不存在的模型ID或未订阅过的模型ID。
    -   如果在同一个Device上加载了多个模型，只能对多个模型下发同样的订阅配置。

-   接口调用顺序：
    -   建议的接口调用顺序如下：

        模型加载--\>[svp\_acl\_prof\_model\_subscribe](svp_acl_prof_model_subscribe.md)接口--\>[svp\_acl\_prof\_get\_op\_desc\_size](svp_acl_prof_get_op_desc_size.md)接口--\>[svp\_acl\_prof\_get\_op\_num](svp_acl_prof_get_op_num.md)接口--\>[svp\_acl\_prof\_get\_op\_type](svp_acl_prof_get_op_type.md)/[svp\_acl\_prof\_get\_op\_name](svp_acl_prof_get_op_name.md)/[svp\_acl\_prof\_get\_op\_start](svp_acl_prof_get_op_start.md)/[svp\_acl\_prof\_get\_op\_end](svp_acl_prof_get_op_end.md)/[svp\_acl\_prof\_get\_op\_duration](svp_acl_prof_get_op_duration.md)/[svp\_acl\_prof\_get\_model\_id](svp_acl_prof_get_model_id.md)接口--\>[svp\_acl\_prof\_model\_unsubscribe](svp_acl_prof_model_unsubscribe.md)接口

    -   错误的接口调用顺序示例如下，以重复定义同一个模型为例：

        模型1加载--\>[svp\_acl\_prof\_model\_subscribe](svp_acl_prof_model_subscribe.md)接口\(指定模型1\)--\>[svp\_acl\_prof\_model\_subscribe](svp_acl_prof_model_subscribe.md)接口\(指定模型1\)--\>[svp\_acl\_prof\_model\_unsubscribe](svp_acl_prof_model_unsubscribe.md)接口
...
参考资源：接口调用示例，参见[Profiling性能数据采集](Profiling性能数据采集.md)。
...
<p id="p556mcpsimp"><a name="p556mcpsimp"></a><a name="p556mcpsimp"></a>需提前调用<a href="svp_acl_prof_create_config.md">svp_acl_prof_create_config</a>接口创建svp_acl_prof_config类型的数据。</p>
</td>
</tr>
</tbody>
</table>
...
<p id="p1777mcpsimp"><a name="p1777mcpsimp"></a><a name="p1777mcpsimp"></a>与<a href="svp_acl_prof_start.md">svp_acl_prof_start</a>接口中的<a href="svp_acl_prof_config.md">svp_acl_prof_config</a>类型数据保持一致。</p>
</td>
</tr>
</tbody>
</table>
...
函数功能：网络场景下，订阅算子的基本信息，包括算子名称、算子类型、算子执行耗时等。同步接口。需要与[svp\_acl\_prof\_model\_unsubscribe](svp_acl_prof_model_unsubscribe.md)接口配对使用。
...
<p id="p7503mcpsimp"><a name="p7503mcpsimp"></a><a name="p7503mcpsimp"></a>调用<a href="svp_acl_mdl_load_from_mem.md">svp_acl_mdl_load_from_mem</a>接口加载模型成功后，会返回模型ID。</p>
</td>
</tr>
...
<p id="p7512mcpsimp"><a name="p7512mcpsimp"></a><a name="p7512mcpsimp"></a>需提前调用<a href="svp_acl_prof_create_subscribe_config.md">svp_acl_prof_create_subscribe_config</a>接口创建<a href="svp_acl_prof_subscribe_config.md">svp_acl_prof_subscribe_config</a>类型的数据。</p>
</td>
</tr>
</tbody>
</table>
...
函数功能：网络场景下，取消订阅算子的基本信息，包括算子名称、算子类型、算子执行耗时等。同步接口。需要与[svp\_acl\_prof\_model\_subscribe](svp_acl_prof_model_subscribe.md)接口配对使用。
...
<p id="p1400mcpsimp"><a name="p1400mcpsimp"></a><a name="p1400mcpsimp"></a>调用<a href="svp_acl_prof_get_op_desc_size.md">svp_acl_prof_get_op_desc_size</a>接口获取到单个算子数据结构的大小后，用户需按照“单个算子数据结构的大小*整数系数”得到的数值申请内存，用于存放Profiling采集到的算子信息数据，作为本接口的输入。</p>
</td>
</tr>
...
<p id="p6372mcpsimp"><a name="p6372mcpsimp"></a><a name="p6372mcpsimp"></a>用户调用<a href="svp_acl_prof_get_op_num.md">svp_acl_prof_get_op_num</a>接口获取算子数量后，这个index的取值范围：[0, (算子数量-1)]</p>
</td>
</tr>
...
<p id="p5633mcpsimp"><a name="p5633mcpsimp"></a><a name="p5633mcpsimp"></a>用户调用<a href="svp_acl_prof_get_op_num.md">svp_acl_prof_get_op_num</a>接口获取算子数量后，这个index的取值范围：[0, (算子数量-1)]</p>
</td>
</tr>
...
<p id="p4233mcpsimp"><a name="p4233mcpsimp"></a><a name="p4233mcpsimp"></a>用户调用<a href="svp_acl_prof_get_op_num.md">svp_acl_prof_get_op_num</a>接口获取算子数量后，这个index的取值范围：[0, (算子数量-1)]</p>
</td>
</tr>
...
<p id="p4867mcpsimp"><a name="p4867mcpsimp"></a><a name="p4867mcpsimp"></a>用户调用<a href="svp_acl_prof_get_op_num.md">svp_acl_prof_get_op_num</a>接口获取算子数量后，这个index的取值范围：[0, (算子数量-1)]</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.3 "><p id="p4882mcpsimp"><a name="p4882mcpsimp"></a><a name="p4882mcpsimp"></a>op_name的实际内存申请长度。取值范围建议不小于<a href="svp_acl_prof_get_op_name_len.md">svp_acl_prof_get_op_name_len</a>。</p>
</td>
</tr>
</tbody>
</table>
...
<p id="p2192mcpsimp"><a name="p2192mcpsimp"></a><a name="p2192mcpsimp"></a>用户调用<a href="svp_acl_prof_get_op_num.md">svp_acl_prof_get_op_num</a>接口获取算子数量后，这个index的取值范围：[0, (算子数量-1)]</p>
</td>
</tr>
</tbody>
</table>
...
<p id="p6835mcpsimp"><a name="p6835mcpsimp"></a><a name="p6835mcpsimp"></a>用户调用<a href="svp_acl_prof_get_op_num.md">svp_acl_prof_get_op_num</a>接口获取算子数量后，这个index的取值范围：[0, (算子数量-1)]</p>
</td>
</tr>
</tbody>
</table>
...
<p id="p4183mcpsimp"><a name="p4183mcpsimp"></a><a name="p4183mcpsimp"></a>用户调用<a href="svp_acl_prof_get_op_num.md">svp_acl_prof_get_op_num</a>接口获取算子数量后，这个index的取值范围：[0, (算子数量-1)]</p>
</td>
</tr>
</tbody>
</table>
...
<p id="p6323mcpsimp"><a name="p6323mcpsimp"></a><a name="p6323mcpsimp"></a>用户调用<a href="svp_acl_prof_get_op_num.md">svp_acl_prof_get_op_num</a>接口获取算子数量后，这个index的取值范围：[0, (算子数量-1)]</p>
</td>
</tr>
</tbody>
</table>
...
返回值说明：返回[svp\_acl\_data\_type](svp_acl_data_type.md)数据的大小，单位bit。
...
<p id="p4769327393"><a name="p4769327393"></a><a name="p4769327393"></a>调用<a href="svp_acl_mdl_load_from_mem.md">svp_acl_mdl_load_from_mem</a>接口加载模型成功后，会返回模型ID。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.3 "><p id="p4942mcpsimp"><a name="p4942mcpsimp"></a><a name="p4942mcpsimp"></a>请检查是否已调用<a href="svp_acl_init.md">svp_acl_init</a>接口进行初始化，请确保已调用接口，且在其它SVP ACL接口之前调用。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.3 "><p id="p5170mcpsimp"><a name="p5170mcpsimp"></a><a name="p5170mcpsimp"></a>请检查针对同一个Stream，是否重复调用<a href="svp_acl_rt_unsubscribe_report.md">svp_acl_rt_unsubscribe_report</a>接口。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.3 "><p id="p5179mcpsimp"><a name="p5179mcpsimp"></a><a name="p5179mcpsimp"></a>请检查是否已调用<a href="svp_acl_rt_subscribe_report.md">svp_acl_rt_subscribe_report</a>接口。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.3 "><p id="p5188mcpsimp"><a name="p5188mcpsimp"></a><a name="p5188mcpsimp"></a>请检查是否已调用<a href="svp_acl_rt_subscribe_report.md">svp_acl_rt_subscribe_report</a>接口。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.3 "><p id="p5197mcpsimp"><a name="p5197mcpsimp"></a><a name="p5197mcpsimp"></a>请检查是否已调用<a href="svp_acl_rt_launch_callback.md">svp_acl_rt_launch_callback</a>接口下发callback任务；</p>
<p id="p5200mcpsimp"><a name="p5200mcpsimp"></a><a name="p5200mcpsimp"></a>请检查<a href="svp_acl_rt_process_report.md">svp_acl_rt_process_report</a>接口中超时时间是否合理；</p>
<p id="p5203mcpsimp"><a name="p5203mcpsimp"></a><a name="p5203mcpsimp"></a>请检查callback任务是否已经处理完成，如果已处理完成，但还调用<a href="svp_acl_rt_process_report.md">svp_acl_rt_process_report</a>接口，则需优化代码逻辑。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.3 "><p id="p5212mcpsimp"><a name="p5212mcpsimp"></a><a name="p5212mcpsimp"></a>请检查是否重复调用<a href="svp_acl_finalize.md">svp_acl_finalize</a>接口进行去初始化。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.3 "><p id="p5269mcpsimp"><a name="p5269mcpsimp"></a><a name="p5269mcpsimp"></a>请检查在调用Dump接口配置Dump信息前，是否已调用<a href="svp_acl_init.md">svp_acl_init</a>接口配置Dump信息，如是，请调整代码逻辑，保留一种方式配置Dump信息即可。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.3 "><p id="p5299mcpsimp"><a name="p5299mcpsimp"></a><a name="p5299mcpsimp"></a>请检查算子缓存信息老化配置，参考<a href="svp_acl_init.md">svp_acl_init</a>处的配置说明及示例。</p>
</td>
</tr>
...
<p id="p6869mcpsimp"><a name="p6869mcpsimp"></a><a name="p6869mcpsimp"></a>该内存需由用户自行管理，调用<a href="svp_acl_rt_malloc.md">svp_acl_rt_malloc</a>接口/<a href="svp_acl_rt_free.md">svp_acl_rt_free</a>接口申请/释放内存，或调用<a href="svp_acl_rt_malloc_host.md">svp_acl_rt_malloc_host</a>接口/<a href="svp_acl_rt_free_host.md">svp_acl_rt_free_host</a>接口申请/释放内存。</p>
</td>
</tr>
...
<p id="p6886mcpsimp"><a name="p6886mcpsimp"></a><a name="p6886mcpsimp"></a>可参照<a href="svp_acl_mdl_get_input_size_by_index.md">svp_acl_mdl_get_input_size_by_index</a>或<a href="svp_acl_mdl_get_output_size_by_index.md">svp_acl_mdl_get_output_size_by_index</a>中描述的计算方法计算获取。</p>
</td>
</tr>
...
<p id="p6897mcpsimp"><a name="p6897mcpsimp"></a><a name="p6897mcpsimp"></a>可参照<a href="svp_acl_mdl_get_input_default_stride.md">svp_acl_mdl_get_input_default_stride</a>或<a href="svp_acl_mdl_get_output_default_stride.md">svp_acl_mdl_get_output_default_stride</a>中的计算方法计算获得。</p>
</td>
</tr>
</tbody>
</table>
...
-   此处仅销毁svp\_acl\_data\_buffer类型的数据，调用[svp\_acl\_create\_data\_buffer](svp_acl_create_data_buffer.md)接口创建svp\_acl\_data\_buffer类型数据时传入的data的内存需由用户自行释放。
...
返回值说明：返回[svp\_acl\_data\_buffer](svp_acl_data_buffer.md)类型中的数据的内存地址。
...
返回值说明：[svp\_acl\_data\_buffer](svp_acl_data_buffer.md)类型中数据的内存大小。
...
返回值说明：[svp\_acl\_data\_buffer](svp_acl_data_buffer.md)类型中stride的大小。
...
<p id="p7438mcpsimp"><a name="p7438mcpsimp"></a><a name="p7438mcpsimp"></a>该内存需由用户自行管理，调用<a href="svp_acl_rt_malloc.md">svp_acl_rt_malloc</a>接口/<a href="svp_acl_rt_free.md">svp_acl_rt_free</a>接口申请/释放内存，或调用<a href="svp_acl_rt_malloc_host.md">svp_acl_rt_malloc_host</a>接口/<a href="svp_acl_rt_free_host.md">svp_acl_rt_free_host</a>接口申请/释放内存。</p>
</td>
</tr>
...
<p id="p7463mcpsimp"><a name="p7463mcpsimp"></a><a name="p7463mcpsimp"></a>可参照<a href="svp_acl_mdl_get_input_default_stride.md">svp_acl_mdl_get_input_default_stride</a>或<a href="svp_acl_mdl_get_output_default_stride.md">svp_acl_mdl_get_output_default_stride</a>参照中的计算方法计算获得。</p>
</td>
</tr>
</tbody>
</table>
...
函数功能：创建svp\_acl\_mdl\_dataset类型的数据，该数据类型用于描述模型推理时的输入数据、输出数据，模型可能存在多个输入、多个输出，每个输入/输出的内存地址、内存大小用[svp\_acl\_data\_buffer](svp_acl_data_buffer.md)类型的数据来描述。同步接口。
...
<td class="cellrowborder" valign="top" width="46%" headers="mcps1.1.4.1.3 "><p id="p3905mcpsimp"><a name="p3905mcpsimp"></a><a name="p3905mcpsimp"></a><a href="svp_acl_mdl_dataset.md">svp_acl_mdl_dataset</a>地址。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="46%" headers="mcps1.1.4.1.3 "><p id="p3912mcpsimp"><a name="p3912mcpsimp"></a><a name="p3912mcpsimp"></a>表明获取的是第几个<a href="svp_acl_data_buffer.md">svp_acl_data_buffer</a>。</p>
</td>
</tr>
</tbody>
</table>
...
-   获取成功，返回[svp\_acl\_data\_buffer](svp_acl_data_buffer.md)的地址。
-   获取失败返回空地址。
...
<td class="cellrowborder" valign="top" width="54%" headers="mcps1.1.4.1.3 "><p id="p4104mcpsimp"><a name="p4104mcpsimp"></a><a name="p4104mcpsimp"></a>待销毁的<a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型的指针。</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.3 "><p id="p6098mcpsimp"><a name="p6098mcpsimp"></a><a name="p6098mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
<p id="p6108mcpsimp"><a name="p6108mcpsimp"></a><a name="p6108mcpsimp"></a><a href="svp_acl_mdl_load_from_mem.md">svp_acl_mdl_load_from_mem</a>接口加载模型成功后，会返回模型ID。</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="46%" headers="mcps1.1.4.1.3 "><p id="p5459mcpsimp"><a name="p5459mcpsimp"></a><a name="p5459mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="46%" headers="mcps1.1.4.1.3 "><p id="p4067mcpsimp"><a name="p4067mcpsimp"></a><a name="p4067mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.3 "><p id="p2546mcpsimp"><a name="p2546mcpsimp"></a><a name="p2546mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
-   其中d\_i表示第i维长度，n表示指定输入的维度个数，default\_stride为默认stride\(详细请参见[svp\_acl\_mdl\_get\_input\_default\_stride](svp_acl_mdl_get_input_default_stride.md)\)。
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.3 "><p id="p1636mcpsimp"><a name="p1636mcpsimp"></a><a name="p1636mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
-   返回output\_size = d\_0 \* d\_1 \*… \* d\_n-2 \* default\_stride，其中d\_i表示第i维长度，n表示指定输出的维度个数，default\_stride为默认stride\(详细请参见[svp\_acl\_mdl\_get\_output\_default\_stride](svp_acl_mdl_get_output_default_stride.md)\)。
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.2.4.1.3 "><p id="p441mcpsimp"><a name="p441mcpsimp"></a><a name="p441mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.3 "><p id="p3247mcpsimp"><a name="p3247mcpsimp"></a><a name="p3247mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.3 "><p id="p894mcpsimp"><a name="p894mcpsimp"></a><a name="p894mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.3 "><p id="p2313mcpsimp"><a name="p2313mcpsimp"></a><a name="p2313mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.3 "><p id="p1894mcpsimp"><a name="p1894mcpsimp"></a><a name="p1894mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.3 "><p id="p6666mcpsimp"><a name="p6666mcpsimp"></a><a name="p6666mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="64%" headers="mcps1.2.4.1.3 "><p id="p1021mcpsimp"><a name="p1021mcpsimp"></a><a name="p1021mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.3 "><p id="p5953mcpsimp"><a name="p5953mcpsimp"></a><a name="p5953mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.3 "><p id="p223mcpsimp"><a name="p223mcpsimp"></a><a name="p223mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.3 "><p id="p7274mcpsimp"><a name="p7274mcpsimp"></a><a name="p7274mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.3 "><p id="p5828mcpsimp"><a name="p5828mcpsimp"></a><a name="p5828mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
-   默认stride = ceil \(ceil\(last\_dim \*  [svp\_acl\_data\_type\_size](svp_acl_data_type_size.md)\(data\_type\) / byte\_bit\_num\) / default\_align\_byte\) \* default\_align\_byte。
...
<td class="cellrowborder" valign="top" width="68.32000000000001%" headers="mcps1.1.4.1.3 "><p id="p4806mcpsimp"><a name="p4806mcpsimp"></a><a name="p4806mcpsimp"></a><a href="svp_acl_mdl_desc.md">svp_acl_mdl_desc</a>类型数据的指针。</p>
</td>
</tr>
...
-   如需销毁svp\_acl\_prof\_config类型的数据，请参见[svp\_acl\_prof\_destroy\_config](svp_acl_prof_destroy_config.md)。
...
函数功能：销毁通过[svp\_acl\_prof\_create\_config](svp_acl_prof_create_config.md)接口创建的svp\_acl\_prof\_config类型的数据。同步接口。
...
-   如需销毁svp\_acl\_prof\_subscribe\_config类型的数据，请参见[svp\_acl\_prof\_destroy\_subscribe\_config](svp_acl_prof_destroy_subscribe_config.md)。
...
函数功能：销毁通过[svp\_acl\_prof\_create\_subscribe\_config](svp_acl_prof_create_subscribe_config.md)接口创建的svp\_acl\_prof\_subscribe\_config类型的数据。同步接口。
...
<td class="cellrowborder" valign="top" width="52%" headers="mcps1.1.4.1.3 "><p id="p596mcpsimp"><a name="p596mcpsimp"></a><a name="p596mcpsimp"></a>待销毁的<a href="svp_acl_prof_subscribe_config.md">svp_acl_prof_subscribe_config</a>类型的指针。</p>
</td>
</tr>
</tbody>
</table>
...
函数功能：创建[svp\_acl\_mdl\_config\_handle](svp_acl_mdl_config_handle.md)类型的数据，表示一个模型加载的配置对象，同步接口。如需销毁[svp\_acl\_mdl\_config\_handle](svp_acl_mdl_config_handle.md)类型的数据，请参见[svp\_acl\_mdl\_destroy\_config\_handle](svp_acl_mdl_destroy_config_handle.md)。
...
-   返回[svp\_acl\_mdl\_config\_handle](svp_acl_mdl_config_handle.md)类型的指针，表示成功。
-   返回nullptr，表示失败。

#### svp\_acl\_mdl\_destroy\_config\_handle<a name="ZH-CN_TOPIC_0000002442020777"></a>

函数功能：销毁通过[svp\_acl\_mdl\_create\_config\_handle](svp_acl_mdl_create_config_handle.md)接口创建的svp\_acl\_mdl\_config\_handle类型的数据。同步接口。
...
<td class="cellrowborder" valign="top" width="51.519999999999996%" headers="mcps1.1.4.1.3 "><p id="p1859213585912"><a name="p1859213585912"></a><a name="p1859213585912"></a>待销毁的<a href="svp_acl_mdl_config_handle.md">svp_acl_mdl_config_handle</a>类型的指针。</p>
</td>
</tr>
</tbody>
</table>
...
当前版本支持的样例以及样例在各形态的编译运行指导请参见[如何获取Sample](如何获取Sample.md)。
...
参考[如何获取Sample](如何获取Sample.md)，获取样例代码。
...
<img src="figures/Sample示例.png" alt="" title="Sample示例" />

#### 原理介绍<a name="ZH-CN_TOPIC_0000002441981025"></a>

在该Sample中，涉及的关键功能点，如[表1](#table607mcpsimp)所示。API接口的详细介绍请参见[SVP ACL API参考](SVP-ACL-API参考.md)。
...
<img src="figures/Sample示例-0.png" alt="" title="Sample示例-0" />

#### 原理介绍<a name="ZH-CN_TOPIC_0000002408581814"></a>

在该Sample中，涉及的关键功能点，如[表1](#table4320mcpsimp)所示。API接口的详细介绍请参见[SVP ACL API参考](SVP-ACL-API参考.md)。
