...
部署架构如[图1]所示。NNN环境包含 PC 端工具侧开发环境和单板侧板端环境，当一个训练好的模型过来后，首先可以经过AMCT（Advanced Model Compression Toolkit，模型压缩工具）进行量化，将模型中部分层量化为 8bit 计算，提升计算效率；其次使用ATC（Advanced Tensor Compiler）工具将量化后的模型或非量化的模型转换为Ascend NNN认识的离线模型；最后，离线模型放置在板端环境，即可进行推理。
...
本章节介绍以原始训练好的模型如何在SVP\_NNN上执行的整体使用流程，运行流程如[图1]所示。
...
1.  准备离线模型om
    ...
    确认model目录下有yolov5\_original.om离线模型。模型编译，请参见[模型编译]。
...
执行结果保存为out\_img\_yolov5.jpg，如[图1]所示。
...
MindCmd是一个Python命令行工具，依赖Python3.7.5和CANN。如果还没有安装CANN软件包，请先参考[软件包安装]章节完成安装。
...
3.  查看版本信息
    ...
    执行如下命令，查看MindCmd工具的版本和工具在[2]中配置的CANN软件包版本信息。
...
2.  执行如下命令设置MindCmd工具工作路径：
    ...
    > **说明：** 
    >-   MindCmd工具与板端采用mount方式共享资源，因此上板推理时MindCmd工具的工作路径不能超出[1]配置的服务器侧挂载路径（HOST\_MOUNT\_PATH）范围。
...
本节示例为[MindCmd工具准备]完成sample网络yolov5s\_cpu.onnx的一键精度比对。所需模型文件、推理数据均位于SVP\__NNN_\_PC软件包中的2\_object\_detection/yolo样例目录，请参考资源准备[资源准备]资源准备章节完成资源准备。
...
如[图1]所示，MindCmd一键推理会自动执行原始模型推理、量化模型推理、功能仿真推理，并将推理结果进行精度比对。
...
3.  执行Oneclick
    ...
    执行Oneclick后，MindCmd会将采集到的性能数据保存在csv表格中（如[图1]所示），csv表格存放位置如[图2]所示，模型的整网性能数据打印如[图3]所示。
    ...
    **图 3**  模型整网性能数据<a name="fig1073092175015"></a>  
    ...
    通过得到的Profiling数据，可以了解模型在推理过程中各层耗时及各层耗时占比、带宽、帧率等性能数据，如[图4]所示，根据获取的性能数据，我们能分析出是否存在关键性能瓶颈，辅助模型性能调优。
