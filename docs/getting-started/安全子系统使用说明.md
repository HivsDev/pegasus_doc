...
3.  用户自定义区域：芯片集成了约25Kbit的用户自定义OTP区域，用以存放用户数据。
    ...
    在SSxxxx SDK包中，提供了OTP相关的读写接口。详细请参考本文“[SSxxxx OTP字段定义](SSxxxx-OTP字段定义.md)  ”以及文档《OTP API 参考》。
...
-   多块加密通道支持字节地址。
-   AES算法支持基于硬件根密钥派生工作密钥进行加解密（如[图1](二级KLAD.md#fig769715318593)）
...
SSxxxx 的JTAG调试接口可以通过OTP进行打开模式、密码模式、永久关闭模式的切换，默认为打开模式。OTP烧写的具体操作方法，请参照本文“[SSxxxx OTP字段定义](SSxxxx-OTP字段定义.md)”和文档《OTP API 参考》。
...
-   jtag\_prt\_mode：在jtag\_ca\_en为1（使能）的情况下有效。jtag\_prt\_mode由2bit控制不同工作模式。
    ...
    详细配置，请参考本文“[SSxxxx OTP字段定义](SSxxxx-OTP字段定义.md)”。
