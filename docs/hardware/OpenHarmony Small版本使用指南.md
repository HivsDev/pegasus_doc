...
-   为减小发布包大小，裁剪非必要的软件模块，主要裁剪的软件如[表1]所示。如果产品开发需要引入，请从社区OpenHarmony 5.1.0 Release版本中同步，放到对应目录即可。
...
1.  在ohos\\vendor\\hisilicon\\hispark\_ss928v100\_linux目录中，新建[图1]所示的rootfs目录。
...
4.  将rootfs\_binary\_files拷贝到rootfs中，还需要修改ohos\\vendor\\hisilicon\\hispark\_ss928v100\_linux\\fs.yml文件。
    ...
    如需要拷贝到自定义目录中，可根据[在rootfs中新增目录打包文件]修改。
...
编译环境目前主要支持Ubuntu18.04和Ubuntu20.04（Ubuntu22.04暂不支持）。推荐使用“apt-get和pip3 install命令安装”，如[图2]所示。

**图 2**  apt-get和pip3 install命令安装<a name="fig1466793985111"></a>  
![]

如果出现[图3]问题"Your system shell isn't bash..."，请执行如下命令。
...
2.  参考[uboot编译]生成boot\_image.bin。
3.  通过ToolPlatform加载emmc\_burn\_table.xml烧写，uboot选择sdk/osdrv/pub/ss928v100\_emmc\_image\_musl/boot\_image.bin。ToolPlatform截图信息如[图1]所示。
...
5.  在子页面中选择“Initial RAM filesystem and RAM disk \(initramfs/initrd\) support”\(按空格键进行选择，按下Save键保存）。
    ...
    **图 2**  配置Initial RAM filesystem and RAM disk \(initramfs/initrd\) support<a name="fig18004118318"></a>  
    ...
    按Exit键返回到“General setup”页面，选择Device Drivers-\> Block devices-\>RAM block device support，如[图3]所示。
...
1.  修改板端passwd，需先mount本地服务器，把板端/etc/passwd拷贝到本地服务器，对本地passwd文件按[图2]方式修改，再拷贝回去替换板端/etc/passwd文件。
...
[表1]中描述了示例应用依赖的资源文件，以及资源资源文件的目录\(路径相对于OpenHarmony根目录\)
...
2.  挂载资源
    ...
    [表1]中描述了对应资源文件的板端路径，需要按该表把资源复制到对应板端路径
