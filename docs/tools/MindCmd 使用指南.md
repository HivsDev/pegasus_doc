...
-   模型和数据准备。
    ...
    > **说明：** 
    >-   推理数据的shape应该与模型所需输入数据的shape相同，如：模型resnet50的shape为（3，224，224），图片的shape也应为（3，224，224）。否则需要自定义数据预处理方式，通过--aapp参数指定数据预处理配置文件样例，数据预处理完整配置方式请参考《ATC工具使用指南》“--insert\_op\_conf ”章节。
